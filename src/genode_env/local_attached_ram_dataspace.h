...
private:
  ...
  template <typename T> static void _swap(T &v1, T &v2) {
    T tmp = v1;
    v1 = v2;
    v2 = tmp;
  }
  ...
  void _alloc_and_attach() {
    ...
    Region_map::Attr attributes{.size = _size,
                                .offset = 0,
                                .use_at = false,
                                .executable = false,
                                .writeable = true};
...
public:
   ...
   */
  Local_attached_ram_dataspace(Ram_allocator &ram, Region_map &rm, size_t size,
                               Cache cache = CACHED)
   ...
   */
  template <typename T> T *local_addr() const {
    return static_cast<T *>(_local_addr);
  }
