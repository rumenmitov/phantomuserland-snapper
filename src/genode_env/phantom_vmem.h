...
private:
	...
	int test_pf_handler(void *address, int write, int ip, struct trap_state *ts)
	{
		...
		warning("Test page fault handler! addr=",
				Hex((addr_t)address),
				" write=",
				write,
				" ip=",
	...
	void _handle_fault()
	{
		...
		if (_pf_handler != nullptr) {
			...
			_pf_handler((void *)state.addr,
						state.type == Region_map::Fault::Type::WRITE ? 1 : 0,
						-1,
						&ts_stub);
			// });
		} else {
...
public:
	Local_fault_handler(Genode::Env &env, Region_map &region_map, size_t page_size)
		: Entrypoint(
			  env, sizeof(addr_t) * 2048, "local_fault_handler", Affinity::Location()),
		  _env(env),
...
struct Phantom::Vmem_adapter
{
	...
	Genode::size_t OBJECT_SPACE_SIZE = 1024ULL * 1024 * 128;

	/* Physical memory required */
	const Genode::uint64_t _phys_space_limit = 2ULL * OBJECT_SPACE_SIZE;
	...
	Vmem_adapter(Env &env) : _env(env)
	{
		...
		_obj_space_allocator
			.add_range(OBJECT_SPACE_START + ARCH_PAGE_SIZE * 0x10, OBJECT_SPACE_SIZE)
			.with_error([](auto error) { throw; });
		...
		if (virt_addr >= OBJECT_SPACE_START &&
							...
							_obj_space.attach(_ram_ds, attribute)
								.with_result(
									...
									[](const Region_map::Attach_error &e) {
										switch (e) {
											case Region_map::Attach_error::
												INVALID_DATASPACE:
												Genode::error("couldn't attach region "
															  "map: invalid ds");
												break;
											case Region_map::Attach_error::
												REGION_CONFLICT:
			...
			if (laddr != virt_addr - OBJECT_SPACE_START) {
				error("Mapped addr does not correspond to virtual one! Got ",
					  Hex(laddr),
					  " expected ",
					  Hex(virt_addr - OBJECT_SPACE_START));
			}
