...
#include <machdep.h>
	...
	// int arch_is_object_land_access_enabled() //< check if current thread attempts to
	// access object space having access disabled
	// {
	//     _stub_print();
	//     return 0;
	// }
							  ...
							  u_int32_t flags)
	{
		...
		if (mapped == page_map) {
			...
			Phantom::main_obj->_vmem_adapter.map_page(p,
													  (addr_t)page_start_addr,
													  writeable);
							   ...
							   int n_pages)  // alloc address of a page, but not memory
	{
		...
		(void)n_pages;
		Genode::error(
			"Trying to allocate virtual addr without physical one! Returning error");
		return 1;
	}
	...
	errno_t hal_alloc_phys_pages(physaddr_t *result, int npages)  // alloc and not map
	{
		...
		// Genode::log("Phys alloc consumed ram : ",
		// main_obj->_vmem_adapter._pseudo_phys_heap.consumed());
		...
		try {
			...
			// temp_res = main_obj->_vmem_adapter._pseudo_phys_heap.alloc(ARCH_PAGE_SIZE *
			// npages);
			temp_res = main_obj->_vmem_adapter.alloc_pseudo_phys(npages);

			*result = (physaddr_t)temp_res;

			// Genode::log("Phys alloc: numpages= : ", npages, " addr=",
			// Hex((physaddr_t)temp_res));
	...
	void hal_free_phys_page(
		physaddr_t paddr)  // alloc and not map - WILL PANIC if page is mapped!
	{
		...
		Genode::retry<Genode::Out_of_ram>(
			[&]() {
				Genode::retry<Genode::Out_of_caps>(
								...
								[](const Region_map::Attach_error &e) {
									switch (e) {
										case Region_map::Attach_error::INVALID_DATASPACE:
											Genode::error(
												"couldn't attach region map: invalid ds");
											break;
					...
					[&]() { main_obj->_vmem_adapter._rm.upgrade_caps(10); },
					16U);
			},
	...
	void memcpy_v2p(physaddr_t to, void *_from, size_t size)
	{
		void *addr;
		...
		while (size > 0) {
			...
			// Genode::log("V2P: ", Hex((addr_t)addr), " ", Hex((addr_t)from), " ",
			// stepSize);
	...
	void memcpy_p2v(void *_to, physaddr_t from, size_t size)
	{
		void *addr;
	...
	int genode_register_page_fault_handler(
		int (*pf_handler)(void *address, int write, int ip, struct trap_state *ts))
	{
