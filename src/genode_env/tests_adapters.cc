...
bool Phantom::test_obj_space()
{
	...
	log("  read     mem                         ",
		(sizeof(void *) == 8) ? "                " : "",
		Hex_range<addr_t>(main_obj->_vmem_adapter.OBJECT_SPACE_START,
...
bool Phantom::test_block_alignment(Phantom::Disk_backend &disk)
{
	...
	return phantom_sync_write_sector(&stub_partition, data, 1, 1);
	// }
}
...
{
	Genode::log("bulk classes ptr=",
				main_obj->_bulk_code_ptr,
				" size=",
				main_obj->_bulk_code_size);
