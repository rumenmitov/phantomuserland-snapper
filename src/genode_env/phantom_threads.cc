...
#include <threads.h>
	...
	int hal_start_kernel_thread_arg(void (*thread)(void *arg), void *arg)
	{
		...
		// Actually, no distinction between regular and kernel thread since in both cases
		// we will be in userspace
	...
	phantom_thread_t *get_current_thread()
	{
		...
		// PhantomGenericThread *ph_thread = (PhantomGenericThread *)Thread::myself();
		// phantom_thread_t *alloced_thread_struct = new (main_obj->_heap)
		// phantom_thread_t(ph_thread->getPhantomStruct());
