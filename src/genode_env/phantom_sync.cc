...
#include <ph_malloc.h>
	...
	int hal_mutex_init(hal_mutex_t *m, const char *name)
	{
		...
		m->impl->lock = (Phantom::Phantom_mutex_wrapper *)ph_malloc(
			sizeof(Phantom::Phantom_mutex_wrapper));
	...
	errno_t hal_cond_wait(hal_cond_t *c, hal_mutex_t *m)
	{
		...
		c->impl->counter = c->impl->counter + 1;
		c->impl->mutex->release();

		// XXX : Not really a good time to preempt the thread. It can be stuck here
		// forever
	...
	errno_t hal_cond_destroy(hal_cond_t *c)
	{
		...
		if (c->impl->counter > 0) {
			Genode::warning("Trying to destroy cond with waiting threads (counter=%d)",
							c->impl->counter);
		}
