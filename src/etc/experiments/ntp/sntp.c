...
static void SNTP_resync(void *arg)
{
	...
	for (;;) {
		...
		if (SNTPGetTime(&cur_server_addr, &t)) { /* if any error retry */
...
#endif
			if (retry >= 3) { /* if numer of retries >= 3 wait 30 secs before next retry
								 sequence ... */
				retry = 0;
...
retry:
	...
	if (raddr.port != SNTP_PORT ||
		(data.mode & 0xc0) ==
			0xc0) /* if source port is not SNTP_PORT or server is not in sync return */
	{
