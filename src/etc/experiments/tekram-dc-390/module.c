...
void amd_write8(Am53c974a *instance, unsigned int reg, unsigned char value)
{
	if (instance->verbose) {
		switch (reg) {
			case 0:
				sprintf(debug, ">Count low : %X\n", value);
				break;
			case 4:
				sprintf(debug, ">Count middle : %X\n", value);
				break;
			case 0x08:
				sprintf(debug, ">FIFO : %X\n", value);
				break;
			case 0x0c:
				sprintf(debug, ">scsi cmd : %X\n", value);
				break;
			case 0x10:
				sprintf(debug, ">scsi dest : %X\n", value);
				break;
			case 0x1c:
				sprintf(debug, ">sync off reg : %X\n", value);
				break;
			case 0x20:
				sprintf(debug, ">ctrl1 : %X\n", value);
				break;
			case 0x2c:
				sprintf(debug, ">ctrl2 : %X\n", value);
				break;
			case 0x30:
				sprintf(debug, ">ctrl3 : %X\n", value);
				break;
			case 0x34:
				sprintf(debug, ">ctrl4 : %X\n", value);
				break;
			case 0x38:
				sprintf(debug, ">Count high : %X\n", value);
				break;
			case 0x40:
				sprintf(debug, ">cmd : %X\n", value);
				break;
			default:
				sprintf(debug, ">iobase+%X : %X\n", reg, value);
		}
		debug_print(debug);
	}
...
static void amdsetsync(struct amd_softc *am,
					   u_int target,
					   u_int clockrate,
					   u_int period,
					   u_int offset,
					   u_int type)
{
	...
	if ((type & AMD_TRANS_CUR) != 0 && (old_period != period || old_offset != offset)) {
...
static int32 scsi_int_dispatch(void *data)
{
	/*
	...
	*/
	...
	if (!(am->_STATREG[am->interrupt] & INTERRUPT) &&
		!(am->_DMASTATUS[am->interrupt] & 8)) {
		// There is no interrupt from this adapter
		/*
				kprintf("AMD Warning : IRQ 9 was flaged but no intr for me (irq
	...
	if (am->ccb.command_completed) {
		/* This is an unexpected intr : what shoukd i do ? well, obviously handle it - but
		 * how ? */
	...
	*/
	if (am->bus_phase == PHASE_MSGIN) {
		...
		for (nb = 0; am->nb_data < 100 && nb < t; nb++,
			am->nb_data++) {  // nb_data is initialized once (at the beginning of IO)
			am->data[am->nb_data] = amd_read8(am, SCSIFIFOREG);
		}  // add a panic if nd_data == 100
		amd_write8(am, SCSICMDREG, MSG_ACCEPTED_CMD);  // 0x12
	} else if ((am->_INSTREG[am->interrupt] & INSTREG_SR) && am->target_selected &&
			   am->ccb.IOnbr2 <= 0 && !am->ccb.command_completed) {
	...
	/*	else if(am->ccb.command_completed && am->bus_phase==PHASE_MSGIN) {
			int nb, t;
			t=(amd_read8(am,CURRENTFIFOREG) & 0x1f);
			for (nb=0;am->nb_data<100 && nb<t; nb++,am->nb_data++) { // nb_data is
	initialized once (at the beginning of IO) am->data[am->nb_data] =
	amd_read8(am,SCSIFIFOREG); } // add a panic if nd_data == 100 amd_write8(am,
	SCSICMDREG, MSG_ACCEPTED_CMD); // 0x12
...
static void recover_phase(Am53c974a *am)
{
	...
	*/
	...
	switch (am->bus_phase) {
		case PHASE_MSGIN:
			strcat(debug, "Message In\n");
			break;
		case PHASE_MSGOUT:
			strcat(debug, "Message Out\n");
			break;
		case PHASE_STATIN:
			strcat(debug, "Status\n");
			break;
		case PHASE_DATAIN:
			strcat(debug, "Data In\n");
			break;
		case PHASE_DATAOUT:
			strcat(debug, "Data Out\n");
			break;
		case PHASE_CMDOUT:
			strcat(debug, "Command\n");
			break;
		default:
			strcat(debug, " Unknown ###\n");
	}
	debug_print(debug);
	am->ccb.command_completed = true;
...
/*static*/ int32 sim_execute_scsi_io(Am53c974a *am, CCB_HEADER *ccbh)
{
	...
	/* ###
	   ...
	   ccb->cam_ch.cam_target_lun,cdb[0]);
	*/
	...
	***/
	...
	if ((ccb->cam_ch.cam_flags & CAM_DIR_NONE) == CAM_DIR_NONE) {
		...
		if (ccb->cam_ch.cam_flags & CAM_SCATTER_VALID) {
			...
			for (i = 0; i < ccb->cam_sglist_cnt && am->ccb.nb_entries < MAX_SCATTER;
				 i++) {

				get_memory_map(iov[i].iov_base,
							   iov[i].iov_len,
							   chunk,
							   MAX_SCATTER - am->ccb.nb_entries);
			...
			get_memory_map(
				ccb->cam_data_ptr, ccb->cam_dxfer_len, am->ccb.entries, MAX_SCATTER);
			while (am->ccb.nb_entries < MAX_SCATTER && chunk->size > 0) {
				nb_left += chunk->size;
				am->ccb.nb_entries++;
				chunk++;
		...
		/* ------ debug only ------ */
		if (am->ccb.nb_entries >= MAX_SCATTER)
			panic("AMD -- MAX_SCATTER limit reached");
		/* ------------------------ */
	...
	***/
	...
	select_target(am, ccb);
	{
		...
		int retry = 10;
		while (retry > 0 &&
			   acquire_sem_etc(am->step_lock, 1, B_RELATIVE_TIMEOUT, 2000000) != B_OK) {
	...
	***/
	...
	//	if ((ccb->cam_dxfer_len > 0) && ((ccb->cam_ch.cam_flags & CAM_DIR_NONE)
	//!=CAM_DIR_NONE))
	if (am->ccb.nb_entries > 0) {
		...
		for (i = 0, chunk = am->ccb.entries; i < MAX_SCATTER && chunk->size > 0;
			 i++, chunk++) {
			u_int32_t resid;

			sprintf(am->when, "%d", i);
			if (am->ccb.IOnbr2 != am->ccb.nb_entries - i)
				debug_print("AMD -- incoherent ionbr2\n");
	...
	if (ccb->cam_dxfer_len > 0) {
		if (nb_left > 0) {
			...
			/* Should consider this as an error ? Probably no ! but BeOS freeze if
			 * not...*/
			am->ccb.command_error = true;
		}
			...
			case 0x02: {
				if (ccb->cam_sense_len == 0) {
					...
					if (err != B_OK) {
						...
						err = get_memory_map(ccb->cam_sense_ptr,
											 ccb->cam_sense_len,
											 am->ccb.entries,
											 MAX_SCATTER);
						if (err) {
							...
							(*pci->write_io_32)(am->iobase + DMASPA,
												(unsigned int)am->ccb.entries[0].address);
							...
							amd_write8(am,
									   DMACMD,
									   READ_DIRECTION | DMACMD_INTE_D | DMACMD_START);

							strcpy(am->when, "rq started");
							err = acquire_sem_etc(
								am->step_lock, 1, B_RELATIVE_TIMEOUT, 2000000);
							if (err != B_OK) {
							...
							resid += (unsigned)amd_read8(am, STCHREG) << 16;
							if (resid) {
								debug_print("# Warning : request sense resid = %d",
											resid);
							}
					...
					err =
						acquire_sem_etc(am->ccb.ccb_lock, 1, B_RELATIVE_TIMEOUT, 200000);
					if (err != B_OK) {
	...
	t2 = system_time();
	sprintf(debug,
			"AMD[perf] duration %ld, size %ld, rate %d\n",
			(long)(t2 - t1),
			ccb->cam_dxfer_len,
			(int)(((float)ccb->cam_dxfer_len) / (t2 - t1) * 1000));
	debug_print(debug);
#endif
...
static status_t init_amd_scsi(Am53c974a *am)
{
	...
	if (install_io_interrupt_handler(am->irq, scsi_int_dispatch, am, 0) == B_ERROR) {
...
static int amdstate(int argc, char **argv)
{
	...
	kprintf("amd last cmd : completed? %d / error? %d / entered %d\n",
			amd->ccb.command_completed,
			amd->ccb.command_error,
			amd->enter);
...
static sim_module_info sim_amd_module = {{"busses/scsi/amd", 0, &std_ops}};

_EXPORT module_info *modules[] = {(module_info *)&sim_amd_module, NULL};
...
static void display_ccb_scsiio(CCB_SCSIIO *ccb, int longformat)
{
	...
	switch (cdb[0]) {
		...
		case 0x1a:
			...
			break;
		case 0x1B:
			op_name = "AllowPreventRemoval ";
			break;
		case 0x1E:
			op_name = "StartStopUnit ";
			break;
		case 0x25:
			op_name = "ReadCapacity ";
			break;
		case 0x28:
			op_name = "Read(10) ";
			break;
		case 0x35:
			...
			break;
		case 0x42:
			op_name = "(CD)ReadSubChannel ";
			break;
		case 0x43:
			op_name = "(CD)ReadToc ";
			break;
		case 0x55:
			op_name = "ModeSelect(10) ";
			break;
		default:
			sprintf(debug,
					"* T%dL%d : %d bytes [",
	...
	*/
	if ((ccb->cam_ch.cam_flags & CAM_DIR_NONE) == CAM_DIR_NONE) {
		sprintf(debug + strlen(debug), "] Data dir NONE\n");
	} else if (ccb->cam_ch.cam_flags & CAM_DIR_IN) {
		sprintf(debug + strlen(debug), "] Data IN : %ld\n", ccb->cam_dxfer_len);
	} else if (ccb->cam_ch.cam_flags & CAM_DIR_OUT) {
		sprintf(debug + strlen(debug), "] Data OUT: %ld\n", ccb->cam_dxfer_len);
	}
	...
	switch (ccb->cam_ch.cam_flags) {
		...
		case 0x8090:
			break;
		default:
			debug_print("# WARNING : cam flag value : %x",
						(unsigned)ccb->cam_ch.cam_flags);
	}
}
	...
	for (i = 0; i < am->interrupt; i++) {
		...
		*/
		...
		if (!(am->_STATREG[i] & COUNT_2_ZERO))
			strcat(debug, "#!C2Z-");
		if (am->_DMASTATUS[i] & DMASTATUS_BCMPLT)
			strcat(debug, "*** : BLAST complete\n");
		if (am->_DMASTATUS[i] & DMASTATUS_DONE)
			strcat(debug, " DMA term");
		if (am->_DMASTATUS[i] & DMASTATUS_ABORT)
			strcat(debug, " DMA transfer aborted\n");
		if (am->_DMASTATUS[i] & DMASTATUS_ERROR)
			strcat(debug, " DMA transfer error\n");
		if (am->_DMASTATUS[i] & DMASTATUS_PWDN)
			strcat(debug, " power down indicator\n");
