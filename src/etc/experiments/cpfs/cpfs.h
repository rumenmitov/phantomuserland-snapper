...
#define cpfs_assert(__check)                                                         \
	if (!(__check))                                                                  \
		cpfs_panic("Assert failed in " __FILE__ " @ %d func %s\n\t" #__check "\n\n", \
				   __LINE__,                                                         \
				   __func__);
...
errno_t cpfs_fs_stat(struct cpfs_fs *fs,
...
errno_t cpfs_file_open(
	struct cpfs_fs *fs, int *file_id, const char *name, int flags, void *user_id_data);
...
errno_t cpfs_disk_trim(
	int disk_id, cpfs_blkno_t block);  // Tell SSD we don't need this disk block anymore
...
errno_t cpfs_os_run_idle_thread(void *(*func_p)(void *arg),
								void *arg);  // Request OS to start thread
...
errno_t cpfs_os_access_rights_check(struct cpfs_fs *fs,
									cpfs_right_t t,
									void *user_id_data,
									const char *fname);
