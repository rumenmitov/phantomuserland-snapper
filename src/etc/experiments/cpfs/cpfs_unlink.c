...
errno_t cpfs_file_unlink(struct cpfs_fs *fs, const char *full_name, void *user_id_data)
{
	const char *last;
	...
	int isdir = 0;
	rc =
		cpfs_is_dir(fs, possible_dir_ino, &isdir);  // Check if inode contains a directory
