...
errno_t cpfs_fsck(cpfs_fs_t *fs, int fix)
{
	errno_t rc;
...
static errno_t fsck_sb(cpfs_fs_t *fs, int fix)
{
	...
	if (sb->itable_end >= sb->itable_pos + ino_table_blkno) {
		fslog(fs,
			  err,
			  "SB sb->itable_end >= sb->itable_pos+ino_table_blkno, %lld",
			  sb->itable_end);
	}
	...
	if( sb->free_count != disk_size - sb->first_unallocated )
		cpfs_panic("sb->free_count (%lld) != disk_size (%lld) - sb->first_unallocated
	(%lld)", (long long)sb->free_count, (long long)disk_size, (long
	long)sb->first_unallocated);
	...
	*/
	// cpfs_touch_blk( fs, sb_blk ); // marks block as dirty, will be saved to disk on
	// unlock
...
errno_t fsck_scan_dir(cpfs_fs_t *fs, cpfs_ino_t dir, size_t depth)
{
	...
	for (cpfs_blkno_t disk_block = itable_pos; disk_block < itable_end; disk_block++) {
		for (int pos = 0; pos < CPFS_INO_PER_BLK; pos++) {
			...
			/*
						{
						//if(inode_p->nlinks){
							fprintf(fsck_scan_ino_log_file,"blk=%lld, pos=%d,
			   data[isdir=%d, fileSize=%llu, nlink=%u, first block=%llu] blocks0[", (long
			   long)disk_block, pos, is_dir, (long long unsigned int)inode.fsize,
			   inode.nlinks,  (long long unsigned int)inode.blocks0[0]); for(int
			   idx=0;idx<CPFS_INO_DIR_BLOCKS, inode.blocks0[idx]!=0;idx++){
								fprintf(fsck_scan_ino_log_file,"%s%lld",  idx==0 ? "" :",
			   ", (long unsigned int) inode.blocks0[idx]);
							}
							fprintf(fsck_scan_ino_log_file,"] indir[");
							for(int idx=0;idx<CPFS_MAX_INDIR, inode.indir[idx]!=0;idx++){
								fprintf(fsck_scan_ino_log_file,"%s%lld", idx==0 ? "" :",
			   ", (long unsigned int) inode.indir[idx]);
							}
...
errno_t fsck_update_block_map(cpfs_fs_t *fs, cpfs_blkno_t blk, fsck_blkstate_t state)
{
	cpfs_assert(
		blk <
		fs->disk_size);  // TODO or superblock disk size? Or shall we check sb before?

	if (fs->fsck_blk_state[blk] != bs_unknown) {
		if ((state == bs_freelist) || (state == bs_freemap)) {
			fslog(fs,
				  err,
				  "block %lld state was %d, attempt to set free\n",
				  blk,
				  fs->fsck_blk_state[blk]);

			fs->fsck_blk_state[blk] = state;
			fs->fsck_rebuild_free = 1;
			return EBUSY;
		} else if ((fs->fsck_blk_state[blk] == bs_freelist) ||
				   (fs->fsck_blk_state[blk] == bs_freemap)) {
			...
			fslog(fs,
				  err,
				  "block %lld state %d, attempt to set %d\n",
				  blk,
				  fs->fsck_blk_state[blk],
				  state);
			// TODO and what?
...
void cpfs_fsck_log(FILE *file, int ino_in_blk, int phys_blk, struct cpfs_inode *inode)
{
	...
	fprintf(file,
			"blk=%d, pos=%d, data[isdir=%d, fileSize=%llu, nlink=%u, first block=%llu] "
			"blocks0[",
			phys_blk,
			ino_in_blk,
			is_dir,
			(long long unsigned int)inode->fsize,
			inode->nlinks,
			(long long unsigned int)inode->blocks0[0]);

	for (int idx = 0; idx < CPFS_INO_DIR_BLOCKS; idx++) {
		fprintf(file,
				"%s%llu",
				idx == 0 ? "" : ", ",
				(long long unsigned int)inode->blocks0[idx]);
	}
	...
	for (int idx = 0; idx < CPFS_MAX_INDIR; idx++) {
		fprintf(file,
				"%s%llu",
				idx == 0 ? "" : ", ",
				(long long unsigned int)inode->indir[idx]);
	}
