...
struct cpfs_sb
{
	...
	cpfs_blkno_t itable_end;  // First unused block of inode table, this one and rest is
							  // not initialized. Used for fast mkfs.

	cpfs_blkno_t disk_size;

	cpfs_blkno_t first_unallocated;  // Number of block at end of FS we didn't use at all.
									 // From this point up to the end all blocks are free.
									 // 0 if not used. Used for fast mkfs.
...
{
	cpfs_fpos_t fsize;
	uint32_t nlinks;  // allways 0 or 1 in this verstion, made for future extensions, if 0
					  // - inode record is free.
	uint32_t ftype;   // nonzero = dir for now

	cpfs_time_t ctime;  // created
	cpfs_time_t atime;  // accessed
	cpfs_time_t mtime;  // modified
	cpfs_time_t vtime;  // version of file forked (not used, will mark time when this
						// backup version of file is forked from main version)

	cpfs_blkno_t acl;  // disk block with access control list, unused now
	cpfs_blkno_t log;  // disk block of audit log list, not used now, will contain log of
					   // all operations to support transactions

	cpfs_blkno_t blocks0[CPFS_INO_DIR_BLOCKS];
	cpfs_blkno_t
		indir[CPFS_MAX_INDIR];  // indirect blocks list with growing level of indirection

	// -------------------------------------------------------------------------------------
	// NB - following is just in-memory state, ignore it on disk, will be inited on any
	// load
#if CPFS_INODE_MUTEX
...
typedef enum
{
	...
	bs_inode,
	bs_superblk
} fsck_blkstate_t;  // allocation state
...
struct cpfs_fs
{
	...
	cpfs_mutex inode_mutex;     // Taken when work with any inode, TODO make inode buffers
								// for concurrent inode io, use mutex to lock search
	cpfs_mutex dir_mutex;  // Directory update/access mutex TODO hack, very inefficient -
						   // need per-inode locks for this!
...
errno_t cpfs_sb_unlock_write(
	cpfs_fs_t *fs);  // if returns error, sb is not written and IS NOT UNLOCKED
...
errno_t cpfs_find_block_4_file(
	cpfs_fs_t *fs,
	cpfs_ino_t ino,
	cpfs_blkno_t logical,
	cpfs_blkno_t *phys);  // maps logical blocks to physical, block must be allocated
...
errno_t cpfs_alloc_block_4_file(cpfs_fs_t *fs,
								cpfs_ino_t ino,
								cpfs_blkno_t logical,
								cpfs_blkno_t *phys);
// returns 0 in any case, either if block found or allocated
errno_t cpfs_find_or_alloc_block_4_file(cpfs_fs_t *fs,
										cpfs_ino_t ino,
										cpfs_blkno_t logical,
										cpfs_blkno_t *phys);

// inode machinery indirect block addressing workers
errno_t calc_indirect_positions(cpfs_fs_t *fs,
								cpfs_blkno_t indexes[CPFS_MAX_INDIR],
								int *start_index,
								cpfs_blkno_t logical);

// void cpfs_fsck_log(FILE *file,  cpfs_ino_t ino_in_blk, cpfs_blkno_t phys_blk,
// cpfs_inode inode);

struct cpfs_inode *cpfs_lock_ino(
	cpfs_fs_t *fs,
	cpfs_ino_t ino);  // makes sure that inode is in memory and no one modifies it
void cpfs_touch_ino(
	cpfs_fs_t *fs,
	cpfs_ino_t ino);  // marks inode as dirty, will be saved to disk on unlock
void cpfs_unlock_ino(
	cpfs_fs_t *fs,
	cpfs_ino_t ino);  // flushes inode to disk before unlocking it, if touched

#if CPFS_INODE_MUTEX
// lock/unlock inode state/file data exclusive access mutex. used in directory update code
// mostly
...
errno_t cpfs_alloc_dirent(cpfs_fs_t *fs,
						  cpfs_ino_t dir_ino,
						  const char *fname,
						  cpfs_ino_t file_ino);  // allocate a new dir entry in a dir
errno_t cpfs_free_dirent(cpfs_fs_t *fs,
						 cpfs_ino_t dir_ino,
						 const char *fname);  // free dir entry (write 0 to inode field)
errno_t cpfs_namei(cpfs_fs_t *fs,
				   cpfs_ino_t dir_ino,
				   const char *fname,
				   cpfs_ino_t *file_ino);  // find name

// was errno_t                 cpfs_namei( cpfs_fs_t *fs, cpfs_ino_t dir_ino, const char
// *fname, cpfs_ino_t *file_ino, int remove ); // find name

// General dir scan func
errno_t cpfs_scan_dir(cpfs_fs_t *fs, cpfs_ino_t dir_ino, dir_scan_func_t f, void *farg);


errno_t cpfs_is_dir(cpfs_fs_t *fs,
					cpfs_ino_t dir_ino,
					int *yesno);  // Check if inode contains a directory
...
// errno_t                 cpfs_dir_scan( cpfs_fs_t *fs, cpfs_ino_t dir_ino, const char
// *fname, cpfs_ino_t *file_ino, int flags );
...
// TODO dirent name cache - in memory hash? Not really as usual client software opens file
// once in its life?

// Parse path name and descend into the last directory, returning last part of path and
// inode of last directory
errno_t cpfs_descend_dir(cpfs_fs_t *fs,
						 const char *path,
						 const char **last,
						 cpfs_ino_t *last_dir_ino);


void *cpfs_lock_blk(cpfs_fs_t *fs,
					cpfs_blkno_t blk);  // makes sure that block is in memory
void cpfs_touch_blk(
	cpfs_fs_t *fs,
	cpfs_blkno_t blk);  // marks block as dirty, will be saved to disk on unlock
void cpfs_unlock_blk(
	cpfs_fs_t *fs,
	cpfs_blkno_t blk);  // flushes block to disk before unlocking it, if touched


// Internal read/wrire impl, used by user calls and internal directory io code

errno_t cpfs_ino_file_read(cpfs_fs_t *fs,
						   cpfs_ino_t ino,
						   cpfs_size_t pos,
						   void *data,
						   cpfs_size_t size,
						   cpfs_blkno_t *phys_blk);
errno_t cpfs_ino_file_write(
	cpfs_fs_t *fs, cpfs_ino_t ino, cpfs_size_t pos, const void *data, cpfs_size_t size);

errno_t cpfs_inode_truncate(
	cpfs_fs_t *fs, cpfs_ino_t ino);  // free all data blocks for inode, set size to 0
