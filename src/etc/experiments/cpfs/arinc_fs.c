...
static void fill_arinc_retcodes(errno_t rc,
								RETURN_CODE_TYPE *RETURN_CODE,
								FILE_ERRNO_TYPE *ERRNO);
static errno_t arinc_find_filesystem(const char *full_file_name,
									 struct cpfs_fs **fs,
									 size_t *name_pos);
static void *arinc_get_user_data(void)
{
	return 0;
}
...
/*
void SEEK_FILE( FILE_ID_TYPE FILE_ID, FILE_SIZE_TYPE OFFSET, FILE_SEEK_TYPE WHENCE,
FILE_SIZE_TYPE *POSITION, RETURN_CODE_TYPE *RETURN_CODE, FILE_ERRNO_TYPE *ERRNO )
{
	errno_t rc = 0;
...
/*
void RESIZE_FILE( FILE_ID_TYPE FILE_ID, FILE_SIZE_TYPE NEW_SIZE, RETURN_CODE_TYPE
*RETURN_CODE, FILE_ERRNO_TYPE *ERRNO )
{
	errno_t rc = 0;
...
void SYNC_FILE( FILE_ID_TYPE FILE_ID, RETURN_CODE_TYPE *RETURN_CODE, FILE_ERRNO_TYPE
*ERRNO )
{
	errno_t rc = 0;
...
void OPEN_DIRECTORY( FILE_NAME_TYPE DIRECTORY_NAME, DIRECTORY_ID_TYPE *DIRECTORY_ID,
RETURN_CODE_TYPE *RETURN_CODE, FILE_ERRNO_TYPE *ERRNO )
{
	errno_t rc = 0;
...
void CLOSE_DIRECTORY( DIRECTORY_ID_TYPE DIRECTORY_ID, RETURN_CODE_TYPE *RETURN_CODE,
FILE_ERRNO_TYPE *ERRNO )
{
	errno_t rc = 0;
...
//void READ_DIRECTORY( DIRECTORY_ID_TYPE DIRECTORY_ID, DIRECTORY_ENTRY_TYPE *ENTRY_NAME,
ENTRY_KIND_TYPE *ENTRY_KIND, RETURN_CODE_TYPE *RETURN_CODE, FILE_ERRNO_TYPE *ERRNO )

void REWIND_DIRECTORY( DIRECTORY_ID_TYPE DIRECTORY_ID, RETURN_CODE_TYPE *RETURN_CODE,
FILE_ERRNO_TYPE *ERRNO )
{
	errno_t rc = 0;
					  ...
					  FILE_ERRNO_TYPE *ERRNO)
{
	...
	if (fs1 != fs2) {
		cpfs_log_error("Attempt to rename dir to differrent volume ('%s' to '%s')",
					   OLD_DIRECTORY_NAME,
					   NEW_DIRECTORY_NAME);
		rc = EINVAL;
		goto fail;
...
arinc_volume_def_t volumes[] = {{.name = "vol0", .disk_id = 0, .disk_size = 10000},
								{.name = "vol1", .disk_id = 1, .disk_size = 12000}};

static int nvol = sizeof(volumes) / sizeof(arinc_volume_def_t);

void arinc_init_filesystems(void)
{
	int i;
	errno_t rc;
	...
	for (i = 0; i < nvol; i++) {
		...
		if (rc) {
			cpfs_log_error(
				"Can't init FS %d (vol '%s'), rc = %d", i, volumes[i].name, rc);
			continue;
		}
		...
		if (rc) {
			cpfs_log_error(
				"Can't mount FS %d (vol '%s'), rc = %d", i, volumes[i].name, rc);
			continue;
		}
...
errno_t arinc_find_filesystem(const char *full_file_name,
							  struct cpfs_fs **fs,
							  size_t *name_pos)
{
	int i;
