...
errno_t cpfs_init(cpfs_fs_t *fs)
{
	errno_t rc;
...
// Mount
errno_t cpfs_mount(cpfs_fs_t *fs)
{
	errno_t rc;
	...
	fs->sb.dirty = 0xFF;  // Next write will update it on disk, we don't need to mark disk
						  // dirty if we never had a reason to write sb
...
errno_t cpfs_umount(cpfs_fs_t *fs)
{
	errno_t rc;
