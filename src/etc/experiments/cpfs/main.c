...
static void test_all(cpfs_fs_t *fsp)
{
	...
	// test_file_create(fsp); 	// create, open and destroy multiple files, try open
	// deleted files

	test_file_data(
		fsp);  // Create, write, close, reopen, read and compare data, in a mixed way

	// test_mutithreaded(fsp);     // Do mix of prev tests in 10 threads, starting tests
	// in random order
...
static void test_mt(cpfs_fs_t *fs)
{
	...
	// test_file_create(fs); 	// create, open and destroy multiple files, try open
	// deleted files

	test_file_data(
		fs);  // Create, write, close, reopen, read and compare data, in a mixed way

	// test_mutithreaded(fs);     // Do mix of prev tests in 10 threads, starting tests in
	// random order
