...
static errno_t cpfs_namei_impl(cpfs_fs_t *fs,
							   cpfs_ino_t dir_ino,
							   const char *fname,
							   cpfs_ino_t *file_ino,
							   int remove)
{
	...
	while (nblk-- > 0) {
		...
		rc = cpfs_ino_file_read(
			fs, dir_ino, blkpos * CPFS_BLKSIZE, data, CPFS_BLKSIZE, &phys_blk);
			...
			if (de->inode && (0 == strcmp(fname, de->name))) {
				*file_ino = de->inode;
				...
				if (remove) {
					...
					rc = cpfs_ino_file_write(
						fs, dir_ino, blkpos * CPFS_BLKSIZE, data, CPFS_BLKSIZE);
					if (rc) {
						cpfs_mutex_unlock(
							fs->dir_mutex);  // TODO need to use inode mutex here!
						cpfs_log_error("Can't write dir ino %d @ blk %d",
									   dir_ino,
									   blkpos);
						return rc;
					}
				...
				if ((*file_ino) >= fs->sb.ninode) {
					cpfs_log_error(
						"cpfs_namei_impl: dir ino %lld fname %s gives ino %lld",
						dir_ino,
						fname,
						file_ino);
					continue;  // Don't match dir entries if inode value is insane -
							   // right?
				}
...
errno_t cpfs_free_dirent(cpfs_fs_t *fs, cpfs_ino_t dir_ino, const char *fname)
{
						  ...
						  cpfs_ino_t file_ino)
{
	...
	if (file_ino >= fs->sb.ninode)
		cpfs_log_error("cpfs_alloc_dirent: dir ino %lld fname %s gives ino %lld",
					   dir_ino,
					   fname,
					   file_ino);
	cpfs_assert(file_ino < fs->sb.ninode);
	...
	// No free entry found, attempt to append one - we append one block, following calls
	// will find rest of blk to be free
...
errno_t cpfs_is_dir(cpfs_fs_t *fs, cpfs_ino_t dir_ino, int *yesno)
{
	...
	* /
			...
			fprintf(fsck_scan_dir_log_file,"blk=%d, pos=%d, data[isdir=%d, fileSize=%lld,
	nlink=%u, first block=%lld] blocks0[", phys_blk, ino_in_blk, *yesno, (long unsigned
	int)rdi->fsize, rdi->nlinks,  (long unsigned int)rdi->blocks0[0]); for(int
	idx=0;idx<CPFS_INO_DIR_BLOCKS, rdi->blocks0[idx]!=0;idx++){
				fprintf(fsck_scan_dir_log_file,"%s%lld",  idx==0 ? "" :", ", (long
	unsigned int) rdi->blocks0[idx]);
			}
			fprintf(fsck_scan_dir_log_file,"] indir[");
			for(int idx=0;idx<CPFS_MAX_INDIR, rdi->indir[idx]!=0;idx++){
				fprintf(fsck_scan_dir_log_file,"%s%lld", idx==0 ? "" :", ", (long unsigned
	int) rdi->indir[idx]);
			}
...
errno_t cpfs_scan_dir(cpfs_fs_t *fs, cpfs_ino_t dir_ino, dir_scan_func_t f, void *farg)
{
	...
	while (nblk-- > 0) {
		...
		for (i = 0; i < CPFS_DIR_PER_BLK; i++) {
			...
			/*            if( TRACE ) {
							fprintf(fsck_scan_dir_log_file,"blk=%lld, pos=%d,
			   data[ino=%lld, name=%s]  \n", (long long)phys_blk, i, (long long)de->inode,
			   de->name ); fflush(fsck_scan_dir_log_file);
						}*/
			dir_scan_ret_t fret = f(fs, de, farg);

			if (fret == dir_scan_success)
				return 0;
			if (fret == dir_scan_error)
				return EMFILE;
		}
