 ...
 * start_index  - start position in indexes which is used, end position is allways the end
 *of array
 *
								...
								cpfs_blkno_t ilogical)
{
	...
	if (logical <
		1LL * CPFS_INDIRECT_PER_BLK * CPFS_INDIRECT_PER_BLK * CPFS_INDIRECT_PER_BLK) {
	...
	if (logical < 1LL * CPFS_INDIRECT_PER_BLK * CPFS_INDIRECT_PER_BLK *
					  CPFS_INDIRECT_PER_BLK * CPFS_INDIRECT_PER_BLK) {
		*start_index = CPFS_MAX_INDIR - 4;
		goto fill_array;
	}

	logical -= CPFS_INDIRECT_PER_BLK * CPFS_INDIRECT_PER_BLK * CPFS_INDIRECT_PER_BLK *
			   CPFS_INDIRECT_PER_BLK;
...
fill_array:
	...
	if ((ai + 1) != *start_index) {
		printf("calc_indir( %lld ) -> ai=%d, start_index=%d, indexes = %lld, %lld, %lld, "
			   "%lld\n",
			   (long long)ilogical,
			   ai,
			   *start_index,
