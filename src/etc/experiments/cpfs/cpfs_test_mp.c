...
static const char test_data[] =
	"MP test: big brown something jumps over a lazy programmer and runs regression tests "
	"concurrently, though quite in vain";


// Unique id for file names - in these tests we need unique file names to make sure
// threads do not clash
...
void test_mp_files(cpfs_fs_t *fs)
{
	...
	if (rc)
		cpfs_panic("create 1 rc=%d", rc);

	rc = cpfs_file_open(fs, &fd2, fn2, O_CREAT, 0);
	if (rc)
		cpfs_panic("create 2 rc=%d", rc);
	...
	if (rc)
		cpfs_panic("can't write data2, rc=%d", rc);

	rc = cpfs_file_read(fd2, 100, test_buf, sizeof(test_data));
	if (rc)
		cpfs_panic("can't read data2, rc=%d", rc);
...
void test_mp_disk_alloc(cpfs_fs_t *fsp)
{
	...
	/*
	   ...
	   long)initial_free, (long long)fsp->sb.free_count );
		}
	*/
...
void test_mp_inode_alloc(cpfs_fs_t *fs)
{
	...
	/* can't do double free test in multithread env - we free for second thread
	   accidentally rc = cpfs_free_inode( fs, i2 ); cpfs_assert( rc != 0 );
