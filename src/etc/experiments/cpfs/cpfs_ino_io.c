...
struct cpfs_inode *cpfs_lock_ino(cpfs_fs_t *fs,
								 cpfs_ino_t ino)  // makes sure that block is in memory
{
	errno_t rc;
	/*
	...
	*/
	...
	if (used)
		cpfs_panic("out of inode buffers");

	cpfs_blkno_t blk;
	rc = cpfs_block_4_inode(fs, ino, &blk);
	if (rc)
		cpfs_panic("cpfs_block_4_inode");
	...
	if (blk > fs->sb.itable_end)
		cpfs_panic("attempt to read inode %lld after fs->sb.itable_end (%lld)",
				   (long long)blk,
				   (long long)fs->sb.itable_end);
	...
	if (rc)
		cpfs_panic("can't write sb");  // TODO just log and refuse call?
...
void cpfs_touch_ino(
	cpfs_fs_t *fs,
	cpfs_ino_t ino)  // marks block as dirty, will be saved to disk on unlock
{
	if (curr_ino != ino)
		cpfs_panic("wrong ino in touch");
...
void cpfs_unlock_ino(
	cpfs_fs_t *fs,
	cpfs_ino_t ino)  // flushes block to disk before unlocking it, if touched
{
	/*
	...
	*/
	if (!used)
		cpfs_panic("double cpfs_unlock_blk");
	if (curr_ino != ino)
		cpfs_panic("wrong ino in unlock");
