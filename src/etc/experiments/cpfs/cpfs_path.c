						 ...
						 cpfs_ino_t *last_dir_ino)
{
	...
	while (1) {
		if (!*path)
			return EINVAL;  // ? can't happen? final stray '/'?
		...
		cur_name_start = next_slash +
						 1;  // Skip to the next part of path for next iteration
...
errno_t cpfs_mkdir(cpfs_fs_t *fs, const char *path, void *user_id_data)
{
	const char *last;
	cpfs_ino_t last_dir_ino;
	...
	rc = cpfs_alloc_dirent(
		fs, last_dir_ino, last, new_dir_ino);  // allocate a new dir entry in a dir
