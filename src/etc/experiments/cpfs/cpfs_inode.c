...
static errno_t find_or_create_indirect(cpfs_fs_t *fs,
									   cpfs_blkno_t *base,
									   cpfs_blkno_t *indexes,
									   int indirection,
									   cpfs_blkno_t *phys,
									   int create,
									   int *created);
...
errno_t cpfs_find_block_4_file(cpfs_fs_t *fs,
							   cpfs_ino_t ino,
							   ...
							   cpfs_blkno_t *phys)
{
	...
	*/
	...
	if (logical * CPFS_BLKSIZE > inode.fsize) {
		cpfs_log_error(
			"cpfs_find_block_4_file: logical*CPFS_BLKSIZE (%d) >= inode.fsize (%d)",
			logical * CPFS_BLKSIZE,
			inode.fsize);
	...
	if (logical < CPFS_INO_DIR_BLOCKS) {
		*phys = inode.blocks0[logical];
		/*
				if( TRACE ) trace(0, "%*s < cpfs_find_block_4_file.  ino=%lld,
		...
		*/
		...
		// printf("cpfs_find_block_4_file calc_indir( %lld ) -> indirection=%d,
		// start_index=%d, indexes = %lld, %lld, %lld, %lld\n", logical, indirection,
		// start_index, indexes[0], indexes[1], indexes[2], indexes[3] );

		int created = 0;

		rc = find_or_create_indirect(fs,
									 inode.indir + indirection - 1,
									 indexes + start_index,
									 indirection,
									 phys,
									 0,
								...
								cpfs_blkno_t *phys)
{
	...
	if (logical < CPFS_INO_DIR_BLOCKS) {
		...
		*phys = ip->blocks0[logical] = new;
		...
		// printf("cpfs_alloc_block_4_file calc_indir( %lld ) -> start_index=%d, indexes =
		// %lld, %lld, %lld, %lld\n", logical, start_index, indexes[0], indexes[1],
		// indexes[2], indexes[3] );
		int indirection = CPFS_MAX_INDIR - start_index;
		// printf("cpfs_find_block_4_file calc_indir( %lld ) -> indirection=%d,
		// start_index=%d, indexes = %lld, %lld, %lld, %lld\n", logical, indirection,
...
fail:
	...
	cpfs_unlock_ino(fs, ino);
	// TODO write indirect blocks support!
	cpfs_log_error("cpfs_alloc_block_4_file: no indirect support, logical = %lld",
				   logical);
	return rc;
}
...
//
// base contains 0 or block num of indirect block. If it is 0 and create is nonzero, block
// will be created. indexes contains logical block numbers for corresponding indirection
// indirection is level of indirection. If 1, we can't go any further, *base contains page
// with phys block numbers - TODO right? create - instructs us to allocate block if
// unallocated created set to nonzero if we did create *base
//
									   ...
									   int *created)
{
	...
	rc = find_or_create_indirect(
		fs, &(ib->child[*indexes]), indexes + 1, indirection - 1, phys, create, created);
...
errno_t cpfs_block_4_inode(cpfs_fs_t *fs, cpfs_ino_t ino, cpfs_blkno_t *oblk)
{
	if (ino >= fs->sb.ninode) {
		cpfs_log_error("cpfs_block_4_inode: ino (%lld) >= fs->sb.ninode (%lld)",
					   ino,
					   fs->sb.ninode);
		return E2BIG;
	}
...
errno_t cpfs_free_inode(cpfs_fs_t *fs, cpfs_ino_t ino)
{
	errno_t rc;
...
static void do_fic_refill(cpfs_fs_t *fs)
{
	...
	// Each disk block has CPFS_INO_PER_BLK inodes. As long as we have place for
	// CPFS_INO_PER_BLK inodes in free_inodes_cache and there are unused blocks in inode
	// space, refill free inode numbers list.
	...
	if ((fs->last_ino_blk < ilast) && (fs->fic_used < (FIC_SZ + CPFS_INO_PER_BLK))) {
		...
		for (i = 0; i < CPFS_INO_PER_BLK; i++) {
			cpfs_ino_t ino = (fs->last_ino_blk - fs->sb.itable_pos) * CPFS_INO_PER_BLK +
							 i;

			cpfs_assert(ino < fs->sb.ninode);

			// printf("refill inodes add %lld\n", ino);
			// fs->free_inodes_cache[fs->fic_used++] =
			// (fs->last_ino_blk-fs->sb.itable_pos) + i;
			fs->free_inodes_cache[fs->fic_used++] = ino;
		}
	...
	cpfs_fs_t *fs, cpfs_ino_t ino)  // free all data blocks for inode, set size to 0
{
	...
	if (inode->nlinks <= 0) {
		cpfs_log_error(
			"cpfs_inode_truncate: attempt to truncate inode %d with zero links", ino);
		return ENOENT;
	}
	...
	for (blk = 0; (blk < nblk) && (blk < CPFS_INO_DIR_BLOCKS); blk++) {
		...
		if (phys_blk == 0) {
			cpfs_log_error(
				"cpfs_inode_truncate: attempt to free blk 0 (%lld block of file)",
				phys_blk,
				blk);
			continue;
		}
	...
	for (ilev = 0; ilev < CPFS_MAX_INDIR - 1; ilev++) {
		...
		if (rc)
			cpfs_log_error(
				"cpfs_inode_truncate: fail to free indirect blocks lev %d for ino %lld",
				ilev + 1,
				ino);
	}
...
errno_t cpfs_inode_update_fsize(cpfs_fs_t *fs, cpfs_ino_t ino, cpfs_size_t size)
{
	...
	if (inode_p == 0) {
		cpfs_unlock_ino(fs,
						ino);  // TODO check if we need to unlock anything if lock failed
		return EIO;
	}
...
errno_t cpfs_file_stat(struct cpfs_fs *fs,
					   const char *name,
					   void *user_id_data,
