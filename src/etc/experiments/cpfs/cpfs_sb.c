...
errno_t cpfs_mkfs(cpfs_fs_t *fs, cpfs_blkno_t disk_size)
{
	...
	if (sb->free_count != disk_size - sb->first_unallocated)
		cpfs_panic(
			"sb->free_count (%lld) != disk_size (%lld) - sb->first_unallocated (%lld)",
			(long long)sb->free_count,
			(long long)disk_size,
			(long long)sb->first_unallocated);
...
errno_t cpfs_mount_sb(cpfs_fs_t *fs)
{
	...
	if (sb->dirty) {
		cpfs_log_error(
			"can't mound dirty disk, need FSCK");  // TODO run fsck from here? different
												   // return code? EFTYPE
		cpfs_unlock_blk(fs, sb_blk);
		return EINVAL;
	}
