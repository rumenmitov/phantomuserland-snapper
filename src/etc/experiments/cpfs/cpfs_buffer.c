...
{
	fs->nbuf = CPFS_MAX_CONCUR_IO *
			   4;  // suppose one IO needs max 4 buffers - TODO unchecked
...
static errno_t cpfs_buf_alloc(cpfs_fs_t *fs, cpfs_blkno_t blk, cpfs_buf_t **buf)
{
	errno_t rc;
	...
	for (i = 0; i < fs->nbuf; i++) {
		...
		if (rc) {
			...
			cpfs_log_error("cpfs_buf_alloc: read err for blk %lld, errno=%d",
						   (long long)blk,
						   rc);
			return EIO;
		}
...
static errno_t cpfs_buf_find(cpfs_fs_t *fs,
							 cpfs_blkno_t blk,
							 cpfs_buf_t **buf)  // char shared
{
	// int nsearch = 0;
		...
		if (fs->buf[i].used && !(fs->buf[i].lock)) {
			...
			if (fs->buf[i].write) {
				...
				if (rc) {
					...
					// add inode num to each buffer, mark inode as having IO error, return
					// error on next IO? close?
					cpfs_log_error("cache write IO error, blk %lld",
								   (long long)fs->buf[i].blk);
					cpfs_panic("io err in cache write blk %lld",
							   (long long)fs->buf[i].blk);
					continue;
				}
				fs->buf[i].write = 0;
...
void cpfs_clear_all_buf(cpfs_fs_t *fs)
{
	...
	for (i = 0; i < fs->nbuf; i++) {
		if (fs->buf[i].used && fs->buf[i].lock)
			cpfs_panic("locked blk %lld in cpfs_clear_all_buf",
					   (long long)fs->buf[i].blk);
		...
		if (fs->buf[i].used && !(fs->buf[i].lock)) {
			...
			if (fs->buf[i].write) {
				...
				if (rc) {
					...
					cpfs_log_error("cache write IO error, blk %lld",
								   (long long)fs->buf[i].blk);
					// cpfs_panic("io err in cache write blk %lld", (long
					// long)fs->buf[i].blk); // panic does not help here
					continue;
				}
