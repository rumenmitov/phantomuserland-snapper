...
errno_t cpfs_file_open(
	cpfs_fs_t *fs, int *file_id, const char *full_name, int flags, void *user_id_data)
{
	errno_t rc;
	cpfs_ino_t file_ino;
	const char *last;
	...
	rc = cpfs_os_access_rights_check(fs,
	...
	// rc = cpfs_namei( cpfs_ino_t dir_ino, const char *fname, cpfs_ino_t *file_ino ); //
	// find name
	...
	if (rc && (flags & O_CREAT)) {
		...
		rc = cpfs_alloc_dirent(
			fs, last_dir_ino, last, file_ino);  // allocate a new dir entry in a dir
		if (rc)
	...
	if (rc)
		return rc;  // TODO close file, or better do it before all
