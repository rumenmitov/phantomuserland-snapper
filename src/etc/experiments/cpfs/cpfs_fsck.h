...
// FSCK makes table of all referred disk blocks (in data block zone, not counting sb/inode
// table blocks

// struct fsck_used_blk {
//     cpfs_blkno_t blk; // used disk block number
//     cpfs_ino_t ino; // number of referring inode
//     cpfs_blkno_t referring_blk; // block number of indirect block referring this block,
//     or 0 if this is direct block referenced from inode int pos_in_blk; // position of
//     this block reference in indirect blk or in inode
// };
//
//
...
//     int inode_offset; // offset of ino in block = ino % CPFS_INO_PER_BLK. sdir=0 ->
//     inode_offset=0 cpfs_ino_t inode; //num inode = (block-1)*CPFS_INO_PER_BLK +offset
//     int isdir;
//
