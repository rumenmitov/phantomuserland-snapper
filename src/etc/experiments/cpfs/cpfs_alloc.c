...
cpfs_blkno_t cpfs_alloc_disk_block(cpfs_fs_t *fs)
{
	...
	if (fb->h.magic != CPFS_FL_MAGIC) {
		cpfs_log_error(
			"Freelist corrupt (blk %lld), attempt to continue on unallocated space",
			(long long)fs->sb.free_list);
		fs->sb.free_list = 0;  // freeing blocks will recreate free list, though part of
							   // disk is inavailable now
		goto no_freelist;
	}
...
no_freelist:
	...
	if (fs->sb.free_count <= 0)
		cpfs_panic(
			"cpfs_alloc_disk_block disk state inconsistency: fs->sb.free_count <= 0");

	fs->sb.free_count--;
	ret = fs->sb.first_unallocated++;

	// if( fs->sb.free_count <= 0 )
	//     cpfs_panic("cpfs_alloc_disk_block disk state inconsistency: fs->sb.free_count
	//     <= 0");
