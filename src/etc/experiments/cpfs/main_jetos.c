...
static const int diskmem_sz = TEST_MEM_DISK_SZ;  // TODO JetOS tried to make smaller disk
												 // for partition size is no more than 16m
...
static int real_main(void)
{
	...
	if (ret != NO_ERROR) {
		...
		printf("process 1 \"started\" (it won't actually run until operating mode "
			   "becomes NORMAL)\n");
	}
