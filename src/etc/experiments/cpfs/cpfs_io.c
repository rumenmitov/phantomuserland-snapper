						   ...
						   cpfs_blkno_t *phys_blk1)
{
	...
	*phys_blk1 = phys_blk;
	...
	// void                    cpfs_touch_blk(  cpfs_blkno_t blk ); // marks block as
	// dirty, will be saved to disk on unlock
	...
	while (size > CPFS_BLKSIZE) {
		...
		if (off)
			return EFAULT;  // assert? panic? we're fried?

		part = CPFS_BLKSIZE;
		if (part > size)
			return EFAULT;  // can't be? assert? panic?
