...
void mass_blk_alloc(cpfs_fs_t *fsp, struct tda_q *tda_q, int cnt)
{
	while (cnt-- > 0) {
		...
		if (!blk)
			cpfs_panic("can't allocate block");
		if (tda_q->pp >= QSZ)
			cpfs_panic("test out of q");
...
void mass_blk_free(cpfs_fs_t *fsp, struct tda_q *tda_q, int cnt)
{
	while (cnt-- > 0) {
		if (tda_q->gp >= QSZ)
			cpfs_panic("mass_blk_free test out of q");
		cpfs_blkno_t blk = tda_q->q[tda_q->gp++];
		if (!blk)
			cpfs_panic("mass_blk_free blk 0");
...
void test_inode_blkmap(cpfs_fs_t *fsp)  // test file block allocation with inode
{
	...
	if (rc)
		cpfs_panic("can't alloc inode, %d", rc);
	...
	if (rc)
		cpfs_panic("cpfs_alloc_block_4_file rc=%d", rc);

	rc = cpfs_find_block_4_file(fsp, ino, 0, &phys1);
	if (rc)
		cpfs_panic("cpfs_find_block_4_file rc=%d", rc);

	if (phys0 != phys1)
		cpfs_panic("allocated and found blocks differ: %lld and %lld",
				   (long long)phys0,
				   (long long)phys1);
...
static const char test_data[] = "big brown something jumps over a lazy programmer and "
								"runs regression tests, though quite in vain";
...
void test_directory(cpfs_fs_t *fsp)
{
...
void test_file_data(cpfs_fs_t *fsp)  // Create, write, close, reopen, read and compare
									 // data, in a mixed way
{
	int fd1, fd2;  //, fd3;
	errno_t rc;
	...
	if (rc)
		cpfs_panic("can't write data2, %d", rc);

	rc = cpfs_file_read(fd2, 100, test_buf, sizeof(test_data));
	if (rc)
		cpfs_panic("can't read data2, %d", rc);
...
void test_path(cpfs_fs_t *fsp)
{
	...
	if (ret != 0)
		cpfs_panic("cpfs_descend_dir must allways succeed for single file name");

	ret =
		cpfs_descend_dir(fsp, "surely_nonexisting_dir/somefile.qq", &last, &last_dir_ino);
	if (ret != ENOENT)
		cpfs_panic("cpfs_descend_dir found nonexisting dir");
...
{
#if 0  // if 1 will test indirection too (many files in directory, dir grows to size which
	   // needs indirection)
    (void) fsp;
...
static errno_t fill_file(cpfs_fs_t *fsp, int num, unsigned size)
{
	...
	memset((void *)junk,
		   0xAA,
		   sizeof(junk));  // valgrind cries about using junk from stack
...
void test_out_of_space(cpfs_fs_t *fsp)
{
	...
	for (num = 0; num < max; num++) {
		...
		if (ret && (num = (max - 1)))
			break;  // last one is possibly not created
		test_int_eq(ret, 0);
	}
