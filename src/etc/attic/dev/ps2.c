...
static void put_buf(struct ui_event *e)
{
	...
	if ((put_pos == get_pos - 1) || ((get_pos == 0) && (put_pos == MAX_EVENTS - 1)))
		goto ret;
...
static void ps2ms_int_handler(void *arg)
{
	...
	if (ps2ms_state_xpos < 0)
		ps2ms_state_xpos = 0;
	if (ps2ms_state_ypos < 0)
		ps2ms_state_ypos = 0;

	if (ps2ms_state_xpos > video_drv->xsize)
		ps2ms_state_xpos = video_drv->xsize;
	if (ps2ms_state_ypos > video_drv->ysize)
		ps2ms_state_ypos = video_drv->ysize;
...
{
	wait_write_ctrl();
	outb(PS2_CTRL_ADDR, PS2_CTRL_WRITE_CMD);
	wait_write_data();
	outb(PS2_DATA_ADDR, cmd);
}
...
{
	wait_write_ctrl();
	outb(PS2_CTRL_ADDR, PS2_CTRL_WRITE_AUX);
	wait_write_data();
	outb(PS2_DATA_ADDR, aux);
}
