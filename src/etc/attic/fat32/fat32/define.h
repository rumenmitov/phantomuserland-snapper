...
#define SET_32BIT_WORD(buffer, location, value)              \
	{                                                        \
		buffer[location + 0] = (BYTE)((value) & 0xFF);       \
...
#else
#define GET_32BIT_WORD(buffer, location)                                           \
	(((UINT32)buffer[location + 0] << 24) + ((UINT32)buffer[location + 1] << 16) + \
	 ((UINT32)buffer[location + 2] << 8) + (UINT32)buffer[location + 3])
#define GET_16BIT_WORD(buffer, location) \
	(((UINT16)buffer[location + 0] << 8) + (UINT16)buffer[location + 1])
