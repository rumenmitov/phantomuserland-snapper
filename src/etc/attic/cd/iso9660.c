...
#define ATAPI_ALIGN_BUFFER(x)                                  \
	if (x % ATAPI_SECTOR_SIZE) {                               \
		x = x + (ATAPI_SECTOR_SIZE - (x % ATAPI_SECTOR_SIZE)); \
	}
...
FILE *ISO9660_ReadFile(char *device, const char *restrict filepath)
{
	...
	struct ISO9660_PVD_s *PVD =
		(struct ISO9660_PVD_s *)ATAPI_ReadSector(adapter, drive, 16);
	struct ISO9660_DirectoryEntry_s *RDE =
		(struct ISO9660_DirectoryEntry_s *)PVD->RootDirectoryEntry;
	...
	struct ISO9660_DirectoryEntry_s *DIRBUFFER =
		(struct ISO9660_DirectoryEntry_s *)malloc(DIR_DataLengthAligned);
	struct ISO9660_DirectoryEntry_s *DIR = DIRBUFFER;

	for (i = 0; i < (DIR_DataLengthAligned / ATAPI_SECTOR_SIZE); i++) {
		memcpy((Byte *)((IntPtr)DIRBUFFER + (i * ATAPI_SECTOR_SIZE)),
			   ATAPI_ReadSector(adapter, drive, RootDirLBA + i),
			   ATAPI_SECTOR_SIZE);
	}
	...
	while (i < DIR_DataLengthAligned) {
		...
		for (x = 32 + DIR->LengthOfFileIdentifier; x < DIR->Length - 1; x++) {
			if (*(Byte *)((IntPtr)DIR + x) == 'N' &&
				*(Byte *)((IntPtr)DIR + x + 1) == 'M') {
				struct ISO9660_RR_NM_Entry_s *NM =
					(struct ISO9660_RR_NM_Entry_s *)((IntPtr)DIR + x);
				...
				if (!memcmp(
						tokenizer, NM->Name, MAX(strlen(tokenizer), NM->Length - 5))) {
					Int32 DIR_LBA;
					...
					if (!(tokenizer = strtok(NULL, "/"))) {
						...
						DIR_LBA = DIR->LBA[0];
						free(DIRBUFFER);
						DIRBUFFER = (struct ISO9660_DirectoryEntry_s *)malloc(
							DIR_DataLengthAligned);
						for (y = 0; y < (DIR_DataLengthAligned / ATAPI_SECTOR_SIZE);
							 y++) {
							memcpy((Byte *)((IntPtr)DIRBUFFER + (y * ATAPI_SECTOR_SIZE)),
								   ATAPI_ReadSector(adapter, drive, DIR_LBA + y),
								   ATAPI_SECTOR_SIZE);
						}
