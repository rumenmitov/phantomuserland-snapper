...
static struct uhci_root_hub_configuration_s sUHCIRootHubConfig =
{
...
static struct uhci_root_hub_string_s sUHCIRootHubStrings[3] = {
    {
        4,								// Descriptor length
...
errno_t
UHCIRootHub_ProcessTransfer(UHCI *uhci_dev, Transfer *transfer)
{
    ...
    case USB_REQUEST_GET_STATUS: {
        ...
        if (uhci_dev->GetPortStatus(request->Index - 1, &portStatus) >= B_OK) {
    ...
    case USB_REQUEST_CLEAR_FEATURE: {
        ...
        if (uhci_dev->ClearPortFeature(request->Index - 1, request->Value) >= B_OK)
            status = B_OK;
        break;
    }
    ...
    case USB_REQUEST_SET_FEATURE: {
        ...
        if (uhci_dev->SetPortFeature(request->Index - 1, request->Value) >= B_OK)
            status = B_OK;
        break;
    }
    }
