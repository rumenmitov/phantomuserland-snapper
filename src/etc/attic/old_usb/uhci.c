...
/**
 *
 * 1. Bring in request. Enqueue to our q and device q.
 *
 * 2. Interrupt. Find what is done in our q.
 *
 * 3. Call back.
 *
**/
...
/*
//static
errno_t uhci_enqueue_request(uhci *u, uhci_req *req)
{
    
    hal_mutex_lock( &(u->lock) );
    queue_enter( &(u->all_requests), req, uhci_req *, all_chain);
    hal_mutex_unlock( &(u->lock) );
...
//static
errno_t uhci_process_request_queue(uhci *u)
{
    uhci_req *req;

    hal_mutex_lock( &(u->lock) );
    queue_iterate(&(u->all_requests), req, uhci_req *, all_chain)
    {
        if( IS_PROCESSED(req->td) )
        {
            assert(!on_dev_q(req->td));

            queue_remove(&(u->all_requests), req, uhci_req *, all_chain);
...
static uhci *uhci_init_hc(int interrupt_line, int io_port, physaddr_t reg_base, size_t reg_size)
{
    uhci *u;

    u = calloc(1,sizeof(uhci));
    if(!u) return 0;


    SHOW_INFO(0, "irq %d", interrupt_line);




    queue_init(&(u->all_requests));
...
static int uhci_start(phantom_device_t *dev); // Start device (begin using)
static int uhci_stop(phantom_device_t *dev);  // Stop device
...
static void uhci_interrupt( void *_dev );
...
{
    SHOW_FLOW0( 1, DEV_NAME " probe" );

    if(pci->sub_class != UHCI_SUB_CLASS || pci->interface != UHCI_INTERFACE)
    {
        SHOW_FLOW( 1, DEV_NAME "got subclass %X (need %X), interface %X (need %X)", pci->sub_class, OHCI_SUB_CLASS, pci->interface, OHCI_INTERFACE );
//getchar();
        return 0;
    }
    ...
    for (i = 0; i < 6; i++)
    {
        if (pci->base[i] > 0xffff)
        {
            ...
            SHOW_INFO( 0, DEV_NAME "base 0x%lx, size 0x%lx", phys_base, phys_size);
        } else if( pci->base[i] > 0) {
            io_port = pci->base[i];
            SHOW_INFO( 0,  DEV_NAME "io_port 0x%x", pci->base[i]);
        }
    }

    SHOW_FLOW0( 1,  DEV_NAME " init" );
    uhci *priv = uhci_init_hc(pci->interrupt, io_port, phys_base, phys_size);

    if(priv == 0)
        SHOW_FLOW0( 1, DEV_NAME " init failed" );
    else
        SHOW_FLOW0( 1, DEV_NAME " ready" );
//getchar();
    ...
    dev->dops.start = uhci_start;
    dev->dops.stop = uhci_stop;

    dev->dops.read = uhci_read;
    dev->dops.write = uhci_write;

    if( hal_irq_alloc( dev->irq, &uhci_interrupt, dev, HAL_IRQ_SHAREABLE ) )
    {
    ...
    uhci *u = calloc( sizeof(uhci), 1 );
    dev->drv_private = u;
/*
    for (i = 0; i < UHCI_MAXTD - 1; i++)
        u->td[i].td_link = (u_int32_t) &u->td[i + 1];

    u->td[UHCI_MAXTD - 1].td_link = 0; // last one
...
uhci_alloc_td(uhci *u)
{
    ...
    ret = u->free_td_list;
    u->free_td_list = (uhci_td_t) ret->td_link;
    hal_mutex_unlock(&u->lock);
...
static void
uhci_free_td( uhci *u, uhci_td_t td )
{
    hal_mutex_lock(&u->lock);
    td.td_link = (u_int32_t) u->free_td_list;
    u->free_td_list = &td;
    hal_mutex_unlock(&u->lock);
}
*/
...
static u_int16_t
uhci_readreg16( phantom_device_t *dev, int reg )
{
...
static void
uhci_writereg16( phantom_device_t *dev, int reg, u_int16_t val )
{
    outw (val, dev->iobase + reg);
}

static void
uhci_writereg32( phantom_device_t *dev, int reg, u_int32_t val)
{
...
static void uhci_interrupt( void *_dev )
{
    ...
    SHOW_FLOW( 9, "UHCI %d interrupt", unit );
...
static int uhci_start(phantom_device_t *dev)
{
    int addr = dev->iobase;
    int unit = dev->seq_number;

    //take_dev_irq(dev);
    SHOW_INFO( 1, "start UHCI @%x, unit %d", addr, unit);
