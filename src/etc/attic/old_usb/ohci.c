...
static ohci_ed *create_endpoint(ohci *oi,
								...
								bool lowspeed)
{
	...
	switch (endpoint->attributes & 0x3) {
		...
		case USB_ENDPOINT_ATTR_INT:
			ed->queue = &oi->interrupt_queue[INT_Q_32MS];  // XXX find the correct queue
	...
	ed->flags = (endpoint->max_packet_size << 16) |
				((endpoint->endpoint_address & 0xf) << 7) | (address & 0x7f);
	...
	for (;;) {
		...
		if (oi->hcca->done_head != 0)
			SHOW_FLOW(6,
					  "WDH 0x%x (0x%lx)",
					  oi->hcca->done_head,
					  phys_to_virt(oi, oi->hcca->done_head));

		done_list = NULL;

		// pull all of the entries from the done list, put them in another list in reverse
		// order
		...
		while (done_list) {
			...
			if (OHCI_TD_FLAGS_CC(td->flags) == OHCI_CC_NoError) {
				...
				SHOW_FLOW(6,
						  "stalled transfer, part of usb transfer %p",
						  td->usb_transfer);

				// walk the head pointer of this endpoint and remove the rest of this
				// transfer's descriptors the next one after this one should be at the
				// head of the list
				...
				while (td_phys != 0) {
					...
					SHOW_FLOW(6,
							  "stalled transfer at %p (0x%lx), usb transfer %p",
							  temp_td,
							  td_phys,
							  temp_td->usb_transfer);
				...
				SHOW_FLOW(
					6, "setting head to 0x%lx, tail is at 0x%x", td_phys, td->ed->tail);
...
static void ohci_interrupt(void *arg)
{
	...
	SHOW_FLOW(7,
			  "oi %p int 0x%x (%d) enabled 0x%x disabled 0x%x",
			  oi,
			  int_stat,
			  oi->rh_ports,
			  oi->regs->interrupt_enable,
			  oi->regs->interrupt_disable);
...
static int ohci_create_endpoint(hc_cookie *cookie,
...
static int ohci_enqueue_transfer(hc_cookie *cookie,
								 ...
								 usb_hc_transfer *transfer)
{
	...
	switch (usb_ed->attributes & 0x3) {
		case USB_ENDPOINT_ATTR_CONTROL: {
			...
			// the direction of the transfer is based off of the first byte in the setup
			// data
			...
			// setup descriptor
			td0->flags = OHCI_TD_FLAGS_CC_NOT | OHCI_TD_FLAGS_DIR_SETUP |
						 OHCI_TD_FLAGS_IRQ_NONE | OHCI_TD_FLAGS_TOGGLE_0;

			// vm_get_page_mapping(vm_get_kernel_aspace_id(), (addr_t)transfer->setup_buf,
			// (addr_t *)&td0->curr_buf_ptr);
			...
			SHOW_FLOW(6,
					  "td0 %p (0x%lx) flags 0x%x, curr_buf_ptr 0x%x, buf_end 0x%x, "
					  "next_td 0x%x",
					  td0,
					  td0->phys_addr,
					  td0->flags,
			...
			if (transfer->data_buf != NULL) {
				// data descriptor
				td1->flags = OHCI_TD_FLAGS_CC_NOT | OHCI_TD_FLAGS_TOGGLE_1 |
							 OHCI_TD_FLAGS_IRQ_NONE | OHCI_TD_FLAGS_ROUNDING |
							 (dir ? OHCI_TD_FLAGS_DIR_IN : OHCI_TD_FLAGS_DIR_OUT);

				// vm_get_page_mapping(vm_get_kernel_aspace_id(),
				// (addr_t)transfer->data_buf, (addr_t *)&td1->curr_buf_ptr);
				...
				SHOW_FLOW(6,
						  "td1 %p (0x%lx) flags 0x%x, curr_buf_ptr 0x%x, buf_end 0x%x, "
						  "next_td 0x%x",
						  td1,
						  td1->phys_addr,
						  td1->flags,
			...
			SHOW_FLOW(6,
					  "td2 %p (0x%lx) flags 0x%x, curr_buf_ptr 0x%x, buf_end 0x%x, "
					  "next_td 0x%x",
					  td2,
					  td2->phys_addr,
					  td2->flags,
			...
			SHOW_FLOW(6,
					  "td3 %p (0x%lx) flags 0x%x, curr_buf_ptr 0x%x, buf_end 0x%x, "
					  "next_td 0x%x",
					  td3,
					  td3->phys_addr,
					  td3->flags,
		...
		case USB_ENDPOINT_ATTR_INT: {
			...
			// vm_get_page_mapping(vm_get_kernel_aspace_id(), (addr_t)transfer->data_buf,
			// (addr_t *)&td0->curr_buf_ptr);
...
static void ohci_test(ohci *oi)  // __attribute__((__unused__))
{
	static usb_endpoint_descriptor desc = {
		sizeof(usb_endpoint_descriptor), USB_DESCRIPTOR_ENDPOINT, 0, 0, 8, 0};
	static const usb_request GET_DESCRIPTOR = {USB_REQTYPE_DEVICE_IN,
											   USB_REQUEST_GET_DESCRIPTOR,
											   USB_DESCRIPTOR_DEVICE << 8,
											   0,
											   8};
	static const usb_request SET_ADDR = {
		USB_REQTYPE_DEVICE_OUT, USB_REQUEST_SET_ADDRESS, 1, 0, 0};
	...
	hal_cond_wait(&transfer.completion_cond,
				  &mutex);  //, 1, SEM_FLAG_TIMEOUT, 5000000, NULL);
	...
	SHOW_FLOW(5, "transaction completed with err 0x%x", transfer.status);
	{
		...
		SHOW_FLOW(5,
				  "device: len %d class 0x%x subclass 0x%x protocol 0x%x",
				  dev->length,
				  dev->device_class,
				  dev->device_subclass,
				  dev->device_protocol);
...
static ohci *ohci_init_hc(int interrupt_line, physaddr_t reg_base, size_t reg_size)
{
	...
	/*
	oi->reg_region = vm_map_physical_memory(vm_get_kernel_aspace_id(), "ohci regs", (void
	**)&oi->regs, REGION_ADDR_ANY_ADDRESS, pinfo->u.h0.base_register_sizes[0],
	LOCK_RW|LOCK_KERNEL, pinfo->u.h0.base_registers[0]); if(oi->reg_region < 0) {
	...
	hal_pages_control_etc(reg_base,
						  oi->regs,
						  reg_npages,
						  page_map,
						  page_rw,
	...
	SHOW_INFO(0,
			  "regs at 0x%lx, mapped to %p, size 0x%lx (%d pages)",
			  oi->regs,
			  reg_base,
			  reg_size,
			  reg_npages);
	...
	/*
	oi->hcca_region = vm_create_anonymous_region(vm_get_kernel_aspace_id(), "ohci hcca",
	(void **)&oi->hcca, REGION_ADDR_ANY_ADDRESS, OHCI_HCCA_SIZE,
	REGION_WIRING_WIRED_CONTIG, LOCK_RW|LOCK_KERNEL); if(oi->hcca_region < 0) {
	...
	*/
	...
	// allocate a null endpoint and transfer descriptor for the 63 interrupt queues we'll
	// have
	for (i = 0; i < 63; i++) {
	...
	for (i = 0; i < 2; i++) {
		oi->interrupt_queue[INT_Q_2MS + i].head->next_ed =
			oi->interrupt_queue[INT_Q_1MS].head;
		oi->interrupt_queue[INT_Q_2MS + i].head->next =
			*oi->interrupt_queue[INT_Q_1MS].head_phys;
	}
	for (i = 0; i < 4; i++) {
		oi->interrupt_queue[INT_Q_4MS + i].head->next_ed =
			oi->interrupt_queue[INT_Q_2MS + i / 2].head;
		oi->interrupt_queue[INT_Q_4MS + i].head->next =
			*oi->interrupt_queue[INT_Q_2MS + i / 2].head_phys;
	}
	for (i = 0; i < 8; i++) {
		oi->interrupt_queue[INT_Q_8MS + i].head->next_ed =
			oi->interrupt_queue[INT_Q_4MS + i / 4].head;
		oi->interrupt_queue[INT_Q_8MS + i].head->next =
			*oi->interrupt_queue[INT_Q_4MS + i / 4].head_phys;
	}
	for (i = 0; i < 16; i++) {
		oi->interrupt_queue[INT_Q_16MS + i].head->next_ed =
			oi->interrupt_queue[INT_Q_8MS + i / 8].head;
		oi->interrupt_queue[INT_Q_16MS + i].head->next =
			*oi->interrupt_queue[INT_Q_8MS + i / 8].head_phys;
	}
	for (i = 0; i < 32; i++) {
		oi->interrupt_queue[INT_Q_32MS + i].head->next_ed =
			oi->interrupt_queue[INT_Q_16MS + i / 16].head;
		oi->interrupt_queue[INT_Q_32MS + i].head->next =
			*oi->interrupt_queue[INT_Q_16MS + i / 16].head_phys;
		oi->hcca->interrupt_table[i] =
			oi->interrupt_queue[INT_Q_32MS + i].head->phys_addr;
	}
	...
	// start it up
	oi->regs->control = CONTROL_HCFS_OPERATIONAL | CONTROL_CLE | CONTROL_BLE |
						CONTROL_PLE;
	...
	oi->regs->interrupt_enable = INT_MIE | INT_OC | /* INT_RHSC | */ INT_FNO | INT_UE |
								 INT_RD | INT_WDH | INT_SO;
