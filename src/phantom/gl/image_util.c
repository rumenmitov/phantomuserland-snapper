...
void gl_convertRGBA_to_5R6G5B(unsigned short *pixmap,
							  unsigned char *rgba,
							  int xsize,
							  int ysize)
{
							   ...
							   int ysize)
{
	...
	n = xsize * ysize;
	for (i = 0; i < n; i++) {
		pixmap[i] = (((unsigned int)p[0]) << 16) | (((unsigned int)p[1]) << 8) |
					(((unsigned int)p[2]));
		p += 3;
	}
}

void gl_convertRGBA_to_8A8R8G8B(unsigned int *pixmap,
								unsigned char *rgba,
								int xsize,
								int ysize)
{
	...
	// BUG! This one is hacked for BGRA format
	for (i = 0; i < n; i++) {
		pixmap[i] = (((unsigned int)p[2]) << 16) | (((unsigned int)p[1]) << 8) |
					(((unsigned int)p[0]));
		p += 4;
	}
}
...
void gl_resizeImageNoInterpolate_rgba2rgb(unsigned char *dest,
										  int xsize_dest,
										  int ysize_dest,
										  unsigned char *src,
										  int xsize_src,
