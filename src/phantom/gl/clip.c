...
void gl_transform_to_viewport(GLContext *c, GLVertex *v)
{
	...
	if (c->texture_2d_enabled) {
		v->zp.s =
			(int)(v->tex_coord.X * (ZB_POINT_S_MAX - ZB_POINT_S_MIN) + ZB_POINT_S_MIN);
		v->zp.t =
			(int)(v->tex_coord.Y * (ZB_POINT_T_MAX - ZB_POINT_T_MIN) + ZB_POINT_T_MIN);
	}
}
			...
			clip_func(clip_ymax, +, Y, X, Z)
				...
				clip_func(clip_zmin, -, Z, X, Y)
					...
					clip_func(clip_zmax, +, Z, X, Y)
						...
						float (*clip_proc[6])(V4 *, V4 *, V4 *) = {clip_xmin,
																   clip_xmax,
																   clip_ymin,
																   clip_ymax,
																   clip_zmin,
...
static inline void updateTmp(
	GLContext *c, GLVertex *q, GLVertex *p0, GLVertex *p1, float t)
{
...
static void gl_draw_triangle_clip(
	GLContext *c, GLVertex *p0, GLVertex *p1, GLVertex *p2, int clip_bit);
