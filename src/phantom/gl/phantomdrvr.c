...
static int glX_resize_viewport(GLContext *c, int *xsize_ptr, int *ysize_ptr)
{
	...
	if (xsize > ctx->w->xsize || ysize > ctx->w->ysize)
		return -1;
...
void PhantomSwapBuffers(window_handle_t drawable)
{
	...
	// GrArea(drawable, ctx->gc, 0, 0, ctx->xsize, ctx->ysize, ctx->gl_context->zb->pbuf,
	// ctx->pixtype);

	int565_to_rgba_move(ctx->w->w_pixel,
						(void *)ctx->gl_context->zb->pbuf,
						ctx->w->xsize * ctx->w->ysize);
	// ph_printf("blit ");
