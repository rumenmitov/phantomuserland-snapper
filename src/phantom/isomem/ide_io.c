...
void setup_simple_ide()
{
	...
	*/
	...
	if (!found)
		found = !phantom_pci_find(&cfg, 0x1106, 0x0571);
	...
	if (found) {
		...
		SHOW_INFO(
			0, "Found PCI IDE, BM base reg=0x%X, irq %d", bm_base_reg, cfg.interrupt);

		SHOW_INFO(8,
				  "base 0 = 0x%X, base 1 = 0x%X, base 2 = 0x%X, base 3 = 0x%X, base 4 = "
				  "0x%X, base 5 = 0x%X",
				  cfg.base[0],
				  cfg.base[1],
				  cfg.base[2],
				  ...
				  cfg.base[5]);
		SHOW_INFO(8,
				  "size 0 = 0x%X, size 1 = 0x%X, size 2 = 0x%X, size 3 = 0x%X, size 4 = "
				  "0x%X, size 5 = 0x%X",
				  cfg.size[0],
				  cfg.size[1],
				  cfg.size[2],
	...
	SHOW_INFO(0,
			  "Found %d devices, dev 0 is %s, dev 1 is %s.",
			  numDev,
...
static errno_t ide_reset(int ndev)
{
	...
	if (rc)
		SHOW_ERROR(0, "can't reset ide dev %d", ndev);
	return rc ? EIO : 0;
}
...
static errno_t ide_trim(int ndev, long secno, size_t nsect)
{
	...
	while (nsect > 0) {
		...
		int rc = reg_non_data_lba28(ndev,
									ATA_CMD_DSM,
									DSM_TRIM,
									count,  // feature reg, sect count
									secno);
...
#else
		// unsure, test
		int rc = reg_pio_data_in_lba28(ndev,
									   int cmd,
									   unsigned int fr,
									   unsigned int sc,
									   unsigned long lba,
...
retry:;
	while (nsect > 0) {
		...
		int rc = reg_pio_data_in_lba28(
			ndev, CMD_READ_SECTORS, 0, count, secno, physaddr, count, 1);
#endif
...
errno_t u0AsyncIo(struct phantom_disk_partition *p, pager_io_request *rq)
{
	...
	SHOW_FLOW(11,
			  "part io sect %d, shift %d, o sect %d",
			  rq->blockNo,
			  p->shift,
			  rq->blockNo + p->shift);
...
static void make_unit_part(int unit, void *aio)
{
	...
	phantom_disk_partition_t *p =
		phantom_create_partition_struct(both, 0, disk_info[unit].nSectors);
	assert(p);
...
void connect_ide_io(void)
{
	...
	if (have_dev[0])
		make_unit_part(0, u0AsyncIo);
	if (have_dev[1])
		make_unit_part(1, u1AsyncIo);
}
...
errno_t simple_ide_idenify_device(int dev)
{
	...
	u_int16_t buf[256];
	int rc = reg_pio_data_in_lba28(dev,
								   CMD_IDENTIFY_DEVICE,  // CMD_IDENTIFY_DEVICE_PACKET,
								   0,
								   0,
								   0L,
								   buf,
