...
// extern errno_t partAsyncIo( struct phantom_disk_partition *p, pager_io_request *rq );
// // disk_pool.c extern errno_t partTrim( struct phantom_disk_partition *p,
// pager_io_request *rq ); // disk_pool.c
...
static errno_t startSync(
	phantom_disk_partition_t *p, void *to, long blockNo, int nBlocks, int isWrite)
{
	...
	if (isWrite)
		ph_memcpy(va, to, nBlocks * p->block_size);
	...
	if (!isWrite)
		ph_memcpy(to, va, nBlocks * p->block_size);
	ret = rq.rc;
...
errno_t phantom_sync_read_block(phantom_disk_partition_t *p,
								void *to,
								long blockNo,
								int nBlocks)
{
...
errno_t phantom_sync_write_block(phantom_disk_partition_t *p,
								 const void *to,
								 long blockNo,
								 int nBlocks)
{
...
errno_t phantom_sync_read_sector(phantom_disk_partition_t *p,
								 void *to,
								 long sectorNo,
								 int nSectors)
{
...
errno_t phantom_sync_write_sector(phantom_disk_partition_t *p,
								  const void *to,
								  long sectorNo,
								  int nSectors)
{
...
phantom_disk_partition_t *phantom_create_partition_struct(phantom_disk_partition_t *base,
														  long shift,
														  long size)
{
...
static void lookup_old_pc_partitions(phantom_disk_partition_t *p)
{
	...
	int lookAt = (p->flags & PART_FLAG_IS_WHOLE_DISK) || (p->type == 0x05) ||
				 (p->type == 0x0F) || (p->type == 0x85);
	...
	for (i = 0x1BE; i <= 0x1EE; i += 16, pno++) {
		...
		SHOW_FLOW(2,
				  "Check partition %d, start %d, size %d, type 0x%02X",
				  pno,
				  pp->start,
				  pp->size,
				  pp->type);
		...
		phantom_disk_partition_t *newp =
			phantom_create_partition_struct(p, pp->start, pp->size);
		newp->type = pp->type;
...
static errno_t doPartGetName(phantom_disk_partition_t *p, char *buf, size_t bufsz)
{
	...
	if (ph_strlcat(buf, "/", bufsz) >= bufsz)
		return ENOMEM;
	if (ph_strlcat(buf, p->name, bufsz) >= bufsz)
		return ENOMEM;
	return 0;
}
...
void dump_partition(phantom_disk_partition_t *p)
{
	...
	ph_printf(" - type %d%s\n",
			  p->type,
			  (p->type == PHANTOM_PARTITION_TYPE_ID) ? " (phantom)" : "");
	ph_printf(" - flags %b\n",
			  p->flags,
			  "\020\1PhantomPartType\2PhantomFS\5Bootable\6Divided\7IsDisk");
