...
volatile int phantom_virtual_machine_snap_request =
	0;  //< Non-zero if we want to do snapshot (or doing it)
...
{
	if (hal_cond_init(&phantom_snap_wait_4_vm_enter, "Snap W4E") ||
...
void phantom_snapper_wait_4_threads(void)
{
	...
	while (vm_persistent_memory_lock_count > 0) {
		...
		SHOW_FLOW(5,
				  "Woken up aft wait for threads, %d not stopped yet",
				  vm_persistent_memory_lock_count);
	}
...
void phantom_snapper_reenable_threads(void)
{
	...
	if (phantom_virtual_machine_snap_request > 0) {
		// I'm not one here
		SHOW_FLOW(5,
				  "Snapper - do not reenable threads, "
				  "phantom_virtual_machine_snap_request is still not zero (%d)",
	...
	SHOW_FLOW(5,
			  "Snapper sleep request is %d, will broadcast",
			  phantom_virtual_machine_snap_request);
...
// SYS code does instruction undo/redo so that snap stores state with SYS instruction not
// executed
#if 1


// interlock code of VM blocking syscall (part of .internal.connection class)
// implementation called from si_connection_13_blocking, calls passed syscall worker
// implemented in cn_*.c in kernel
					 ...
					 pvm_object_t *args)
{
	...
	// if( n_param < 1 ) SYSCALL_THROW(pvm_create_string_object( "blocking: need at least
	// 1 parameter" ));
...
#warning FIX ME
	...
	vm_lock_persistent_memory();
#endif
	ref_dec_o(arg);  // BUG FIXME ref will be lost if restart - add to restart list before
					 // call, remove after?
	// BUG FIXME snapper won't continue until this thread is unblocked: end of snap waits
	// for all stooped threads to awake ? fixed with
...
static void do_vm_lock_persistent_memory(void)
{
	...
	GET_CURRENT_THREAD()
		->sub_from_pers_mem_lock_count++;  // to update counter above in thread death
