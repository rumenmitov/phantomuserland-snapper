...
errno_t hal_irq_alloc(int irq, void (*func)(void *arg), void *_arg, int is_shareable)
{
	...
	if (is_shareable && !queue_empty(&heads[irq]) &&
		(!((struct handler_q *)queue_first(&heads[irq]))->is_shareable)) {
...
void hal_softirq_dispatcher(struct trap_state *ts)
{
	...
	while (softirq_requests) {
		...
		for (sirq = SOFT_IRQ_COUNT - 1; sirq >= 0; sirq--) {
			...
			// hal_sti(); // TODO ERROR we have to come to scheduler thread switch without
			// turning interrupts on! We keep spinlock locked!
			...
			if (0 == softirq_requests)
				break;  // Usually we have just one
		}
	}
// final:
