...
static __inline__ void disk_page_io_finish(disk_page_io *me)
{
	disk_page_io_release(me);
}

static __inline__ void disk_page_io_seek(disk_page_io *me, disk_page_no_t dpage)
{
	me->req.disk_page = dpage;
}
static __inline__ disk_page_no_t disk_page_io_tell(disk_page_io *me)
{
	return me->req.disk_page;
}

static __inline__ void *disk_page_io_data(disk_page_io *me)
{
...
static __inline__ void disk_page_io_load_async(disk_page_io *me, disk_page_no_t dpage)
{
	disk_page_io_seek(me, dpage);
	disk_page_io_load_me_async(me);
}
static __inline__ void disk_page_io_save_async(disk_page_io *me, disk_page_no_t dpage)
{
	disk_page_io_seek(me, dpage);
	disk_page_io_save_me_async(me);
}

static __inline__ errno_t disk_page_io_load_sync(disk_page_io *me, disk_page_no_t dpage)
{
	disk_page_io_load_async(me, dpage);
	return disk_page_io_wait(me);
}
static __inline__ errno_t disk_page_io_save_sync(disk_page_io *me, disk_page_no_t dpage)
{
	disk_page_io_save_async(me, dpage);
	return disk_page_io_wait(me);
}

static __inline__ void disk_page_io_load_me_sync(disk_page_io *me)
{
	disk_page_io_load_me_async(me);
	disk_page_io_wait(me);
}
static __inline__ void disk_page_io_save_me_sync(disk_page_io *me)
{
	disk_page_io_save_me_async(me);
	disk_page_io_wait(me);
}
...
static __inline__ void disk_page_cache_seek_async(disk_page_cache *me,
												  disk_page_no_t dpage)
{
	disk_page_cache_flush(me);
	disk_page_cache_wait(me);
...
static __inline__ void disk_page_cache_seek_sync(disk_page_cache *me,
												 disk_page_no_t dpage)
{
	disk_page_cache_flush(me);
	disk_page_cache_wait(me);
...
static __inline__ void disk_page_cache_seek_noread(disk_page_cache *me,
												   disk_page_no_t dpage)
{
	disk_page_cache_flush(me);
	disk_page_cache_wait(me);
...
// virtual void            load_curr() { if(curr_dirty) save_curr();
// head_p.load_sync(curr_page); }
static __inline__ void pagelist_flush(pagelist *me)
{
	disk_page_cache_flush(&me->curr_p);
}

void pagelist_init(pagelist *me, disk_page_no_t root_page, int init, int magic);
static __inline__ void pagelist_finish(pagelist *me)
{
	pagelist_flush(me);
	disk_page_cache_finish(&me->curr_p);
}
...
static __inline__ void pagelist_seek(pagelist *me)
{
	me->curr_displ = me->pos = 0;
	disk_page_cache_seek_async(&me->curr_p, me->root_page);
}
