...
static void pager_free_blocklist_page(disk_page_no_t blk)
{
	...
	if (freed_list_blocks->used >= __countof(freed_list_blocks->list)) {
		struct phantom_mem_blocklist *new_node =
			ph_calloc(1, sizeof(struct phantom_mem_blocklist));
...
void pager_enqueue_for_pagein(pager_io_request *rq)
{
	assert(rq->phys_page);
	if (rq->flag_pagein)
		panic("enqueue_for_pagein: page is already on pager queue (in)");
	if (rq->flag_pageout)
		panic("enqueue_for_pagein: page is already on pager queue (out)");
...
void pager_enqueue_for_pageout(pager_io_request *rq)
{
	assert(rq->phys_page);
	if (rq->flag_pagein)
		panic("enqueue_for_pageout: page is already on pager queue (in)");
	if (rq->flag_pageout)
		panic("enqueue_for_pageout: page is already on pager queue (out)");
...
void pager_init(paging_device *device)
{
	...
	if (_DEBUG)
		hal_printf("Pager get superblock");
	pager_get_superblock();
	if (_DEBUG)
		hal_printf(" ...DONE\n");
...
{
	if (pagein_is_in_process || pageout_is_in_process)
		return;
	if (pagein_is_in_process && pageout_is_in_process)
		panic("double io in start_io");

	if (_DEBUG)
		hal_printf("pager start io\n");
	...
	if (pagein_q_start) {
		...
		paging_device_start_read_rq(pdev,
									pager_current_request,
									page_device_io_final_callback);
	} else if (pageout_q_start) {
		pager_current_request = pageout_q_start;
		pageout_q_start = pager_current_request->next_page;
		if (pageout_q_start == 0)
			pageout_q_end = 0;

		pageout_is_in_process = 1;
		paging_device_start_write_rq(pdev,
									 pager_current_request,
									 page_device_io_final_callback);
	}
}
...
static void pager_io_done()  // called after io is complete
{
	...
	if ((!pagein_is_in_process) && (!pageout_is_in_process))
		panic("stop when no io in pager");
	if (pagein_is_in_process && pageout_is_in_process)
		panic("double io in stop_io");
...
void pager_enqueue_for_pagein(pager_io_request *p)
{
	...
	if (p->flag_pagein || p->flag_pageout)
		panic("enqueue_for_pagein: page is already on pager queue");
	...
	if (pagein_q_start == 0) {
		pagein_q_start = p;
		pagein_q_end = p; /*start_io();*/
	} else {
		pagein_q_end->next_page = p;
		pagein_q_end = p;
	}
...
#else
	if (p->flag_pagein || p->flag_pageout)
		panic("enqueue_for_pagein_fast: page is already on pager queue");
...
void pager_enqueue_for_pageout(pager_io_request *p)
{
	...
	if (p->flag_pagein || p->flag_pageout)
		panic("enqueue_for_pageout: page is already on pager queue");
...
#else
	if (p->flag_pagein || p->flag_pageout)
		panic("enqueue_for_pageout_fast: page is already on pager queue");
...
int find_superblock(phantom_disk_superblock *out,
					disk_page_no_t *pages,
					int n_pages,
					disk_page_no_t exclude,
					disk_page_no_t *where)
{
	int i;
	for (i = 0; i < n_pages; i++) {
...
static int is_default_sb_block(disk_page_no_t page)
{
	for (int i = 0; i < __countof(sb_default_page_numbers); i++) {
		if (sb_default_page_numbers[i] == page)
			return 1;
	}
...
void pager_fix_incomplete_format()
{
	...
	while (is_default_sb_block(free))
		free++;
	...
	for (i = superblock.disk_start_page; i < max; i++) {
		if (i == free || is_default_sb_block(i))
			continue;
...
{
	if (pager_try_get_superblock_from(sb_default_page_numbers[0]) == 0)
		return;
...
int pager_try_get_superblock_from(disk_page_no_t root_block)
{
	...
	if (rc)
		SHOW_ERROR(0, "sb read err @%d", root_block);

	root_sb_cs_ok = phantom_calc_sb_checksum(&root_sb);

	SHOW_FLOW(0,
			  "root sb sys name = '%.*s', checksum %s\n",
			  DISK_STRUCT_SB_SYSNAME_SIZE,
			  root_sb.sys_name,
	...
	SHOW_FLOW(
		7,
		"root sb sb2 addr %d, sb3 addr %d, free start %d, free list %d, fs clean %d\n",
		root_sb.sb2_addr,
		root_sb.sb3_addr,
		root_sb.free_start,
		root_sb.free_list,
	...
	if (root_sb_cs_ok && (!sb1_ok) && !(sb2_ok) && root_sb.sb2_addr == 0 &&
		root_sb.sb3_addr == 0 && root_sb.free_start != 0 && root_sb.free_list == 0 &&
		root_sb.fs_is_clean) {
	...
	if (root_sb_cs_ok && sb1_ok && sb2_ok && superblocks_are_equal(&root_sb, &sb1) &&
		...
		if ((root_sb.version & 0xFFFF) < DISK_STRUCT_VERSION_MINOR)
			hal_printf(
				" Warning: Disk FS minor version number is low: 0x%X, mine is 0x%X...",
				root_sb.version & 0xFFFF,
				DISK_STRUCT_VERSION_MINOR);
	...
	size_t n_sb_default_page_numbers = sizeof(sb_default_page_numbers) /
									   sizeof(disk_page_no_t);
	if (!sb1_ok)
		sb1_ok = find_superblock(
			&sb1, sb_default_page_numbers, n_sb_default_page_numbers, 0, &found_sb1);
	...
	if (sb1_ok) {
		...
		if (!sb2_ok)
			sb2_ok = find_superblock(&sb2,
									 sb_default_page_numbers,
									 n_sb_default_page_numbers,
									 found_sb1,
									 &found_sb2);
	}
...
static void get_copy_dests(disk_page_no_t sb1,
						   disk_page_no_t sb2,
						   disk_page_no_t *sb1_out,
						   disk_page_no_t *sb2_out)
{
...
void pager_update_superblock()
{
	...
	get_copy_dests(superblock.sb2_addr,
				   superblock.sb3_addr,
				   &superblock.sb2_addr,
				   &superblock.sb3_addr);
	...
	if (rc)
		SHOW_ERROR0(0, "Superblock 0 (main) write error!");  // TODO rc


	if (need_fsck) {
		SHOW_ERROR0(
			0,
			" disk is insane, will update root copy only (who called me here, btw?)...");
	} else {
		rc = write_superblock(&superblock, superblock.sb2_addr);
		if (rc)
			SHOW_ERROR0(0, "Superblock 1 write error!");  // TODO rc
	...
	*((phantom_disk_superblock *)disk_page_io_data(&superblock_io)) = superblock;
	...
	if (!need_fsck) {  // XXX : why is this?
		...
		if (rc)
			panic("Superblock 1 write error!");
		rc = disk_page_io_save_sync(&superblock_io, superblock.sb3_addr);
		if (rc)
			panic("Superblock 2 write error!");
	}

	// if secondary write is interrupted, root superblock is still good
	rc = disk_page_io_save_sync(&superblock_io,
								sb_default_page_numbers[3]);  // save secondary copy
	if (rc)
		panic("Superblock secondary write error!");

	// if root write is interrupted, we already have secondary + 2 copies
	rc = disk_page_io_save_sync(&superblock_io,
								sb_default_page_numbers[0]);  // save secondary copy
	if (rc)
		panic("Superblock root write error!");
	SHOW_FLOW0(0, " saved all 3");

	if (need_fsck)  // XXX : what is this?
	{
		SHOW_ERROR0(0,
					" disk is insane, will update root copy only (who called me here, "
					"btw?)...\n");
	}
...
// Call under lock
void pager_extend_active_free_list(disk_page_no_t fp)
{
	...
	struct phantom_disk_blocklist *list =
		(struct phantom_disk_blocklist *)disk_page_io_data(&freelist_io);
	...
	if (active_freelist_head) {
		...
		if (rc)
			panic("pager_extend_active_free_list");
	}
...
{
	hal_mutex_lock(&pager_freelist_mutex);
	if (active_freelist_head == 0)
		panic("No active head");
	...
	errno_t rc = disk_page_io_save_sync(&freelist_io, active_freelist_head);
	if (rc)
		SHOW_ERROR0(0, "freelist block write error!");  // TODO rc
	...
	active_freelist_root =
		((struct phantom_disk_blocklist *)disk_page_io_data(&freelist_io))->head.next;

#endif
	pager_refill_free_reserve();
	if (pager_alloc_page(&active_freelist_head) == 0)
		panic("Out of disk");
...
// call with lock
static void pager_put_to_free_list(disk_page_no_t free_page)
{
...
#if USE_SYNC_IO
		// errno_t rc = phantom_sync_write_block( pp, &u.free_head, superblock.free_list,
		// 1 ); if( rc ) panic( "empty freelist block write error!" );
		write_freehead_sure(superblock.free_list);
...
void pager_refill_free_reserve()
{
	...
	while (free_reserve_n < free_reserve_size) {
		if (list->head.used > 0 && list->head.used < N_REF_PER_BLOCK)
...
#if USE_SYNC_IO
			// errno_t rc = phantom_sync_read_block( pp, &u.free_head,
			// superblock.free_list, 1 ); if( rc ) panic( "empty freelist block read
			// error!" );
			read_freehead_sure();
			...
			if (list->head.magic != DISK_STRUCT_MAGIC_FREEHEAD ||
				list->head.used > N_REF_PER_BLOCK || list->head._reserved != 0) {
	...
	while (free_reserve_n < free_reserve_size &&
		   active_free_start < superblock.disk_page_count)
		free_reserve[free_reserve_n++] = active_free_start++;

	if (active_free_start >= superblock.disk_page_count - 100) {
		SHOW_ERROR(0,
				   "Running out of free space! %x %x",
				   active_free_start,
				   superblock.disk_page_count);
	}
}
...
int64_t pager_calculate_free_block_count(void)
{
	...
	SHOW_INFO(9,
			  "Disk usage stats: %d blocks free (%d.%d%% : %d true [%d startfree, %d in "
			  "freelist], %d nodes)",
			  total_free,
			  percentage / 10,
			  percentage % 10,
...
int pager_alloc_page_locked(disk_page_no_t *out_page_no)
{
	hal_mutex_lock(&pager_freelist_mutex);
	assert(freelist_inited);
...
void pager_free_page(disk_page_no_t page_no)
{
	...
	if (((unsigned long)page_no) < ((unsigned long)superblock.disk_start_page))
		panic("Free: freeing block below disk start: %ld < %ld",
			  (unsigned long)page_no,
			  (unsigned long)superblock.disk_start_page);
...
int pager_fast_fsck()
{
...
#else
	struct phantom_disk_blocklist *flist =
		(struct phantom_disk_blocklist *)disk_page_io_data(&freelist_io);
#endif
	...
	if (flist->head.used > N_REF_PER_BLOCK || flist->head._reserved != 0) {
		need_fsck = 1;
