...
int do_test_video(const char *test_parm)
{
	...
	window_handle_t wb = drv_video_window_create(xsize + 100,
												 ysize + 100,
												 cw_x - 50,
												 cw_y - 50,
												 COLOR_LIGHTRED,
	...
	window_handle_t w = drv_video_window_create(
		xsize, ysize, cw_x, cw_y, console_bg, "Test FG Window", WFLAG_WIN_DECORATED);
	...
	for (i = 0; i < 1; i++) {
#if USE_ZBUF_SHADOW
		scr_zbuf_request_reset_square(
			e1.w.rect.x, e1.w.rect.y, e1.w.rect.xsize, e1.w.rect.ysize);
#else
