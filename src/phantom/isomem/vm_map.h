...
// #define PAGE_TOUCH_HISTORY_SIZE 20
	...
	int wired_count;  // If nonzero, page must be present and can't be moved/paged out.
					  // Physical address must not change.
...
// pages in the end of accessed q to be inaccessible, and page fault will reenable access
// (or will we use hardware access marker?). Unaccessed pages will come to the beginning
// of unaccessed queue. Pages to be freed will be extracted from the end of unaccessed
// queue. Any page access will move page to the beginning of accessed q.
...
void *vm_map_get_object_address_space_start();  // { return
												// start_of_virtual_address_space; }
