...
static int out_of_disk(disk_page_no_t disk_block_num)
{
	...
	if (((unsigned long)disk_block_num) <
		((unsigned long)pager_superblock_ptr()->disk_start_page))
	...
	return ((unsigned long)disk_block_num) >
		   ((unsigned long)pager_superblock_ptr()->disk_page_count);
}
...
static void visit_all(amap_elem_addr_t from,
...
static void iterate_map(void (*i_func)(disk_page_no_t disk_block_num, int flags),
						int flags)
{
...
static int fsck_forlist_ext(disk_page_no_t list_start,
							unsigned int check_magic,
							int (*leaf_f)(disk_page_no_t),
							int (*node_f)(disk_page_no_t));
...
static int fsck_forlist(disk_page_no_t list_start,
						unsigned int check_magic,
						int (*process_f)(disk_page_no_t))
{
							...
							int (*node_f)(disk_page_no_t))
{
	...
	while (next) {
		...
		if ((check_magic != 0) && (curr->head.magic != check_magic)) {
			SHOW_ERROR(0,
					   "FSCK: list magic is wrong, 0x%x instead of 0x%x, blk %d",
					   curr->head.magic,
					   check_magic,
					   currp_no);
			insane = 1;
		...
		if (used > N_REF_PER_BLOCK) {
			SHOW_ERROR(0,
					   "FSCK: overused list page, magic 0x%x, blk %d",
					   curr->head.magic,
					   currp_no);
			insane = 1;
			break;
		...
		if (used != N_REF_PER_BLOCK && next != 0)
			SHOW_ERROR(0,
					   "FSCK warning: incomplete list page, magic 0x%x, blk %d",
					   curr->head.magic,
					   currp_no);
		...
		if (leaf_f != NULL) {
			...
			for (i = 0; i < used; i++) {
				...
				if (lbn) {
					if (out_of_disk(lbn)) {
						SHOW_ERROR(0,
								   "FSCK: list data block is out of disk, blk %d",
								   lbn);
						insane = 1;
						break;
					}
...
static int phantom_fsck_lists()
{
	...
	ph_printf("-- Boot list... ");
	if (sb->boot_list &&
		fsck_forlist(sb->boot_list, DISK_STRUCT_MAGIC_BOOT_LOADER, NULL)) {
		sb->boot_list = 0;
		corruption = 1;
		pager_update_superblock();
	} else
	...
	/*
	if( sb->kernel_list && fsck_forlist( sb->kernel_list, DISK_STRUCT_MAGIC_BOOT_KERNEL,
	NULL ) )
	{
		sb->kernel_list = 0;
		corruption = 1;
		...
		ph_printf("Ok\n");
		*/
	phantom_fsck_do_list(
		&(sb->kernel_list), "Kernel", DISK_STRUCT_MAGIC_BOOT_KERNEL, &corruption);

	int i;
	for (i = 0; i < DISK_STRUCT_N_MODULES; i++) {
		/*if( sb->boot_module[i] && fsck_forlist( sb->boot_module[i],
		DISK_STRUCT_MAGIC_BOOT_MODULE, NULL ) )
		{
			sb->boot_module[i] = 0;
			corruption = 1;
			pager_update_superblock();
		...
		phantom_fsck_do_list(
			&(sb->boot_module[i]), str, DISK_STRUCT_MAGIC_BOOT_MODULE, &corruption);
	}
	...
	phantom_fsck_do_list(
		&(sb->log_list), "Log list", DISK_STRUCT_MAGIC_LOG_LIST, &corruption);
