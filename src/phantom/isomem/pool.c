...
static errno_t do_pool_foreach(
	pool_t *pool,
...
#define MK_HANDLE(p, a, e) \
	(((((p)->magic) & 0xFF) << 24) | ((a & 0xFF) << 16) | (e & 0xFFFF))
...
pool_t *create_pool()
{
	return create_pool_ext(512, 256);
}

pool_t *create_pool_ext(int inital_elems, int arena_size)
{
	if (inital_elems < arena_size)
		inital_elems = arena_size;
...
#define CHECK_ARENA(na)                       \
	if (((na) > pool->narenas) || ((na) < 0)) \
	panic("arena id is wrong: %d", na)
...
#define CHECK_EL(_a, _ne)                                \
	({                                                   \
		int __ne = (_ne);                                \
		if (((__ne) > (_a)->arena_size) || ((__ne) < 0)) \
			panic("arena el is wrong");                  \
	})
#define CHECK_ZERO(_a, _ne)            \
	({                                 \
		int __ne = (_ne);              \
		if ((_a)->ptrs[(__ne)] == 0)   \
			panic("arena el is zero"); \
	})
#define CHECK_REF(_a, _ne)              \
	({                                  \
		int __ne = (_ne);               \
		if (((_a)->refc[(__ne)] <= 0))  \
			panic("arena el ref <= 0"); \
	})

#define CHECK_MAGIC(_p, _h)            \
	({                                 \
		int __hm = HANDLE_2_MAGIC(_h); \
		if (((_p)->magic != __hm))     \
			panic("arena magic");      \
	})
