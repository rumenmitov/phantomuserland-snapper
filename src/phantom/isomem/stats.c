...
static void do_phantom_dump_stats_buf(char *buf, int len)
{
...
#if COMPILE_PERSISTENT_STATS
	rc = ph_snprintf(
		buf, len, "\x1b[35m Statistics    curr/sec t/sec      this run    life\x1b[37m");
#else
	...
	for (i = 0; i < MAX_STAT_COUNTERS; i++) {
		...
		if ((0 == stat_total_counters[i]) && (0 == (long)ps->total_prev_and_this_runs))
			continue;
#else
		// Out of space - skip zero lines
...
static void stat_update_persistent_storage(void *ign)
{
	...
	vm_lock_persistent_memory();
	for (i = 0; i < MAX_STAT_COUNTERS; i++) {
		pdata[i].total_prev_and_this_runs = pdata[i].total_prev_runs +
											stat_total_counters[i];
	}
	vm_unlock_persistent_memory();
#endif
