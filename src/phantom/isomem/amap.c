...
static amap_entry_t *split(amap_t *map, amap_entry_t *e, amap_elem_size_t n_elem_in_first)
{
	...
	amap_entry_t *ne =
		new_entry(e->start + n_elem_in_first, e->n_elem - n_elem_in_first, e->flags);
...
static amap_entry_t *find(amap_t *map, amap_elem_addr_t from)
{
	amap_entry_t *ie;
...
void amap_iterate_all(
	amap_t *map,
	void (*f)(amap_elem_addr_t from, amap_elem_size_t n_elem, u_int32_t flags, void *arg),
	void *arg)
{
	amap_entry_t *ie;
	queue_iterate(&(map->queue), ie, amap_entry_t *, chain)
...
errno_t amap_check(amap_t *map,
				   amap_elem_addr_t from,
				   amap_elem_size_t n_elem,
				   u_int32_t flags)
{
...
static errno_t rejoin_range(amap_t *map, amap_elem_addr_t from, amap_elem_size_t n_elem)
{
	if (from >= map->start + map->n_elem)
		return E2BIG;
...
void amap_dump(amap_t *map)
{
	...
	queue_iterate(&(map->queue), ie, amap_entry_t *, chain)
	{
		...
		*/
		ph_printf("[0x%09qX - 0x%09qX[ : 0x%4X\n",
				  ie->start,
				  ie->start + ie->n_elem,
				  ie->flags);
	}
...
errno_t amap_modify(amap_t *map,
					amap_elem_addr_t from,
					amap_elem_size_t n_elem,
					u_int32_t flags)
{
...
errno_t amap_check_modify(amap_t *map,
						  amap_elem_addr_t from,
						  amap_elem_size_t n_elem,
						  u_int32_t flags,
						  int *modified)
{
	...
	// We can finish later 'cause last elem can be bigger than our interval and have good
	// flags
