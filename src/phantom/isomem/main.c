...
void phantom_multiboot_main()
{
	...
	*/
	...
	// amaps divide all physicall address space and apply some flags on it in order to
	// indicate for which purpose each partition is used. Probably, can be turned off
...
int phantom_main_entry_point(int argc, char **argv, char **envp)
{
	...
	ph_snprintf(phantom_uname.machine,
				sizeof(phantom_uname.machine),
				"%s/%s",
				arch_name,
				board_name);
	...
	*/
	...
	ph_printf("\n\x1b[33m\x1b[44mPhantom " PHANTOM_VERSION_STR
			  " (SVN rev %s) @ %s starting\x1b[0m\n\n",
			  svn_version(),
	...
	*/
	...
	ph_printf("Starting persistent memory test\n");
	{
		...
		for (unsigned long i = 0; i < 200; i++) {
			...
			if (i == 0) {
				// Firstly, check if we have something in a snapshot. If we have, then we
				// just verify that it is fine
				char test_string[] = "phantom snapshot test string is here";
				char *addr_to_check = (char *)hal_object_space_address() +
									  sizeof(struct pvm_object_storage);

				if (ph_strncmp(addr_to_check, test_string, ph_strlen(test_string)) == 0) {
					ph_printf("-- Detected test string in 0th page of obj.space. Will "
					...
					ph_memcpy((void *)hal_object_space_address(),
							  &header,
							  sizeof(struct pvm_object_storage));
...
void start_phantom()
{
	...
	driver_genode_disk_init();  // <- Will register device in Phantom and create necessary
								// structs
