...
errno_t cn_timer_do_operation(int op_no,
							  struct data_area_4_connection *c,
							  struct data_area_4_thread *t,
							  pvm_object_t o)
{
	(void)t;
...
static pvm_object_t cn_timer_blocking_syscall_worker(pvm_object_t conn,
													 ...
													 pvm_object_t arg)
{
	...
	/*
	   ...
	   __XSTRING(__LINE__)  ); return pvm_create_null_object();
			}
...
// Init connection
errno_t cn_timer_init(struct data_area_4_connection *c,
					  struct data_area_4_thread *t,
					  const char *suffix)
{
	(void)t;
