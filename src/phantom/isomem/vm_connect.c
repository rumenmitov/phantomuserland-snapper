 ...
 * object ref count (with the help of pool), and object's own ref count will be
 *incremented just by one so that if kernel crashes we will decrement it on next restart
 *and delete object, if it was used just by kernel. Got it? If no, ask dz. :)
 *
 **/
...
static unsigned int kohandle_entry_hash_func(void *_e,
											 const void *_key,
											 unsigned int range)
{
	kohandle_entry_t *e = _e;
...
#warning check ref dec
	// ref_dec_o(el->o); // We lost our ref - let vm refcount system know - must be done
	// above
}
...
{
	kohandles = hash_init(256,
						  offsetof(kohandle_entry_t, next),
...
errno_t ko_make_object(ko_handle_t *ret,
					   ko_handle_t ko_class,
					   void *init)  // C'tor args? Or call it manually?
{
...
void ko_spawn_method(
	ko_handle_t *ko_this, int method, int nargs, ko_handle_t *args, int flags);
//! Async call - void, does not wait, calls calback
void ko_spawn_method_callback(ko_handle_t *ko_this,
							  int method,
							  int nargs,
							  ko_handle_t *args,
							  int flags,
							  void (*callback)(void *cb_arg, ko_handle_t *ret),
							  void *cb_arg);
...
static struct conntab connection_types_table[] = {
	...
	{"udp:",
	 sizeof(struct cn_udp_persistent),
	 sizeof(struct cn_udp_volatile),
	 CON_F_NAMES(udp)},
	{"fio:",
	 sizeof(struct cn_fio_persistent),
	 sizeof(struct cn_fio_volatile),
	 CON_F_NAMES(fio)},
	{"url:",
	 sizeof(struct cn_url_persistent),
	 sizeof(struct cn_url_volatile),
	 CON_F_NAMES(url)},
};
							   ...
							   struct data_area_4_thread *tc)
{
	...
	if (tc)
		da->owner = tc;  // on restart we get zero tc
	assert(da->owner);
	...
	for (i = 0; i < ctt_size; i++) {
		...
		if (te->persistent_state_size) {
			...
			// also probably worth to make sure contents of objects are not used
			// anywhere... although it is not incref'ed so...
...
errno_t phantom_connection_callback_binary(struct data_area_4_connection *da,
										   void *data,
										   size_t size)
{
...
errno_t phantom_connection_callback_string(struct data_area_4_connection *da,
										   const char *data)
{
