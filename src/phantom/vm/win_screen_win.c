...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	switch (iMessage) {
		...
		case WM_DESTROY:
			exit(0);
			if (MessageBox(hWnd, "Do you really want to quit?", "Message", MB_YESNO) ==
				IDYES)
				PostQuitMessage(0);
			break;
		...
		case WM_PAINT:
			...
			GdiFlush();
			if (!BitBlt(
					hDC, 0, 0, VSCREEN_WIDTH, VSCREEN_HEIGHT, hBitmapDC, 0, 0, SRCCOPY)) {
		...
		case WM_MOUSEHOVER: {
			...
			int yPos = VSCREEN_HEIGHT -
					   (short)(0x0FFFF & (lParam >> 16));  // GET_Y_LPARAM(lParam);
...
void win_scr_init_window(void)
{
	...
	screenBitmap = CreateDIBSection(
		hScreenDC, lpbi, DIB_RGB_COLORS, (void *)&win_src_screen_image, 0, 0);
