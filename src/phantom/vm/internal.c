...
#define IINIT(__cn)                                                              \
	syscall_table_4_##__cn, &n_syscall_table_4_##__cn, pvm_internal_init_##__cn, \
		pvm_gc_iter_##__cn
/*
#else
...
// NB!  GC optimization: if PHANTOM_OBJECT_STORAGE_FLAG_IS_CHILDFREE flag is present, the
// pvm_gc_iter_##something  function will not be used!

struct internal_class pvm_internal_classes[] = {
	{".internal.void",
	 PVM_ROOT_OBJECT_NULL_CLASS,
	 syscall_table_4_void,
	 &n_syscall_table_4_void,
	 ...
	 PHANTOM_OBJECT_STORAGE_FLAG_IS_CHILDFREE | PHANTOM_OBJECT_STORAGE_FLAG_IS_IMMUTABLE |
		 PHANTOM_OBJECT_STORAGE_FLAG_IS_INTERNAL,  // must be internal for sycall func
												   // lookup goes up the class hierarchy
												   // to find some internal one
	 0},
	{".internal.class",
	...
	*/
	...
	{".internal.connection",
	 ...
	 PHANTOM_OBJECT_STORAGE_FLAG_IS_INTERNAL | PHANTOM_OBJECT_STORAGE_FLAG_IS_FINALIZER,
	 0},

	{".internal.float",
	 PVM_ROOT_OBJECT_FLOAT_CLASS,
	 syscall_table_4_float,
	 &n_syscall_table_4_int,
...
#if 0
#define CHECK_NAME(data, len, name) \
	((len == (sizeof(name) - 1)) && (0 == ph_strncmp(data, name, len)))
