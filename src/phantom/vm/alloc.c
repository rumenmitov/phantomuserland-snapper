...
// TODO Object alloc - gigant lock for now. This is to be redone with separate locks for
// buckets/arenas.
...
{
	return pvm_object_space_start;
}
void *get_pvm_object_space_end()
{
	return pvm_object_space_end;
}
...
static inline int find_arena(unsigned int size, unsigned int flags, bool saturated)
{
	...
	if (flags & (PHANTOM_OBJECT_STORAGE_FLAG_IS_CALL_FRAME |
		...
		arena = 0;  // never dies
			 ...
			 (PHANTOM_OBJECT_STORAGE_FLAG_IS_CODE | PHANTOM_OBJECT_STORAGE_FLAG_IS_CLASS |
...
static inline unsigned int round_size(unsigned int size, int arena)
{
	...
	//        size = (((size - 1)/ 72) + 1) * 72 ;  //align 72 bytes (typical sizes: 40
	//        and 68 bytes) - avoid fragmentation
...
void pvm_alloc_clear_mem()
{
	...
	int i;
	for (i = 0; i < ARENAS; i++) {
		init_free_object_header((pvm_object_storage_t *)start_a[i],
								end_a[i] - start_a[i]);
	}
}
...
#define PVM_MIN_FRAGMENT_SIZE       \
	(sizeof(pvm_object_storage_t) + \
	 sizeof(int)) /* should be a minimal object size at least */
...
// suspected to break object land - alloc_collapse_with_next_free: assertion
// 'op->_ah.object_start_marker == PVM_OBJECT_START_MARKER' failed
...
// walk through
static pvm_object_storage_t *alloc_wrap_to_next_object(
	pvm_object_storage_t *op, void *start, void *end, int *wrap, int arena)
{
	assert(op->_ah.object_start_marker == PVM_OBJECT_START_MARKER);
...
bool pvm_object_is_allocated(pvm_object_storage_t *o)
{
	return o->_ah.object_start_marker == PVM_OBJECT_START_MARKER &&
		   ...
		   o->_ah.exact_size <
			   (o->_da_size + sizeof(pvm_object_storage_t) + PVM_MIN_FRAGMENT_SIZE) &&
...
#define CURR_POS curr_a[arena]
	...
	while (result == 0) {
		...
		if (PVM_OBJECT_AH_ALLOCATOR_FLAG_FREE !=
			curr->_ah.alloc_flags)  // refcount == 0, but refzero or in buffer or both
		{
			DEBUG_PRINT("(c)");
...
static pvm_object_storage_t *pool_alloc(unsigned int size, int arena)
{
	...
	do {
		...
		/*
		 * NB!  In fact, it is a huge risk to call GC here:  being called whithin
		 * constructor GC will free objects just allocated but not yet linked to parents.
		 * Highly destructive!
		 *
		 */
		if (vm_alloc_mutex)
...
static int memcheck_one(unsigned int i, void *start, void *end)
{
	...
	ph_printf(
		"Memcheck: checking object memory allocation consistency (at %p, %ld bytes)\n",
		start,
		(long)(end - start));

	while (((void *)curr) < end) {
		if (!pvm_alloc_is_object(curr)) {
			// ph_printf("Memcheck: %ld objects, memory: %ld used, %ld free, %ld
			// largest\n", objects, used, free, largest ); return 0;
	...
	// free ); ph_printf("Memcheck: %ld objects, memory: %ld used, %ld free, %ld largest
	...
	if ((void *)curr == end) {
		ph_printf("Memcheck: reached exact arena end at %p (%ld bytes used)\n",
				  curr,
				  (long)(((void *)curr) - start));
		if (debug_memory_leaks)
			memcheck_print_histogram(i);
		return 0;
	}

	ph_printf("\n\n-----------------\nMemcheck ERROR: reached out of arena end at 0x%p "
			  "(%ld bytes size)\n-----------------\n\n",
			  curr,
			  (long)(((void *)curr) - start));
	return 1;
...
static int64_t count_objects(void *start, void *end)
{
	...
	while (((void *)curr) < end) {
		...
		if (curr->_ah.alloc_flags & PVM_OBJECT_AH_ALLOCATOR_FLAG_ALLOCATED) {
			if (!pvm_object_is_allocated(curr))
				return -1;
			count++;
			used_bytes += curr->_ah.exact_size;

			if (curr->_class == pvm_get_int_class())
				int_count++;
			if (curr->_class == pvm_get_long_class())
				long_count++;
			if (curr->_class == pvm_get_string_class())
				str_count++;
			if (curr->_class == pvm_get_array_class())
				arr_count++;
			if (curr->_class == pvm_get_page_class())
				page_count++;
		}
	...
	SHOW_INFO(0,
			  "Arena @%p, ints: %d, longs: %d, strs: %d, arrs: %d, pages: %d | %d%% used",
			  start,
			  int_count,
			  long_count,
			  str_count,
...
void paint_object_memory_map(window_handle_t w, rect_t *r)
{
	...
	if (vertical_pixels_per_arena[0] == 0) {
		...
		for (i = 0; i < ARENAS; i++) {
			...
			LOG_FLOW(0,
					 "omem=%uK, arena mem=%uK, mem %%=%u, ysize %d, ysize part = %d",
					 omem_bytes / 1024,
					 arena_bytes / 1024,
					 percentage,
					 r->ysize,
	...
	for (y = 0; y < r->ysize; y++) {
		for (x = 0; x < r->xsize; x++) {
			...
			w_draw_pixel(w,
						 x + r->x,
						 y + r->y,
						 (cnt_used == 0) ? COLOR_BLUE
										 : ((cnt_free == 0) ? COLOR_RED : COLOR_GREEN));
