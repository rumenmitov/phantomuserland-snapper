...
/*
  ...
  can't be implemmnted here - we're unable to bring both Phantom and host OS struct stat
in one C file and move fields by hand.
...
int net_curl(const char *url, char *obuf, size_t obufsize, const char *headers)
{
	...
	/*
		...
		if (setsockopt (sockfd, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout,
			...
			error("setsockopt failed\n");
	*/
	// SHOW_FLOW( 0, "TCP - create socket to %d.%d.%d.%d port %d", ip0, ip1, ip2, ip3,
	// port);
	...
	// PhantomOSNetTest/0.1 (PhanomOS i686; ru)\r\nAccept: text/html\r\nConnection:
	// close\r\n\r\n" );
	...
	while (1) {
		nread = read(sock,
					 buf,
					 sizeof(buf) - 1);  // , &addr, SOCK_FLAG_TIMEOUT, 1000L*1000*50
		buf[sizeof(buf) - 1] = 0;
