...
void pvm_set_array_ofield(pvm_object_t o, unsigned int slot, pvm_object_t value)
{
	...
	if (pvm_is_null(da->page) || slot >= da->page_size) {
		...
		if ((!pvm_is_null(da->page)) && da->page_size > 0) {
			...
			da->page = pvm_create_page_object(new_page_size,
											  (void *)&(old_page->da),
											  da->page_size);
			ref_dec_o(old_page);
		} else
...
void pvm_pop_array(pvm_object_t array, pvm_object_t value_to_pop)
{
	...
	verify_p(array);
	if (!(PHANTOM_OBJECT_STORAGE_FLAG_IS_INTERNAL & (array->_flags)) ||
...
pvm_object_t pvm_get_field(pvm_object_t o, unsigned int slot)
{
	verify_p(o);
	if (PHANTOM_OBJECT_STORAGE_FLAG_IS_INTERNAL & (o->_flags)) {
...
void pvm_set_field(pvm_object_t o, unsigned int slot, pvm_object_t value)
{
	verify_p(o);
	verify_o(value);
	...
	if (da_po_ptr(o->da)[slot])
		ref_dec_o(da_po_ptr(o->da)[slot]);  // decr old value
...
{
	if (pvm_is_null(tclass))
		return 0;
	if (pvm_is_null(object))
		return 0;
...
{
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_FINALIZER)
		ph_printf("FINALIZER ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_CHILDFREE)
		ph_printf("CHILDFREE ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_THREAD)
		ph_printf("THREAD ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_STACK_FRAME)
		ph_printf("STACK_FRAME ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_CALL_FRAME)
		ph_printf("CALL_FRAME ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_INTERNAL)
		ph_printf("INTERNAL ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_RESIZEABLE)
		ph_printf("RESIZEABLE ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_STRING)
		ph_printf("STRING ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_INT)
		ph_printf("INT ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_DECOMPOSEABLE)
		ph_printf("DECOMPOSEABLE ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_CLASS)
		ph_printf("CLASS ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_INTERFACE)
		ph_printf("INTERFACE ");
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_CODE)
		ph_printf("CODE ");
}
...
void dumpo(addr_t addr)
{
	...
	if (o->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_STRING) {
		...
		if (da) {
			ph_printf("'");
			pvm_object_print(da->class_name);
			ph_printf("' @%p", o);
			if ((!pvm_isnull(da->class_parent)) &&
				(pvm_get_null_class() != da->class_parent)) {
				ph_printf(" Parent: ");
				pvm_object_dump(da->class_parent);
		...
		if (cl) {
			...
			if (cda) {
				ph_printf("'");
				pvm_object_print(cda->class_name);
				ph_printf("' o@%p class@%p", o, cl);
			} else {
				ph_printf("<no data area>");
			}
		} else
...
pvm_object_t pvm_get_class_name(pvm_object_t o)
{
