...
#define page_push(v)                             \
	do {                                         \
		s->stack[s->common.free_cell_ptr++] = v; \
	} while (0)
...
#define set_me(to)                               \
	do {                                         \
		rootda->common.curr = to;                \
		rootda->curr_da = s = (void *)&(to->da); \
	} while (0)
...
#define check_underflow()                           \
	do {                                            \
		if (page_is_empty()) {                      \
			if (no_prev())                          \
				pvm_exec_panic0("stack underflow"); \
...
#define set_next_prev()                                                                 \
	{                                                                                   \
		pvm_object_da(s->common.next, object_stack)->common.prev = rootda->common.curr; \
		pvm_object_da(s->common.next, object_stack)->common.root = rootda->common.root; \
	}
...
void pvm_ostack_push(struct data_area_4_object_stack *rootda, pvm_object_t o)
{
	...
	check_overflow();
	if (page_is_full())
		panic("opush page full after mkpage");
	page_push(o);
}
...
void pvm_ostack_abs_set(struct data_area_4_object_stack *rootda,
						int abs_pos,
						pvm_object_t val)
{
	...
	while (abs_pos >= pagesize) {
		...
		if (pvm_is_null(c))
			pvm_exec_panic0("o abs_set: out of stack");
		abs_pos -= pagesize;
	}

	// TODO: assert should be here instead of decrement - it is an error in bytecode
	// compiler/implementation
	if (pvm_object_da(c, object_stack)->stack[abs_pos] != 0)
		ref_dec_o(pvm_object_da(c, object_stack)
					  ->stack[abs_pos]);  // decr prev value - avoid memory leak
...
pvm_object_t pvm_ostack_abs_get(struct data_area_4_object_stack *rootda, int abs_pos)
{
	...
	while (abs_pos >= pagesize) {
		c = pvm_object_da(c, object_stack)->common.next;
		if (pvm_is_null(c))
			pvm_exec_panic0("o abs_get: out of stack");
		abs_pos -= pagesize;
	}
 ...
 * \param[in]  depth     Distance to object we need from stack top, 0 = last element on
 *stack.
 *
 **/
...
{
	if (depth < 0)
		pvm_exec_panic0("stack pull: overflow");
	...
	while (displ < 0) {
		...
		if (pvm_is_null(c))
			pvm_exec_panic0("stack pull: underflow");
...
#define set_next_prev()                                                                  \
	{                                                                                    \
		pvm_object_da(s->common.next, integer_stack)->common.prev = rootda->common.curr; \
		pvm_object_da(s->common.next, integer_stack)->common.root = rootda->common.root; \
	}
...
void pvm_istack_reserve(struct data_area_4_integer_stack *rootda, int n_slots)
{
	...
	while (n_slots-- > 0) {
		check_overflow();
		if (page_is_full())
			panic("ipush page full after mkpage");
...
void pvm_istack_abs_set(struct data_area_4_integer_stack *rootda, int abs_pos, int val)
{
	...
	while (abs_pos >= pagesize) {
		...
		if (pvm_is_null(c))
			pvm_exec_panic0("i abs_set: out of stack");
		abs_pos -= pagesize;
	}
...
int pvm_istack_abs_get(struct data_area_4_integer_stack *rootda, int abs_pos)
{
	...
	while (abs_pos >= pagesize) {
		c = pvm_object_da(c, integer_stack)->common.next;
		if (pvm_is_null(c))
			pvm_exec_panic0("i abs_get: out of stack");
		abs_pos -= pagesize;
	}
...
void pvm_lstack_abs_set(struct data_area_4_integer_stack *rootda,
						int abs_pos,
						int64_t val)
{
	unsigned int pagesize = rootda->common.__sSize;
	...
	while (abs_pos >= pagesize) {
		c = pvm_object_da(c, integer_stack)->common.next;
		if (pvm_is_null(c))
			pvm_exec_panic0("l abs_set: out of stack");
		abs_pos -= pagesize;
	}
...
int64_t pvm_lstack_abs_get(struct data_area_4_integer_stack *rootda, int abs_pos)
{
	...
	while (abs_pos >= pagesize) {
		c = pvm_object_da(c, integer_stack)->common.next;
		if (pvm_is_null(c))
			pvm_exec_panic0("l abs_get: out of stack");
		abs_pos -= pagesize;
	}
...
// #undef set_next_prev
// #define make()  pvm_create_istack_object()
// #define set_next_prev()  { pvm_object_da(s->common.next, integer_stack)->common.prev =
// rootda->common.curr; pvm_object_da(s->common.next, integer_stack)->common.root =
// rootda->common.root; }

#define lpage_push(v)                                         \
	do {                                                      \
		((int64_t *)s->stack)[s->common.free_cell_ptr++] = v; \
	} while (0)
...
#define set_next_prev()                                               \
	{                                                                 \
		pvm_object_da(s->common.next, exception_stack)->common.prev = \
			rootda->common.curr;                                      \
		pvm_object_da(s->common.next, exception_stack)->common.root = \
			rootda->common.root;                                      \
	}


void pvm_estack_push(struct data_area_4_exception_stack *rootda,
					 struct pvm_exception_handler e)
{
...
int pvm_estack_foreach(struct data_area_4_exception_stack *rootda,
					   void *pass,
					   int (*func)(void *pass, struct pvm_exception_handler *elem))
{
	struct data_area_4_exception_stack *s = rootda->curr_da;
...
static pvm_object_t pvm_create_general_stack_object(pvm_object_t object_class,
													int ssize,
													int da_size)
{
...
{
	return pvm_create_general_stack_object(pvm_get_istack_class(),
										   PVM_INTEGER_STACK_SIZE,
										   sizeof(struct data_area_4_integer_stack));
}

pvm_object_t pvm_create_ostack_object()
{
	return pvm_create_general_stack_object(pvm_get_ostack_class(),
										   PVM_OBJECT_STACK_SIZE,
										   sizeof(struct data_area_4_object_stack));
}

pvm_object_t pvm_create_estack_object()
{
	return pvm_create_general_stack_object(pvm_get_estack_class(),
										   PVM_EXCEPTION_STACK_SIZE,
...
void pvm_gc_iter_ostack(gc_iterator_call_t func, pvm_object_t os, void *arg)
{
	...
	if (da->common.next != 0)
		gc_fcall(func, arg, da->common.next);  // we are starting from root, so following
											   // next is enough. Tail recursion
}
...
{
	struct data_area_4_exception_stack *sda =
		(struct data_area_4_exception_stack *)os->da;
...
{
	struct data_area_4_exception_stack *da =
		(struct data_area_4_exception_stack *)&(os->da);
