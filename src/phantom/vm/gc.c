...
// see Bacon algorithm (US Patent number 6879991, issued April 12, 2005) or (US Patent
// number 7216136 issued 8 May 2007)
// TODO: not implemented,
// Need persistent cycles_root_buffer not collected by usual gc/refcount - new internal
// object type?
...
// typedef void (*gc_iterator_func_t)( gc_iterator_call_t func, pvm_object_t  os, void
// *arg );
...
void run_gc()
{
	...
	if (vm_alloc_mutex)
		hal_mutex_lock(vm_alloc_mutex);  // TODO avoid Giant lock

	if (my_run != gc_n_run)  // lock acquired when concurrent gc run finished
	{
		if (vm_alloc_mutex)
			hal_mutex_unlock(vm_alloc_mutex);  // TODO avoid Giant lock
	...
	if (gc_flags_last_generation == 0)
		gc_flags_last_generation++;  // != 0 'cause allocation reset gc_flags to zero
	...
	//
	// Root is always used. All other objects, including pvm_root and
	// pvm_root.threads_list, should be reached from root...
...
static int free_unmarked()
{
	...
	for (curr = start; curr < end;
		 curr += ((pvm_object_storage_t *)curr)->_ah.exact_size) {
		...
		if ((p->_ah.gc_flags != gc_flags_last_generation) &&
			(p->_ah.alloc_flags !=
			 PVM_OBJECT_AH_ALLOCATOR_FLAG_FREE))  // touch not accessed but allocated
												  // objects
		{
			if (p->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_FINALIZER) {
				// based on the assumption that finalizer is only valid for some internal
				// childfree objects - is it correct?
				gc_finalizer_func_t func =
					pvm_internal_classes[pvm_object_da(p->_class, class)->sys_table_id]
						.finalizer;
				if (func != 0)
					func(p);
			}
...
static void gc_process_children(gc_iterator_call_t f, pvm_object_storage_t *p, void *arg)
{
	...
	gc_iterator_func_t func =
		pvm_internal_classes[pvm_object_da(p->_class, class)->sys_table_id].iter;
...
// -----------------------------------------------------------------------
// Refcount processor.
// Takes object which is found to be nonreferenced (has
// PVM_OBJECT_AH_ALLOCATOR_FLAG_REFZERO flag) and processes all its children. Those with
// only one ref will become marked with PVM_OBJECT_AH_ALLOCATOR_FLAG_REFZERO flag too.
// -----------------------------------------------------------------------
...
static void do_refzero_process_children(pvm_object_storage_t *p)
{
	...
	if (!(p->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_INTERNAL)) {
		...
		// ref_dec_o( p->_class );  // Why? a kind of mismatch in the compiler, in
		// opcode_os_save8
	...
	// TEMP - skip classes and interfaces too. we must not reach them, in fact... how do
	// we?
	if ((p->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_CLASS) ||
...
static void ref_dec_proccess_zero(pvm_object_storage_t *p)
{
	...
	DEBUG_PRINT("(X)");
	p->_ah.alloc_flags |=
		PVM_OBJECT_AH_ALLOCATOR_FLAG_REFZERO;  // beware of
											   // PVM_OBJECT_AH_ALLOCATOR_FLAG_IN_BUFFER
	p->_ah.alloc_flags &=
		~PVM_OBJECT_AH_ALLOCATOR_FLAG_ALLOCATED;  // beware of
												  // PVM_OBJECT_AH_ALLOCATOR_FLAG_IN_BUFFER
	pvm_collapse_free(p);
#endif
}
...
void do_ref_dec_p(pvm_object_storage_t *p)
{
	...
	if (p->_ah.refCount < INT_MAX)  // Do we really need this check? Sure, we see many
									// decrements for saturated objects!
	{
		// if( 0 == ( --(p->_ah.refCount) ) )
		if (0 == (ATOMIC_ADD_AND_FETCH(&(p->_ah.refCount), -1))) {
			...
			if (p->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_CHILDFREE) {
				if (p->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_FINALIZER) {
					// based on the assumption that finalizer is only valid for some
					// internal childfree objects - is it correct?
					gc_finalizer_func_t func =
						pvm_internal_classes[pvm_object_da(p->_class, class)
												 ->sys_table_id]
							.finalizer;
					if (func != 0)
						func(p);
		...
		// if we decrement refcount and stil above zero - mark an object as potential
		// cycle root; and internal objects can't be a cycle root (sic!)
		else {
		nonzero:;
			if (!(p->_flags & PHANTOM_OBJECT_STORAGE_FLAG_IS_INTERNAL)) {
				if (!(p->_ah.alloc_flags & PVM_OBJECT_AH_ALLOCATOR_FLAG_IN_BUFFER)) {
				...
				p->_ah.alloc_flags |=
					PVM_OBJECT_AH_ALLOCATOR_FLAG_WENT_DOWN;  // set down flag
			}
		}
		// nokill:;
	}
...
void ref_inc_p(pvm_object_storage_t *p)
{
	...
	if (p->_ah.refCount < INT_MAX) {
		...
		if (p->_ah.alloc_flags & PVM_OBJECT_AH_ALLOCATOR_FLAG_IN_BUFFER)
			p->_ah.alloc_flags &=
				~PVM_OBJECT_AH_ALLOCATOR_FLAG_WENT_DOWN;  // clear down flag
	}
}
