...
void videotest_truetype(void)
{
	drv_video_window_t *w = drv_video_window_create(
		WXS, WYS, 300, 300, COLOR_BLACK, "TrueType Test Window", WFLAG_WIN_DECORATED);
	...
	w_font_draw_string(
		w, &drv_video_8x16san_font, "Test font", COLOR_BLACK, COLOR_GREEN, 0, 0);
	...
	*/
	...
	if (font2 == INVALID_POOL_HANDLE) {
		// w_font_draw_string( w, &drv_video_8x16san_font, "TrueType Font Failed",
		// COLOR_BLACK, COLOR_GREEN, 0, 30 );
		ph_printf("\n\nTTF 2 FAIL\n\n");
	} else
...
void videotest_overlay(void)
{
	...
	drv_video_window_t *w_large = drv_video_window_create(
		WXS, WYS, 100, 100, COLOR_BLACK, "Large Window", WFLAG_WIN_DECORATED);

	drv_video_winblt(w_large);


	drv_video_window_t *w_small = drv_video_window_create(
		WXS / 2, WYS / 2, 150, 150, COLOR_BLACK, "Small Window", WFLAG_WIN_DECORATED);
...
void videotest_pbm()
{
	...
	if (result)
		...
		drv_video_window_t *w = drv_video_window_create(
			900, 600, 10, 10, COLOR_BLACK, "BMP Test Window", WFLAG_WIN_DECORATED);
		drv_video_winblt(w);
