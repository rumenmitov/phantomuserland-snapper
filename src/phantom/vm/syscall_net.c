...
static int si_tcp_tostring_5(pvm_object_t me,
							 pvm_object_t *ret,
...
static int si_tcp_connect_16(pvm_object_t me,
							 pvm_object_t *ret,
							 struct data_area_4_thread *tc,
							 int n_args,
							 pvm_object_t *args)
...
static int si_tcp_disconnect_17(pvm_object_t me,
								pvm_object_t *ret,
								struct data_area_4_thread *tc,
								int n_args,
								pvm_object_t *args)
...
static int si_tcp_send_19(pvm_object_t me,
						  pvm_object_t *ret,
						  struct data_area_4_thread *tc,
						  int n_args,
						  pvm_object_t *args)
...
static int si_tcp_recv_21(pvm_object_t me,
						  pvm_object_t *ret,
						  struct data_area_4_thread *tc,
						  int n_args,
						  pvm_object_t *args)
...
static int si_tcp_accept_23(pvm_object_t me,
							pvm_object_t *ret,
							struct data_area_4_thread *tc,
							int n_args,
							pvm_object_t *args)
...
static int si_tcp_curl_24(pvm_object_t me,
						  pvm_object_t *ret,
						  struct data_area_4_thread *tc,
						  int n_args,
						  pvm_object_t *args)
{
	...
	if (url_len > 1024)
		SYSCALL_THROW_STRING("URL too long");
	if (hdr_len > 1024)
		SYSCALL_THROW_STRING("HTTP headers too long");

	char surl[url_len + 1];
	ph_strlcpy(surl, pvm_get_str_data(url), url_len + 1);
	char shdr[hdr_len + 1];
	ph_strlcpy(shdr, pvm_get_str_data(hdr), hdr_len + 1);
