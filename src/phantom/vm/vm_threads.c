...
static void remove_vm_thread_from_list(pvm_object_storage_t *os)
{
	...
	if (!nthreads)
		SHOW_ERROR0(
			0, "There were 0 live threads in image, and some thread is dead. Now -1?");
	...
	while (nthreads--) {
		...
		if (th == os) {
			pvm_set_array_ofield(pvm_root.threads_list,
								 nthreads,
...
static void start_new_vm_thread(pvm_object_t new_thread)
{
	...
	// int tid = hal_start_thread(thread_run_func, &new_thread,
	// THREAD_FLAG_VM|THREAD_FLAG_USER);
	...
	/*
		...
		if(tda->sleep_flag)
		{
			...
			if(!didit)
			{
				SHOW_ERROR( 0, "Sleeping VM thread has no means to wakeup (%p)",
	new_thread );
			}
...
void phantom_finish_all_threads(void)
{
	...
	phantom_virtual_machine_snap_request =
		1;  // TODO check snap_sync to make sure it works as supposed with
			// vm_lock_persistent_memory based code
