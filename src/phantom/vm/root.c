...
void pvm_root_init(void)
{
	...
	dbg_add_command(runclass,
					"runclass",
					"runclass class [method ordinal] - create object of given class and "
					"run method (ord 8 by default)");
	...
	if (root->_ah.object_start_marker != PVM_OBJECT_START_MARKER) {
		...
		vm_unlock_persistent_memory();  // We return to main, rest of code assumed to not
										// to have objects access
		return;
	}
	...
	for (i = 0; i < pvm_n_internal_classes; i++) {
		pvm_internal_classes[i].class_object =
			pvm_get_field(root, pvm_internal_classes[i].root_index);
	}
	...
	vm_unlock_persistent_memory();  // We return to main, rest of code assumed to not to
									// have objects access
}
...
static void process_generic_restarts(pvm_object_t root)
{
	...
	ref_inc_o(prev_restart_list);  // Below we set array slot, erasing ptr to
								   // prev_restart_list, which leads to ref dec to us
...
static void pvm_create_root_objects()
{
	...
	// make sure refcount is disabled for all objects created here: 3-rd argument of
	// pvm_object_alloc is true (obsolete: ref_saturate_p)
	...
	pvm_root.sys_interface_object =
		pvm_object_alloc(N_SYS_METHODS * sizeof(pvm_object_t), flags, 0);
	...
	int i;
	for (i = 0; i < pvm_n_internal_classes; i++) {
		flags = PHANTOM_OBJECT_STORAGE_FLAG_IS_INTERNAL |
		...
		// pvm_internal_classes[i].class_object.interface      =
		// pvm_root.sys_interface_object.data;
	}
  ...
  pvm_internal_init_fs(pvm_get_fs_class());
	...
	// ref_saturate_o(pvm_root.restart_list); //Need it? definitely no - this list is
	// recreated each restart
...
static void set_root_from_table()
{
#define SET_ROOT_CLASS(cn, ri)                                                        \
	(pvm_root.cn##_class =                                                            \
		 pvm_internal_classes[pvm_iclass_by_root_index(PVM_ROOT_OBJECT_##ri##_CLASS)] \
			 .class_object)
...
GCINLINE pvm_object_t pvm_get_null_class()
{
	return pvm_root.null_class;
}
GCINLINE pvm_object_t pvm_get_class_class()
{
	return pvm_root.class_class;
}
GCINLINE pvm_object_t pvm_get_interface_class()
{
	return pvm_root.interface_class;
}
GCINLINE pvm_object_t pvm_get_code_class()
{
	return pvm_root.code_class;
}
GCINLINE pvm_object_t pvm_get_int_class()
{
	return pvm_root.int_class;
}
GCINLINE pvm_object_t pvm_get_long_class()
{
	return pvm_root.long_class;
}
GCINLINE pvm_object_t pvm_get_float_class()
{
	return pvm_root.float_class;
}
GCINLINE pvm_object_t pvm_get_double_class()
{
	return pvm_root.double_class;
}
GCINLINE pvm_object_t pvm_get_string_class()
{
	return pvm_root.string_class;
}
GCINLINE pvm_object_t pvm_get_array_class()
{
	return pvm_root.array_class;
}
GCINLINE pvm_object_t pvm_get_page_class()
{
	return pvm_root.page_class;
}
GCINLINE pvm_object_t pvm_get_thread_class()
{
	return pvm_root.thread_class;
}
GCINLINE pvm_object_t pvm_get_call_frame_class()
{
	return pvm_root.call_frame_class;
}
GCINLINE pvm_object_t pvm_get_istack_class()
{
	return pvm_root.istack_class;
}
GCINLINE pvm_object_t pvm_get_ostack_class()
{
	return pvm_root.ostack_class;
}
GCINLINE pvm_object_t pvm_get_estack_class()
{
	return pvm_root.estack_class;
}
GCINLINE pvm_object_t pvm_get_boot_class()
{
	return pvm_root.boot_class;
}
GCINLINE pvm_object_t pvm_get_binary_class()
{
	return pvm_root.binary_class;
}
GCINLINE pvm_object_t pvm_get_bitmap_class()
{
	return pvm_root.bitmap_class;
}
GCINLINE pvm_object_t pvm_get_closure_class()
{
	return pvm_root.closure_class;
}
GCINLINE pvm_object_t pvm_get_world_class()
{
	return pvm_root.world_class;
}
#if COMPILE_WEAKREF
GCINLINE pvm_object_t pvm_get_weakref_class()
{
	return pvm_root.weakref_class;
}
#endif
GCINLINE pvm_object_t pvm_get_window_class()
{
	return pvm_root.window_class;
}
GCINLINE pvm_object_t pvm_get_directory_class()
{
	return pvm_root.directory_class;
}
GCINLINE pvm_object_t pvm_get_connection_class()
{
	return pvm_root.connection_class;
}

GCINLINE pvm_object_t pvm_get_mutex_class()
{
	return pvm_root.mutex_class;
}
GCINLINE pvm_object_t pvm_get_cond_class()
{
	return pvm_root.cond_class;
}
GCINLINE pvm_object_t pvm_get_sema_class()
{
	return pvm_root.sema_class;
}

GCINLINE pvm_object_t pvm_get_tcp_class()
{
	return pvm_root.tcp_class;
}
GCINLINE pvm_object_t pvm_get_udp_class()
{
	return pvm_root.udp_class;
}

GCINLINE pvm_object_t pvm_get_net_class()
{
	return pvm_root.net_class;
}
GCINLINE pvm_object_t pvm_get_http_class()
{
	return pvm_root.http_class;
}
GCINLINE pvm_object_t pvm_get_time_class()
{
	return pvm_root.time_class;
}
GCINLINE pvm_object_t pvm_get_stat_class()
{
	return pvm_root.stat_class;
}
GCINLINE pvm_object_t pvm_get_io_class()
{
	return pvm_root.io_class;
}
GCINLINE pvm_object_t pvm_get_port_class()
{
	return pvm_root.port_class;
}
GCINLINE pvm_object_t pvm_get_ui_control_class()
{
	return pvm_root.ui_control_class;
}
GCINLINE pvm_object_t pvm_get_ui_font_class()
{
...
void phantom_setenv(const char *name, const char *value)
{
	...
	pvm_object_t s = pvm_create_string_object_binary_cat(
		ename, ph_strlen(ename), value, ph_strlen(value));
...
static o_restart_func_t find_restart_f(pvm_object_t _class)
{
	...
	// TODO fix this back
	// if( syscall_index >= da->class_sys_table_size ) pvm_exec_panic("find_syscall:
	// syscall_index no out of table size" );
