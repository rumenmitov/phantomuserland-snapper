...
#define opcode_copy \
	0x19  // create new object, copy of stack top (just copy of data area as is) TODO do
		  // we need it?
#define opcode_os_compose32 \
	0x1A  // n objects from ostack combine into the object. topmost is a class
#define opcode_os_decompose 0x1B  // decompose topmost object on stack
// deprecated? no, we use it
#define opcode_os_pull32 \
	0x1C  // copy opbject n steps down the ostack on top. pull 0 is dup;
// deprecated and was not implemented
// id(opcode_os_assign32,0x1D) // copy stack top opbject n steps down the ostack. pull 0
// is nop
...
#define opcode_const_pool \
	0x28  // int32 follows - get constant with corresponding index from object constant
		  // pool of this class
#define opcode_stack_reserve \
	0x29  // int8 object stack size to reserve (push zeroes), int8 integer stack size to
		  // reserve
// ? unclear how to init objects
// id(opcode_stack_init,0x2A)  // int32 const pool id, int8 object stack size to init
// (push data from const pool??), int8 integer stack size to init (rest of constant data)
#define opcode_cast 0x2B  // pop class, pop object, cast, push object
#define opcode_push_catcher \
	0x2D  // jump address folows, top of o stack - class of objects to catch
#define opcode_pop_catcher 0x2E
#define opcode_throw \
	0x2F  // thow top of stack, if stack is empty - will throw special system-wide object,
...
#define opcode_prefix_long \
	0x5C  // next operation works on longs (uses 2x slots on int stack)
#define opcode_prefix_float \
	0x5D  // next operation works on floats (uses 1x slots on int stack)
#define opcode_prefix_double \
	0x5E  // next operation works on doubles (uses 2x slots on int stack)
// id(opcode_lock_this, 0x60)		// mutex in 'this' is locked, automatic unlock on
// return id(opcode_unlock_this, 0x61)	// mutex in 'this' is unlocked
...
// 64-6e
#define opcode_static_invoke \
	0x6E  // arg int32 method ordinal, int32 n_args. stack (from top): class ptr, this,
		  // args
#define opcode_dynamic_invoke \
	0x6F  // no args. stack (from top): string method name, this (or null for static),
		  // n_args, args
#define opcode_ishl 0x70  // shift left
#define opcode_ishr 0x71  // shift right signed
#define opcode_ushr 0x72  // shift right unsigned
#define opcode_arg_count \
	0x73  // byte with number of args this func waits for - compares with int on istack,
		  // throws if not eq
#define opcode_fromi \
	0x74  // cast int from int stack to current (as defined by prefix) type on int stack
#define opcode_froml 0x75  // cast from long
#define opcode_fromf 0x76  // cast from float
#define opcode_fromd 0x77  // cast from double
// 73-7f
// We can't have sys shortcuts because of syscall restart needs fixed sycall instr size
// for restart
