...
static pvm_object_t pvm_exec_find_static_method(pvm_object_t class_ref,
												int method_ordinal);
static int pvm_exec_find_catch(struct data_area_4_exception_stack *stack,
							   unsigned int *jump_to,
							   pvm_object_t thrown_obj);

static int pvm_exec_assert_type(struct data_area_4_thread *da,
								pvm_object_t obj,
								pvm_object_t type);
 ...
 *        all push/pop/get/set operations with objects just copy pointers and do not
 * change refcount
...
#define LISTI(iName)                                    \
	do {                                                \
		if (debug_print_instr)                          \
			lprintf("%s @ %d; ", (iName), da->code.IP); \
	} while (0)
#define LISTIA(fmt, a)                       \
	do {                                     \
		if (debug_print_instr) {             \
			lprintf((fmt), a);               \
			lprintf(" @ %d; ", da->code.IP); \
		}                                    \
	} while (0)
...
{
	struct data_area_4_call_frame *cf =
		(struct data_area_4_call_frame *)&(da->call_frame->da);
...
{
	pvm_object_t ret =
		pvm_object_da(da->call_frame, call_frame)->prev;  // prev exists - we guarantee it

	pvm_object_t return_val = os_empty() ? pvm_get_null_object()
										 : os_pop();  // return value not in stack? Why?
 ...
 *
 **/
static void pvm_exec_do_throw_object(struct data_area_4_thread *da,
									 pvm_object_t thrown_obj)
{
	...
	os_push(thrown_obj);
	if (DEB_CALLRET || debug_print_instr)
		ph_printf("throw stack_depth -> %d)", da->stack_depth);
}
...
static void pvm_exec_sys(struct data_area_4_thread *da, unsigned int syscall_index)
{
	...
	if (func == 0) {
		...
		if (0 == func(o, &ret, da, n_param, args))  // exec syscall
		{
			...
			pvm_exec_do_throw_object(
				da, ret ? ret : pvm_create_string_object("syscall thrown null"));
...
static void init_cfda(struct data_area_4_thread *da,
					  ...
					  pvm_object_t class_ref)
{
	...
	// ph_printf("new_this @%p %s\n", new_this, pvm_is_null(new_this) ? "null" : "not
	// null"); ph_printf("class_ref @%p\n", class_ref);
	...
	// fill 'em in correct order
	{
		unsigned int i;
		for (i = n_param; i; i--) {
			pvm_ostack_abs_set(pvm_object_da(cfda->ostack, object_stack),
							   i - 1,
							   os_pop());
		}
	}
	...
	if (!pvm_is_null(class_ref)) {
		...
		if (!related) {
			ph_printf("new_this @%p: ", new_this);
			pvm_object_dump(new_this);
			ph_printf("class_ref @%p: ", class_ref);
			pvm_object_dump(class_ref);
...
static void pvm_exec_call(struct data_area_4_thread *da,
						  unsigned int method_index,
						  unsigned int n_param,
						  int do_optimize_ret,
						  pvm_object_t new_this)
{
	...
	if ((opcode_os_drop == pvm_code_get_byte_speculative(&(da->code)) &&
		...
		(opcode_ret == pvm_code_get_byte_speculative(&(da->code)) && do_optimize_ret)) {
 ...
 * \param[in]  n_param         Number of parameters to copy on new stack
 * \param[in]  class_ref       Class to look up method in (we do static call, not use
 *'this') \param[in]  new_this        This for called method
 *
 * Create new call frame, init it.
 *
 * Set new call frame as current.
...
static void pvm_exec_static_call(struct data_area_4_thread *da,  // Current thread
								 int method_ordinal,
								 int n_param,
...
void pvm_exec_simulated_call(struct data_area_4_thread *da,
							 pvm_object_t new_this,
							 int method,
							 int n_args,
							 pvm_object_t args[])
 ...
 *
 * This function returns true if top bytecode method returns or uncatched exception
 *happens.
 *
 * TODO: exception must be recorded somehow, no?
 *
 * TODO: have special checkpoint to check for snapshot is going to start. Pause and
 *save_fast_acc, report being ready for snap. Will be unpaused by snapshot code after
 *finishing 1st phase (setting all pages to readonly).
 *
 * TODO: Long (blocking) operations called from here can't be recorded in snapshot (C
 *stack is not in snap and can't be there), so we have to restart them each snap - set a
 *longjmp return point for that, return there and restart last opcode on restart.
 *
 * TODO: If thread was in a driver during snap, do a different longjmp, and make a mark in
 *a thread's state that if thread is restored, it has to get special REBOOT exception
 *delivered. Reset that flag after returning from syscall and before being ready for a
 *next snap..
 *
 *
 *
...
#define DO_FPOINT (prefix_float || prefix_double)
	...
	while (1) {
...
#if NEW_SNAP_SYNC
		touch_snap_catch();  // touch special memory page, if snap is scheduled, page will
							 // be write-protected and we'll gen page fault
#else
		if (phantom_virtual_machine_snap_request) {
			pvm_exec_save_fast_acc(da);  // Before snap
			phantom_thread_wait_4_snap();
			// pvm_exec_load_fast_acc(da); // We don't need this, if we die, we will enter
			// again from above :)
		}
#endif
		...
		if (prefix_long) {
			...
			switch (instruction) {
				default:
					prefix_long = 1;  // attempt nonprefix impl of inctruction, maybe it
				...
				case opcode_isublu:
					LISTI("l-isublu");
					{
						int64_t u = ls_pop();
						int64_t l = ls_pop();
						if (0 || debug_print_instr)
							ph_printf("%lld - %lld ; ", l, u);
						ls_push(l - u);
					}
					break;
				...
				case opcode_idivlu:
					LISTI("l-idivlu");
					{
						int64_t u = ls_pop();
						int64_t l = ls_pop();
						// if(debug_print_instr) ph_printf("%ld/%ld = %ld;", (long)l,
						// (long)u, (long)(l/u) );
						if (0 || debug_print_instr)
							ph_printf("%lld/%lld",
									  l,
									  u);  // next line can crash, let me be printed
						if (0 || debug_print_instr)
							ph_printf(" = %lld ; ", l / u);
						ls_push(l / u);
					}
		...
		if (prefix_float) {
			...
			switch (instruction) {
				default:               // Try classic implementation of that op
					prefix_float = 1;  // attempt nonprefix impl of inctruction, maybe it
									   // checks for a modifier
					goto noprefix;
		...
		if (prefix_double) {
			...
			switch (instruction) {
				default:
					prefix_double = 1;  // attempt nonprefix impl of inctruction, maybe it
										// checks for a modifier
					goto noprefix;
				...
				case opcode_isublu:
					LISTI("d-isublu");
					{
						...
						ls_push(TO_LONG(r));
						// ph_printf("d-sublu %g - %g -> %g ( %g ) ; ", TO_DOUBLE(l),
						// TO_DOUBLE(u), r, TO_LONG(r) );
					}
					break;
				...
				case opcode_idivlu:
					LISTI("d-idivlu");
					{
						...
						double r = AS_DOUBLE(l, u, /);
						ls_push(TO_LONG(r));
						// ph_printf("d-idivlu %g / %g -> %g ( %g ) ; ", TO_DOUBLE(l),
						// TO_DOUBLE(u), r, TO_LONG(r) );
					}
					break;
	...
	noprefix:
		switch (instruction) {
				...
				// NB! Not break, continue, or else code after the main switch will reset
				// prefix and print warning
			...
			case opcode_debug: {
				...
				if (is_empty())
					ph_printf(", istack empty");
				else
					ph_printf(",\n\tistack top = %d", is_top());
				if (os_empty())
					ph_printf(", ostack empty");
				else {
					ph_printf(",\n\tostack depth %d top = {",
							  pvm_ostack_count(da->_ostack));
			...
			case opcode_cast: {
				...
				if (1 || debug_print_instr) {
					...
					// if( DEB_CALLRET || debug_print_instr ) ph_printf( "%d)",
					// da->stack_depth );
#else
					// ignore - there's duplicate '.ru.dz.phantom.system.shell' for some
					// reason :(
			...
			case opcode_summon_null:
				LISTI("push null");
				os_push(
					pvm_get_null_object());  // so what opcode_os_push_null is for then?
				break;
			...
			case opcode_new:
				LISTI("new");
				{
					...
					os_push(no);
					// ph_printf("\nop_new\tclass @%p\t", cl ); ph_printf("new_this @%p
        ...
        case opcode_os_decompose:
			...
			case opcode_short_call_0:
				pvm_exec_call(da, 0, 0, 1, pvm_get_null_object());
				break;
			case opcode_short_call_1:
				pvm_exec_call(da, 1, 0, 1, pvm_get_null_object());
				break;
			case opcode_short_call_2:
				pvm_exec_call(da, 2, 0, 1, pvm_get_null_object());
				break;
			...
			case opcode_static_invoke: {
				...
				// ph_printf("\nstatic_invoke\nnew_this @%p %s\n", new_this,
				// pvm_is_null(new_this) ? "null" : "not null"); ph_printf("class_ref
				// @%p\n", class_ref);
			...
			case opcode_stack_reserve: {
				...
				// ); pvm_istack_reserve( pvm_object_da(da->_istack, integer_stack),
				// i_reserve );
...
// sys_sleep:
				...
				if ((instruction & 0xE0) == opcode_call_00) {
					...
					pvm_exec_call(
						...
						n_param,
						0,
						pvm_get_null_object());  // no optimization for soon return
					break;
				}
		...
		// NB! We have continue after instructions that set prefix (long/float/double) so
		// that control does not reach here and we do not reset prefix and do not print
		// warning
...
syscall_func_t pvm_exec_find_syscall(pvm_object_t _class, unsigned int syscall_index)
{
	...
	// pvm_exec_panic("find_syscall: internal class index (%d) out of table (%d)",
	// da->sys_table_id, pvm_n_internal_classes );
...
static pvm_object_t pvm_exec_get_iface_method(pvm_object_t iface,
											  unsigned int method_index)
{
	if (iface == 0)
 ...
 * \param[in]  thrown_obj   Object that is being thrown, class of this object is checked
 *against catcher class
 *
...
void pvm_exec_set_cs(struct data_area_4_call_frame *cfda, pvm_object_t code)
{
	...
	cfda->code = cda->code;
	// TODO set ref from cfda to code for gc to make sure code wont be collected while
	// running
	// pvm_object_t co;
	// co = code;
	// co.interface = 0;
 ...
 *       system start. Cache is implemented in userland and can be absent in early boot
 *time. See syscall.c pvm_class_cache_* funcs implementing such cache. See also
 *pvm_root.class_dir
 *
...
pvm_object_t pvm_exec_lookup_class_by_name(pvm_object_t name)
{
	...
	// Try persistent class cache
	{
		...
		errno_t rc = pvm_class_cache_lookup(pvm_get_str_data(name),
											pvm_get_str_len(name),
											&found_class);
		if (rc == 0)
			return found_class;
	}
	...
	if (!pvm_is_null(found_class)) {
		// errno_t rc =
		pvm_class_cache_insert(pvm_get_str_data(name),
							   pvm_get_str_len(name),
							   found_class);
	}
#endif
								 ...
								 pvm_object_t args[])
{
	...
	int i;
	for (i = n_args; i > 0; i--) {
		pvm_ostack_push(pvm_object_da(cfda->ostack, object_stack),
						ref_inc_o(args[i - 1]));
	}

	pvm_istack_push(pvm_object_da(cfda->istack, integer_stack),
					n_args);  // pass him real number of parameters
	...
	// vm_unlock_persistent_memory(); // pvm_exec will lock, do not need nested lock -
	// will prevent snaps
	//  NO - we must prevent snap here!! it won't work right after restart, part of state
	//  is in C code
	pvm_exec(thread);
	// vm_lock_persistent_memory();
