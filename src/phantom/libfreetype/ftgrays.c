...
static void gray_render_scanline(
	RAS_ARG_ TCoord ey, TPos x1, TCoord y1, TPos x2, TCoord y2)
{
...
static void gray_render_line(RAS_ARG_ TPos to_x, TPos to_y)
{
	...
	if (ey1 != ey2) {
		...
		while (ey1 != ey2) {
			...
			gray_render_scanline(
				RAS_VAR_ ey1, x, (TCoord)(ONE_PIXEL - first), x2, (TCoord)first);
			x = x2;
	...
	gray_render_scanline(RAS_VAR_ ey1, x, (TCoord)(ONE_PIXEL - first), to_x, fy2);
...
/* cells table                                              */
static void gray_shell_sort(PCell cells, int count)
{
...
static void gray_quick_sort(PCell cells, int count)
{
...
static int gray_check_sort(PCell cells, int count)
{
	PCell p, q;
...
static int gray_move_to(const FT_Vector *to, FT_Raster raster)
{
	TPos x, y;
...
static int gray_line_to(const FT_Vector *to, FT_Raster raster)
{
	gray_render_line((PRaster)raster, UPSCALE(to->x), UPSCALE(to->y));
	return 0;
}


static int gray_conic_to(const FT_Vector *control, const FT_Vector *to, FT_Raster raster)
{
...
static void gray_render_span(int y, int count, const FT_Span *spans, PRaster raster)
{
	unsigned char *p;
...
static void gray_dump_cells(RAS_ARG)
{
	PCell cell, limit;
	int y = -1;
...
static void gray_sweep(RAS_ARG_ const FT_Bitmap *target)
{
	...
	for (;;) {
		...
		if (cur < limit && start->y == cur->y) {
			...
			if (cur->x > x)
				gray_hline(RAS_VAR_ x, y, cover * (ONE_PIXEL * 2), cur->x - x);
		} else {
			/* draw a gray span until the end of the clipping region */
			if (cover && x < ras.max_ex - ras.min_ex)
				gray_hline(RAS_VAR_ x,
						   y,
						   cover * (ONE_PIXEL * 2),
	...
	if (ras.render_span && ras.num_gray_spans > 0)
		ras.render_span(
			ras.span_y, ras.num_gray_spans, ras.gray_spans, ras.render_span_data);
...
/*                                                                       */
static int FT_Outline_Decompose(const FT_Outline *outline,
...
extern int gray_raster_render(PRaster raster, const FT_Raster_Params *params)
{
...
static void grays_init_gamma(PRaster raster)
{
	unsigned int x, a;
...
const FT_Raster_Funcs ft_grays_raster = {FT_GLYPH_FORMAT_OUTLINE,
