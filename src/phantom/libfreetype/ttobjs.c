				 ...
				 TT_GlyphZone zone)
{
	...
	if (FT_NEW_ARRAY(zone->org, maxPoints * 2) ||
		FT_NEW_ARRAY(zone->cur, maxPoints * 2) || FT_NEW_ARRAY(zone->tags, maxPoints) ||
			 ...
			 FT_Parameter *params)
{
	...
	if (face->root.face_flags & FT_FACE_FLAG_SCALABLE) {
		...
		if (!error)
			error = tt_face_load_loca(face, stream) || tt_face_load_cvt(face, stream) ||
	...
	face->unpatented_hinting =
		FT_BOOL(library->debug_hooks[FT_DEBUG_HOOK_UNPATENTED_HINTING] != NULL);
...
tt_size_init(FT_Size ttsize) /* TT_Size */
{
	...
	TT_Set_CodeRange(
		exec, tt_coderange_font, face->font_program, face->font_program_size);
...
static FT_Error Reset_Outline_Size(TT_Size size)
{
	...
	if (metrics->x_ppem >= metrics->y_ppem) {
		...
		size->ttmetrics.y_ratio = FT_MulDiv(metrics->y_ppem, 0x10000L, metrics->x_ppem);
	} else {
		size->ttmetrics.scale = metrics->y_scale;
		size->ttmetrics.ppem = metrics->y_ppem;
		size->ttmetrics.x_ratio = FT_MulDiv(metrics->x_ppem, 0x10000L, metrics->y_ppem);
		size->ttmetrics.y_ratio = 0x10000L;
	}

	/* Compute root ascender, descender, text height, and max_advance */
	metrics->ascender = FT_PIX_ROUND(FT_MulFix(face->root.ascender, metrics->y_scale));
	metrics->descender = FT_PIX_ROUND(FT_MulFix(face->root.descender, metrics->y_scale));
	metrics->height = FT_PIX_ROUND(FT_MulFix(face->root.height, metrics->y_scale));
	metrics->max_advance =
		FT_PIX_ROUND(FT_MulFix(face->root.max_advance_width, metrics->x_scale));
...

	{
		...
		TT_Set_CodeRange(
			exec, tt_coderange_cvt, face->cvt_program, face->cvt_program_size);
...
static FT_Error Reset_SBit_Size(TT_Size size)
{
	...
	error = sfnt->set_sbit_strike(face, metrics->x_ppem, metrics->y_ppem, &strike_index);
	...
	if (!error) {
		...
		/* XXX: Is this correct? */
		sbit_metrics->max_advance = (strike->hori.min_origin_SB + strike->hori.max_width +
