...
/*                                                                       */
static FT_Error Load_SBit_Const_Metrics(TT_SBit_Range range, FT_Stream stream)
{
	FT_Error error;
									  ...
									  FT_Bool load_offsets)
{
	...
	if (FT_NEW_ARRAY(range->glyph_codes, count) || FT_FRAME_ENTER(size))
		goto Exit;
	...
	for (n = 0; n < count; n++) {
		...
		if (load_offsets)
			range->glyph_offsets[n] = (FT_ULong)range->image_offset + FT_GET_USHORT();
	}
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
static FT_Error Load_SBit_Range(TT_SBit_Range range, FT_Stream stream)
{
	FT_Error error;
	FT_Memory memory = stream->memory;
	...
	switch (range->index_format) {
		...
		case 3: /* variable metrics with 2-byte offsets */
		{
			...
			for (n = 0; n < num_glyphs; n++)
				range->glyph_offsets[n] =
					(FT_ULong)(range->image_offset +
							   (large ? FT_GET_ULONG() : FT_GET_USHORT()));
			FT_FRAME_EXIT();
		} break;
...
#define FT_STRUCTURE TT_SBit_StrikeRec
	...
	static const FT_Frame_Field strike_end_fields[] = {/* no FT_FRAME_START */
	...
	if (version != 0x00020000L || num_strikes >= 0x10000L) {
	...
	/* allocate the index ranges for each strike table */
	{
		...
		while (count > 0) {
			...
			while (count2 > 0) {
				...
				range->table_offset = table_base + strike->ranges_offset + FT_GET_ULONG();
				count2--;
				range++;
			}
			...
			while (count2 > 0) {
				/* Read the header */
				if (FT_STREAM_SEEK(range->table_offset) || FT_FRAME_ENTER(8L))
					goto Exit;
	...
	if (face->num_sbit_strikes) {
		...
		for (n = 0; n < face->num_sbit_strikes; n++) {
			...
			/* assume 72dpi */
			bsize->height = (FT_Short)((height * strike->y_ppem + fupem / 2) / fupem);
			bsize->width = (FT_Short)((avg * strike->y_ppem + fupem / 2) / fupem);
						...
						FT_ULong *astrike_index)
{
	...
	for (i = 0; i < face->num_sbit_strikes; i++) {
		if (((FT_UInt)face->sbit_strikes[i].y_ppem == y_ppem) &&
			((x_ppem == 0) || ((FT_UInt)face->sbit_strikes[i].x_ppem == x_ppem))) {
			*astrike_index = i;
			return SFNT_Err_Ok;
		}
	}
	...
	for (; range < range_limit; range++) {
		if (glyph_index >= (FT_UInt)range->first_glyph &&
			...
			switch (range->index_format) {
				...
				case 2:
					*aglyph_offset = range->image_offset + range->image_size * delta;
					break;
				   ...
				   FT_ULong *aglyph_offset)
{
	...
	if (!face->sbit_strikes || (face->num_sbit_strikes <= strike_index))
		goto Fail;
...
/*                                                                       */
FT_LOCAL(FT_Error)
tt_load_sbit_metrics(FT_Stream stream, TT_SBit_Range range, TT_SBit_Metrics metrics)
{
...
/*                                                                       */
static void crop_bitmap(FT_Bitmap *map, TT_SBit_Metrics metrics)
{
	/***********************************************************************/
	/*                                                                     */
	...
	Found_Top:
		...
		if (count > 0) {
			...
			FT_MEM_MOVE(line, line + count * line_len, (rows - count) * line_len);
								 ...
								 FT_Stream stream)
{
	...
	if (x_offset < 0 || x_offset + metrics->width > map->width || y_offset < 0 ||
								...
								FT_Int depth)
{
	...
	/* the array of elements.                                    */
	{
		...
		if (FT_READ_USHORT(num_components) || FT_NEW_ARRAY(components, num_components))
			goto Exit;
		...
		for (; count > 0; count--, comp++) {
			...
			error = find_sbit_range(comp->glyph_code, strike, &elem_range, &elem_offset);
			if (error)
				goto Fail_Memory;
						...
						TT_SBit_MetricsRec *metrics)
{
	...
	error = tt_find_sbit_image(
		face, glyph_index, strike_index, &range, &strike, &glyph_offset);
	if (error)
		goto Exit;
