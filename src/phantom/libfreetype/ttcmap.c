...
const TT_CMap_ClassRec tt_cmap0_class_rec = {{sizeof(TT_CMapRec),
...
FT_CALLBACK_TABLE_DEF
const TT_CMap_ClassRec tt_cmap2_class_rec = {{sizeof(TT_CMapRec),
...
FT_CALLBACK_DEF(FT_Error)
tt_cmap4_init(TT_CMap4 cmap, FT_Byte *table)
{
	FT_Byte *p;
...
static FT_Int tt_cmap4_set_range(TT_CMap4 cmap, FT_UInt range_index)
{
...
static void tt_cmap4_reset(TT_CMap4 cmap, FT_UInt code, FT_UInt range_index)
{
...
tt_cmap4_validate(FT_Byte *table, FT_Validator valid)
{
	...
	if (valid->level >= FT_VALIDATE_PARANOID) {
		...
		if (search_range > num_segs || search_range * 2 < num_segs ||
	...
	/* overlap; check also the offsets                               */
	{
		...
		for (n = 0; n < num_segs; n++) {
			...
			if (offset && offset != 0xFFFFU) {
				...
				/*                                                        */
				if (valid->level >= FT_VALIDATE_PARANOID || n != num_segs - 1 ||
...
FT_CALLBACK_TABLE_DEF
const TT_CMap_ClassRec tt_cmap6_class_rec = {{sizeof(TT_CMapRec),
...
FT_CALLBACK_TABLE_DEF
const TT_CMap_ClassRec tt_cmap8_class_rec = {{sizeof(TT_CMapRec),
...
tt_face_build_cmaps(TT_Face face)
{
	...
	for (; num_cmaps > 0 && p + 8 <= limit; num_cmaps--) {
		...
		if (offset && table + offset + 2 < limit && table + offset >= table) {
			...
			for (; *pclazz; pclazz++) {
				...
				if (clazz->format == format) {
					...
					ft_validator_init(
						FT_VALIDATOR(&valid), cmap, limit, FT_VALIDATE_DEFAULT);
					...
					if (valid.validator.error == 0) {
						...
						((TT_CMap)(face->root.charmaps[face->root.num_charmaps - 1]))
							->unsorted = FT_BOOL(error);
					} else {
