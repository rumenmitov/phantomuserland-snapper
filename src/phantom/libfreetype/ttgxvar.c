...
#define FT_Stream_FTell(stream)        ((stream)->cursor - (stream)->base)
#define FT_Stream_SeekSet(stream, off) ((stream)->cursor = (stream)->base + (off))
...
/*                                                                       */
static FT_UShort *ft_var_readpackedpoints(FT_Stream stream, FT_UInt *point_cnt)
{
	FT_UShort *points;
	FT_Int n;
	FT_Int runcnt;
...
/*                                                                       */
static FT_Short *ft_var_readpackeddeltas(FT_Stream stream, FT_Int delta_cnt)
{
	FT_Short *deltas;
	FT_Int runcnt;
	FT_Int i;
	...
	while (i < delta_cnt) {
		...
		if (runcnt & GX_DT_DELTAS_ARE_ZERO) {
			...
			for (j = 0; j <= (runcnt & GX_DT_DELTA_RUN_COUNT_MASK) && i < delta_cnt; ++j)
				deltas[i++] = 0;
...
static void ft_var_load_avar(TT_Face face)
{
	...
	if (version != 0x00010000L || axisCount != (FT_Long)blend->mmvar->num_axis)
		goto Exit;
	...
	for (i = 0; i < axisCount; ++i, ++segment) {
		...
		for (j = 0; j < segment->pairCount; ++j) {
			segment->correspondence[j].fromCoord = FT_GET_SHORT()
												   << 2; /* convert to Fixed */
			segment->correspondence[j].toCoord = FT_GET_SHORT()
												 << 2; /* convert to Fixed */
		}
	}
...
#define FT_STRUCTURE GX_GVar_Head
	...
	if (blend->tuplecount != 0) {
		if (FT_NEW_ARRAY(blend->tuplecoords, gvar_head.axisCount * blend->tuplecount))
			goto Exit;
...
#define FT_STRUCTURE GX_FVar_Axis
	...
	if (face->blend == NULL) {
		...
		if ((error = face->goto_table(face, TTAG_fvar, stream, &table_len)) != 0)
			goto Exit;
		...
		if (fvar_head.version != (FT_Long)0x00010000L || fvar_head.countSizePairs != 2 ||
			fvar_head.axisSize != 20 ||
			fvar_head.instanceSize != 4 + 4 * fvar_head.axisCount ||
			fvar_head.offsetToData + fvar_head.axisCount * 20U +
					fvar_head.instanceCount * fvar_head.instanceSize >
				table_len) {
			error = TT_Err_Invalid_Table;
			goto Exit;
		}
		...
		face->blend->mmvar_len =
			sizeof(FT_MM_Var) + fvar_head.axisCount * sizeof(FT_Var_Axis) +
		...
		mmvar->num_designs = (FT_UInt)-1; /* meaningless in this context; each glyph */
										  /* may have a different number of designs  */
										  /* (or tuples, as called by Apple)         */
		mmvar->num_namedstyles = fvar_head.instanceCount;
		mmvar->axis = (FT_Var_Axis *)&(mmvar[1]);
		mmvar->namedstyle = (FT_Var_Named_Style *)&(mmvar->axis[fvar_head.axisCount]);

		next_coords = (FT_Fixed *)&(mmvar->namedstyle[fvar_head.instanceCount]);
	...
	if (master != NULL) {
		...
		mmvar->axis = (FT_Var_Axis *)&(mmvar[1]);
		mmvar->namedstyle = (FT_Var_Named_Style *)&(mmvar->axis[mmvar->num_axis]);
		next_coords = (FT_Fixed *)&(mmvar->namedstyle[mmvar->num_namedstyles]);
...
TT_Set_MM_Blend(TT_Face face, FT_UInt num_coords, FT_Fixed *coords)
{
...
#if 1  // dz fixed?
		for (i = 0; i < num_coords && blend->normalizedcoords[i] == coords[i]; ++i) {
			if (i == num_coords)
	...
	FT_MEM_COPY(blend->normalizedcoords, coords, num_coords * sizeof(FT_Fixed));
...
TT_Set_Var_Design(TT_Face face, FT_UInt num_coords, FT_Fixed *coords)
{
	...
	for (i = 0; i < mmvar->num_axis; ++i, ++a) {
		...
		if (coords[i] < a->def) {
			normalized[i] = -FT_MulDiv(coords[i] - a->def, 0x10000L, a->minimum - a->def);
		} else if (a->maximum == a->def)
			normalized[i] = 0;
		else {
			normalized[i] = FT_MulDiv(coords[i] - a->def, 0x10000L, a->maximum - a->def);
		}
	}
		...
		for (i = 0; i < mmvar->num_axis; ++i, ++av) {
			for (j = 1; j < (FT_UInt)av->pairCount; ++j)
				if (normalized[i] < av->correspondence[j].fromCoord) {
					normalized[i] =
						FT_MulDiv(FT_MulDiv(normalized[i] -
...
tt_face_vary_cvt(TT_Face face, FT_Stream stream)
{
	...
	for (i = 0; i < (tupleCount & 0xFFF); ++i) {
		...
		apply = ft_var_apply_tuple(
			blend, (FT_UShort)tupleIndex, tuple_coords, im_start_coords, im_end_coords);
		...
		deltas = ft_var_readpackeddeltas(stream,
										 point_count == 0 ? face->cvt_size : point_count);
		if (localpoints == NULL || deltas == NULL)
			...
			for (j = 0; j < face->cvt_size; ++j)
				face->cvt[j] = (FT_Short)(face->cvt[j] + FT_MulFix(deltas[j], apply));
		}
			...
			for (j = 0; j < point_count; ++j) {
				...
				face->cvt[pindex] =
					(FT_Short)(face->cvt[pindex] + FT_MulFix(deltas[j], apply));
			}
		}
						 ...
						 FT_UInt n_points)
{
	...
	*deltas = delta_xy;
	...
	if (glyph_index >= blend->gv_glyphcnt ||
		blend->glyphoffsets[glyph_index] == blend->glyphoffsets[glyph_index + 1])
	...
	for (i = 0; i < (tupleCount & GX_TC_TUPLE_COUNT_MASK); ++i) {
		...
		deltas_x = ft_var_readpackeddeltas(stream,
										   point_count == 0 ? n_points : point_count);
