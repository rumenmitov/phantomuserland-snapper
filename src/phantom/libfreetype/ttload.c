...
#define FT_STRUCTURE TT_TableRec
	...
	if (num_tables == 0) {
		if (FT_STREAM_SEEK(offset) || FT_STREAM_SKIP(4) || FT_READ_USHORT(num_tables) ||
			FT_STREAM_SKIP(6))
			goto Bad_Format;
	...
	for (nn = 0; nn < num_tables; nn++) {
		...
		if (table.Offset + table.Length > stream->size && table.Tag != glyx_tag &&
			table.Tag != locx_tag)
			goto Bad_Format;
		...
		head_retry:
			 ...
			 */
			if (table.Length < 0x36 || FT_STREAM_SEEK(table.Offset + 12) ||
				FT_READ_ULONG(magic) || magic != 0x5F0F3CF5UL)
				goto Bad_Format;
...
#define FT_STRUCTURE TTC_HeaderRec
		...
		FT_FRAME_START(8), FT_FRAME_LONG(version), FT_FRAME_LONG(count), FT_FRAME_END};
...
tt_face_load_directory(TT_Face face, FT_Stream stream, SFNT_Header sfnt)
{
	...
	for (; entry < limit; entry++) { /* loop through the tables and get all entries */
...
FT_LOCAL_DEF(FT_Error)
tt_face_load_any(
	TT_Face face, FT_ULong tag, FT_Long offset, FT_Byte *buffer, FT_ULong *length)
{
	FT_Error error;
	FT_Stream stream;
	TT_Table table;
...
#define FT_STRUCTURE TT_MaxProfile
	...
	const FT_Frame_Field maxp_fields_extra[] = {FT_FRAME_START(26),
	...
	if (maxProfile->version >= 0x10000L) {
		...
		face->root.internal->max_points =
			(FT_UShort)FT_MAX(maxProfile->maxCompositePoints, maxProfile->maxPoints);

		face->root.internal->max_contours =
			(FT_Short)FT_MAX(maxProfile->maxCompositeContours, maxProfile->maxContours);
...
static FT_Error tt_face_load_metrics(TT_Face face, FT_Stream stream, FT_Bool vertical)
{
	...
	if (num_shorts < 0) {
		...
		error = vertical ? SFNT_Err_Invalid_Vert_Metrics : SFNT_Err_Invalid_Horiz_Metrics;
		goto Exit;
	}

	if (FT_QNEW_ARRAY(*longs, num_longs) || FT_QNEW_ARRAY(*shorts, num_shorts))
		goto Exit;
	...
	/* do we have an inconsistent number of metric values? */
	{
		...
		TT_ShortMetrics *limit = cur + FT_MIN(num_shorts, num_shorts_checked);
...
#define FT_STRUCTURE TT_OS2
	...
	const FT_Frame_Field os2_fields_extra[] = {FT_FRAME_START(8),
											   FT_FRAME_ULONG(ulCodePageRange1),
											   FT_FRAME_ULONG(ulCodePageRange2),
											   FT_FRAME_END};

	const FT_Frame_Field os2_fields_extra2[] = {FT_FRAME_START(10),
...
tt_face_load_gasp(TT_Face face, FT_Stream stream)
{
	...
	if (FT_QNEW_ARRAY(gaspranges, num_ranges) || FT_FRAME_ENTER(num_ranges * 4L))
		goto Exit;
...
tt_face_load_hdmx(TT_Face face, FT_Stream stream)
{
...

	{
		...
		for (; cur < limit; cur++) {
			/* read record */
			if (FT_READ_BYTE(cur->ppem) || FT_READ_BYTE(cur->max_width))
				goto Exit;
