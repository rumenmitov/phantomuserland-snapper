...
static void af_latin_metrics_init_widths(AF_LatinMetrics metrics, FT_Face face)
{
	...
	for (bb = 0; bb < AF_LATIN_BLUE_MAX; bb++) {
		...
		for (; p < limit && *p; p++) {
			...
			/* lies, then see its previous and next points                  */
			{
				...
				round =
					FT_BOOL(FT_CURVE_TAG(glyph->outline.tags[prev]) != FT_CURVE_TAG_ON ||
...
af_latin_hints_compute_segments(AF_GlyphHints hints, AF_Dimension dim)
{
	...
	for (; contour < contour_limit; contour++) {
		...
		if (FT_ABS(last->out_dir) == major_dir && FT_ABS(point->out_dir) == major_dir) {
...
af_latin_hints_compute_edges(AF_GlyphHints hints, AF_Dimension dim)
{
	...
	scale = (dim == AF_DIMENSION_HORZ) ? hints->x_scale : hints->y_scale;

	up_dir = (dim == AF_DIMENSION_HORZ) ? AF_DIR_UP : AF_DIR_RIGHT;
	...
	edge_distance_threshold = FT_MulFix(laxis->edge_distance_threshold, scale);
	if (edge_distance_threshold > 64 / 4)
		edge_distance_threshold = 64 / 4;

	edge_distance_threshold = FT_DivFix(edge_distance_threshold, scale);
	...
	if (has_serifs || !anchor) {
		...
		for (edge = edges; edge < edge_limit; edge++) {
			...
			if (edge->serif)
				...
				edge->pos = anchor->pos + FT_PIX_ROUND(edge->opos - anchor->opos);
			...
			if (edge + 1 < edge_limit && edge[1].flags & AF_EDGE_DONE &&
				edge->pos > edge[1].pos)
				edge->pos = edge[1].pos;
		}
	}
...
FT_CALLBACK_TABLE_DEF const AF_ScriptClassRec af_latin_script_class = {
	AF_SCRIPT_LATIN,
	af_latin_uniranges,
