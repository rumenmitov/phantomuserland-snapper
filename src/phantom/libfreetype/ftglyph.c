...
FT_CALLBACK_DEF(FT_Error)
ft_outline_glyph_init(FT_Glyph outline_glyph, FT_GlyphSlot slot)
{
	...
	error =
		FT_Outline_New(library, source->n_points, source->n_contours, &glyph->outline);
	if (error)
		goto Exit;
...
ft_outline_glyph_copy(FT_Glyph outline_source, FT_Glyph outline_target)
{
	...
	error = FT_Outline_New(
		library, source->outline.n_points, source->outline.n_contours, &target->outline);
	if (!error)
...
FT_CALLBACK_TABLE_DEF
const FT_Glyph_Class ft_outline_glyph_class = {sizeof(FT_OutlineGlyphRec),
				   ...
				   FT_Bool destroy)
{
	...
	error =
		ft_new_glyph(glyph->library, &ft_bitmap_glyph_class, (FT_Glyph *)(void *)&bitmap);
	if (error)
		goto Exit;
