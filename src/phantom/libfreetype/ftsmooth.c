								 ...
								 const FT_Vector *origin)
{
	...
	return ft_smooth_render_generic(
		render, slot, mode, origin, FT_RENDER_MODE_NORMAL, 0, 0);
}
									 ...
									 const FT_Vector *origin)
{
	...
	error =
		ft_smooth_render_generic(render, slot, mode, origin, FT_RENDER_MODE_LCD, 3, 0);
	if (!error)
									   ...
									   const FT_Vector *origin)
{
	...
	error =
		ft_smooth_render_generic(render, slot, mode, origin, FT_RENDER_MODE_LCD_V, 0, 3);
	if (!error)
