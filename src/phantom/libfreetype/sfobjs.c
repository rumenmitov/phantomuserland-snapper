...
static FT_String *tt_name_entry_ascii_from_ucs4(TT_NameEntry entry, FT_Memory memory)
{
	FT_String *string;
	FT_UInt len, code, n;
	FT_Byte *read = (FT_Byte *)entry->string;
...
/*                                                                       */
static FT_String *tt_face_get_name(TT_Face face, FT_UShort nameid)
{
...
#undef LOAD_
#define LOAD_(x) ((error = sfnt->load_##x(face, stream)) != SFNT_Err_Ok)
			   ...
			   FT_Parameter *params)
{
	...
	/* now set up root fields */
	{
		...
		if (psnames_error == SFNT_Err_Ok && face->postscript.FormatType != 0x00030000L)
      ...
      if ( TT_FACE_HAS_KERNING( face ) )
		...
		if (has_outline == TRUE) {
			...
			root->height =
				(FT_Short)(root->ascender - root->descender + face->horizontal.Line_Gap);
