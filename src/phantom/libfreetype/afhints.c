...
FT_LOCAL(FT_Error)
af_axis_hints_new_edge(AF_AxisHints axis, FT_Int fpos, FT_Memory memory, AF_Edge *aedge)
{
	FT_Error error = AF_Err_Ok;
	AF_Edge edge = NULL;
	AF_Edge edges;
...
static void af_glyph_hints_compute_inflections(AF_GlyphHints hints)
{
	...
	for (; contour < contour_limit; contour++) {
		...
		angle_seg = af_angle_atan(end->fx - start->fx, end->fy - start->fy);
		...
		do {
			...
			angle_in = af_angle_atan(start->fx - before->fx, start->fy - before->fy);
		...
		do {
			...
			do {
				...
				angle_out = af_angle_atan(after->fx - end->fx, after->fy - end->fy);
		...
		for (point = points; point < point_limit; point++) {
...

			{
...

				{
					...
					u = before->pos + FT_MulFix(fu - before->fpos, before->scale);
				}
			}
		...
		if (point <= end_point) {
			...
			while (point <= end_point) {
				if (point->flags & touch_flag) {
					...
					af_iup_interp(cur_touched + 1, point - 1, cur_touched, point);
					cur_touched = point;
				}
				point++;
			}
			...
			if (cur_touched == first_touched) {
				...
				/* of the contour                                          */
				af_iup_interp(cur_touched + 1, end_point, cur_touched, first_touched);
