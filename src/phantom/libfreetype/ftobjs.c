...
FT_Load_Glyph(FT_Face face, FT_UInt glyph_index, FT_Int32 load_flags)
{
	...
	if (load_flags & FT_LOAD_NO_RECURSE) {
		...
		load_flags |= FT_LOAD_NO_SCALE | FT_LOAD_NO_HINTING | FT_LOAD_NO_BITMAP |
	...
	autohint = FT_BOOL(
		hinter &&
		!(load_flags & (FT_LOAD_NO_SCALE | FT_LOAD_NO_HINTING | FT_LOAD_NO_AUTOHINT)) &&
		FT_DRIVER_IS_SCALABLE(driver) && FT_DRIVER_USES_OUTLINES(driver));
	if (autohint) {
		if (FT_DRIVER_HAS_HINTER(driver) && !(load_flags & FT_LOAD_FORCE_AUTOHINT))
			autohint = 0;
	}
	...
	if (autohint && !(face->internal->transform_matrix.yy <= 0 ||
		...
		if (FT_HAS_FIXED_SIZES(face) && (load_flags & FT_LOAD_NO_BITMAP) == 0) {
			error = driver->clazz->load_glyph(
				slot, face->size, glyph_index, load_flags | FT_LOAD_SBITS_ONLY);
		...
		error = hinting->load_glyph(
			(FT_AutoHinter)hinter, slot, face->size, glyph_index, load_flags);
	} else {
		error = driver->clazz->load_glyph(slot, face->size, glyph_index, load_flags);
		if (error)
			goto Exit;
...
Load_Ok:
	...
	if ((load_flags & FT_LOAD_LINEAR_DESIGN) == 0 &&
		...
		slot->linearVertAdvance =
			FT_MulDiv(slot->linearVertAdvance, metrics->y_scale, 64);
	}
	...
	if (!error && slot->format != FT_GLYPH_FORMAT_BITMAP &&
		slot->format != FT_GLYPH_FORMAT_COMPOSITE && load_flags & FT_LOAD_RENDER) {
...
static void destroy_face(FT_Memory memory, FT_Face face, FT_Driver driver)
{
	...
	FT_Stream_Free(face->stream, (face->face_flags & FT_FACE_FLAG_EXTERNAL_STREAM) != 0);
						  ...
						  FT_Face *aface)
{
	...
	// FT_TRACE1( ( "depp2\n" ) );
	error = clazz->init_face(stream, face, (FT_Int)face_index, num_params, params);
	if (error)
		goto Fail;
	// FT_TRACE1( ( "depp3\n" ) );
									  ...
									  FT_Face *aface)
{
	...
	error = new_memory_stream(library, base, size, memory_stream_close, &stream);
	if (error) {
		FT_FREE(base);
		return error;
	}
									   ...
									   FT_Face *aface)
{
	...
	return open_face_from_buffer(library, pfb_data, pfb_pos, face_index, "type1", aface);
									   ...
									   FT_Face *aface)
{
	...
	is_cff = rlen > 4 && sfnt_data[0] == 'O' && sfnt_data[1] == 'T' &&
			 sfnt_data[2] == 'T' && sfnt_data[3] == 'O';

	error = open_face_from_buffer(
		library, sfnt_data, rlen, face_index, is_cff ? "cff" : "truetype", aface);
							  ...
							  FT_Face *aface)
{
	...
	error = FT_Raccess_Get_HeaderInfo(
		library, stream, resource_offset, &map_offset, &rdara_pos);
	if (error)
		return error;

	error = FT_Raccess_Get_DataOffsets(library,
									   stream,
									   map_offset,
									   rdara_pos,
									   FT_MAKE_TAG('P', 'O', 'S', 'T'),
									   ...
									   &count);
	if (!error) {
		error = Mac_Read_POST_Resource(
			library, stream, data_offsets, count, face_index, aface);
							...
							FT_Face *aface)
{
	...
	dlen = (header[0x53] << 24) | (header[0x54] << 16) | (header[0x55] << 8) |
		   header[0x56];
#if 0
    rlen = ( header[0x57] << 24 ) |
           ( header[0x58] << 16 ) |
...
#define FT_COMPONENT trace_raccess
	...
	for (i = 0; i < FT_RACCESS_N_RULES; i++) {
		...
		FT_TRACE3(("Try rule %d: %s (offset=%d) ...", i, args2.pathname, offsets[i]));
...
#define FT_COMPONENT trace_objs
	...
	if ((FT_ERROR_BASE(error) == FT_Err_Unknown_File_Format ||
		...
		(args->flags & FT_OPEN_PATHNAME))
		error = load_face_in_embedded_rfork(library, stream, face_index, aface, args);
	return error;
}
			 ...
			 FT_Face *aface)
{
	...
	if ((args->flags & FT_OPEN_DRIVER) && args->driver) {
		...
		if (FT_MODULE_IS_DRIVER(driver)) {
			...
			error = open_face(driver, stream, face_index, num_params, params, &face);
			if (!error)
				goto Success;
		} else
...
FT_Attach_Stream(FT_Face face, FT_Open_Args *parameters)
{
	...
	FT_Stream_Free(stream,
				   (FT_Bool)(parameters->stream && (parameters->flags & FT_OPEN_STREAM)));
...
static void ft_recompute_scaled_metrics(FT_Face face, FT_Size_Metrics *metrics)
{
	...
	metrics->max_advance =
		FT_PIX_ROUND(FT_MulFix(face->max_advance_width, metrics->x_scale));
}
				 ...
				 FT_UInt vert_resolution)
{
	...
	if (clazz->set_char_sizes)
		error = clazz->set_char_sizes(
			face->size, char_width, char_height, horz_resolution, vert_resolution);
	return error;
}
...
FT_Set_Pixel_Sizes(FT_Face face, FT_UInt pixel_width, FT_UInt pixel_height)
{
	...
	if (clazz->set_pixel_sizes)
		error = clazz->set_pixel_sizes(face->size, pixel_width, pixel_height);
			...
			if (kern_mode != FT_KERNING_UNSCALED) {
				...
				if (kern_mode != FT_KERNING_UNFITTED) {
					...
					if (face->size->metrics.x_ppem < 25)
						akerning->x =
							FT_MulDiv(akerning->x, face->size->metrics.x_ppem, 25);
					if (face->size->metrics.y_ppem < 25)
						akerning->y =
							FT_MulDiv(akerning->y, face->size->metrics.y_ppem, 25);
	...
	if (cmap) {
		...
		for (i = 0; i < face->num_charmaps; i++) {
			if ((FT_CMap)face->charmaps[i] == cmap) {
				...
				if (FT_RENEW_ARRAY(
						face->charmaps, face->num_charmaps, face->num_charmaps - 1))
					return;
...
FT_CMap_New(FT_CMap_Class clazz, FT_Pointer init_data, FT_CharMap charmap, FT_CMap *acmap)
{
	...
	if (!FT_ALLOC(cmap, clazz->size)) {
		...
		if (FT_RENEW_ARRAY(face->charmaps, face->num_charmaps, face->num_charmaps + 1))
			goto Fail;
...
FT_EXPORT_DEF(FT_Error)
FT_Load_Sfnt_Table(
	FT_Face face, FT_ULong tag, FT_Long offset, FT_Byte *buffer, FT_ULong *length)
{
	FT_Service_SFNT_Table service;
...
FT_Set_Debug_Hook(FT_Library library, FT_UInt hook_index, FT_DebugHook_Func debug_hook)
{
	if (library && debug_hook &&
		hook_index < (sizeof(library->debug_hooks) / sizeof(void *)))
