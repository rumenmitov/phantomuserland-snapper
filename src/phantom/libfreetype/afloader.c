								 ...
								 FT_UInt depth)
{
	...
	switch (slot->format) {
		case FT_GLYPH_FORMAT_OUTLINE:
			...
			loader->pp2.x = FT_MulFix(slot->metrics.horiAdvance, hints->x_scale) +
							hints->x_delta;
			...
			metrics->clazz->script_hints_apply(hints, &gloader->current.outline, metrics);
		...
		case FT_GLYPH_FORMAT_COMPOSITE: {
			...
			FT_ARRAY_COPY(gloader->current.subglyphs, slot->subglyphs, num_subglyphs);
			...
			for (nn = 0; nn < num_subglyphs; nn++) {
				...
				error = af_loader_load_g(
					loader, scaler, subglyph->index, load_flags, depth + 1);
				if (error)
					goto Exit;
				...
				if (subglyph->flags &
					(FT_SUBGLYPH_FLAG_SCALE | FT_SUBGLYPH_FLAG_XY_SCALE |
