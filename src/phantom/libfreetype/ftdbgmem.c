...
static const FT_UInt ft_mem_primes[] = {
	7,      11,      19,      37,      73,      109,     163,     251,      367,
	557,    823,     1237,    1861,    2777,    4177,    6247,    9371,     14057,
	21089,  31627,   47431,   71143,   106721,  160073,  240101,  360163,   540217,
...
static FT_Pointer ft_mem_table_alloc(FT_MemTable table, FT_Long size)
{
...
static void ft_mem_table_free(FT_MemTable table, FT_Pointer block)
{
	FT_Memory memory = table->memory;
...
static void ft_mem_table_resize(FT_MemTable table)
{
	...
	if (new_size != table->size) {
		...
		new_buckets = (FT_MemNode *)ft_mem_table_alloc(table,
...
static FT_MemTable ft_mem_table_new(FT_Memory memory)
{
	...
	table->buckets = (FT_MemNode *)memory->alloc(memory,
...
static void ft_mem_table_destroy(FT_MemTable table)
{
	...
	if (table) {
		...
		ph_printf("FreeType: total memory allocations = %ld\n", table->alloc_total);
		ph_printf("FreeType: maximum memory footprint = %ld\n", table->alloc_max);
...
static FT_MemNode *ft_mem_table_get_nodep(FT_MemTable table, FT_Byte *address)
{
	FT_ULong hash;
...
static FT_MemSource ft_mem_table_get_source(FT_MemTable table)
{
	...
	for (;;) {
		...
		if (node->file_name == table->file_name && node->line_no == table->line_no)
			goto Exit;
	...
	if (node == NULL)
		ft_mem_debug_panic("not enough memory to perform memory debugging\n");
...
static void ft_mem_table_set(FT_MemTable table, FT_Byte *address, FT_ULong size)
{
	FT_MemNode *pnode, node;
	...
	if (table) {
		...
		if (table->nodes * 3 < table->size || table->size * 3 < table->nodes)
			ft_mem_table_resize(table);
	}
}


static void ft_mem_table_remove(FT_MemTable table, FT_Byte *address)
{
	if (table) {
		FT_MemNode *pnode, node;
...
extern FT_Pointer ft_mem_debug_alloc(FT_Memory memory, FT_Long size)
{
	...
	if (table->bound_count && table->alloc_count >= table->alloc_count_max)
		return NULL;
...
extern void ft_mem_debug_free(FT_Memory memory, FT_Pointer block)
{
	FT_MemTable table = (FT_MemTable)memory->user;
...
FT_Alloc_Debug(
	FT_Memory memory, FT_Long size, void **P, const char *file_name, FT_Long line_no)
{
	FT_MemTable table = (FT_MemTable)memory->user;
...
static int ft_mem_source_compare(const void *p1, const void *p2)
{
...
extern void FT_DumpMemory(FT_Memory memory)
{
	...
	if (table) {
		...
		sources = (FT_MemSource *)ft_mem_table_alloc(table, sizeof(*sources) * count);
