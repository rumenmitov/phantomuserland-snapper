...
static void tt_face_get_metrics(
	TT_Face face, FT_Bool vertical, FT_UInt idx, FT_Short *abearing, FT_UShort *aadvance)
{
...
/*                                                                       */
static void Get_HMetrics(
	TT_Face face, FT_UInt idx, FT_Bool check, FT_Short *lsb, FT_UShort *aw)
{
...
/*                                                                       */
static FT_Byte *Get_Advance_WidthPtr(TT_Face face, FT_Int ppem, FT_UInt gindex)
{
#ifdef FT_OPTIMIZE_MEMORY
...
/*                                                                       */
static void Get_VMetrics(
	TT_Face face, FT_UInt idx, FT_Bool check, FT_Short *tsb, FT_UShort *ah)
{
	FT_UNUSED(check);
...
#define cur_to_org(n, zone) FT_ARRAY_COPY((zone)->org, (zone)->cur, (n))

#define org_to_cur(n, zone) FT_ARRAY_COPY((zone)->cur, (zone)->org, (n))
...
/*    interpretation.                                                    */
/*                                                                       */
static FT_Error TT_Process_Simple_Glyph(TT_Loader load, FT_Bool debug)
{
	FT_GlyphLoader gloader = load->gloader;
	...
	if (((TT_Face)load->face)->doblend) {
		...
		error = TT_Vary_Get_Glyph_Deltas(
			(TT_Face)(load->face), load->glyph_index, &deltas, n_points);
	...
	if (IS_HINTED(load->load_flags)) {
		...
		if (n_ins > 0) {
			error = TT_Set_CodeRange(
				load->exec, tt_coderange_glyph, load->exec->glyphIns, n_ins);
			if (error)
				goto Exit;
									...
									FT_UInt recurse_count)
{
	...
	if (face->root.internal->incremental_interface) {
		error = face->root.internal->incremental_interface->funcs->get_glyph_data(
			face->root.internal->incremental_interface->object, glyph_index, &glyph_data);
		if (error)
			goto Exit;
	...
	if (count == 0) {
		...
		if (((TT_Face)(loader->face))->doblend) {
			...
			error = TT_Vary_Get_Glyph_Deltas(
				(TT_Face)(loader->face), glyph_index, &deltas, 4);
			if (error)
				goto Exit;
	...
	if (contours_count >= 0) {
...

		{
			...
			error = TT_Process_Simple_Glyph(loader, (FT_Bool)(size && size->debug));
		}
	...
	/* otherwise, load a composite! */
		...
		if (face->doblend) {
			...
			if ((error = TT_Vary_Get_Glyph_Deltas(
					 face, glyph_index, &deltas, gloader->current.num_subglyphs + 4)) !=
				0)
				goto Exit;
		...
		/* Now, read each subglyph independently. */
		{
			...
			for (n = 0; n < (FT_Int)num_subglyphs; n++) {
				...
				error = load_truetype_glyph(loader, subglyph->index, recurse_count + 1);
				if (error)
					goto Fail;
				...
				if (subglyph->flags &
					(WE_HAVE_A_SCALE | WE_HAVE_AN_XY_SCALE | WE_HAVE_A_2X2)) {
  ...
  /*                                                                       */
              ...
              y = FT_MulFix( y, n );
						...
						FT_Fixed mac_xscale = FT_SqrtFixed(
							FT_MulFix(subglyph->transform.xx, subglyph->transform.xx) +
							FT_MulFix(subglyph->transform.xy, subglyph->transform.xy));
						FT_Fixed mac_yscale = FT_SqrtFixed(
							FT_MulFix(subglyph->transform.yy, subglyph->transform.yy) +
			...
			if (num_subglyphs > 0 && loader->exec && ins_pos > 0 &&
				...
				if (FT_STREAM_SEEK(ins_pos) || FT_READ_USHORT(n_ins))
					goto Fail;
				...
				error = TT_Set_CodeRange(exec, tt_coderange_glyph, exec->glyphIns, n_ins);
				if (error)
					goto Fail;
...
Exit:
	...
	if (glyph_data_loaded)
		face->root.internal->incremental_interface->funcs->free_glyph_data(
			face->root.internal->incremental_interface->object, &glyph_data);
#endif
...
static FT_Error compute_glyph_metrics(TT_Loader loader, FT_UInt glyph_index)
{
...

	{
		...
		/* scale the metrics */
		if (!(loader->load_flags & FT_LOAD_NO_SCALE)) {
			top = FT_MulFix(top_bearing + loader->bbox.yMax, y_scale) - bbox.yMax;
	...
	if (!face->postscript.isFixedPitch && size && IS_HINTED(loader->load_flags)) {
		FT_Byte *widthp =
			Get_Advance_WidthPtr(face, size->root.metrics.x_ppem, glyph_index);
...
TT_Load_Glyph(TT_Size size, TT_GlyphSlot glyph, FT_UInt glyph_index, FT_Int32 load_flags)
{
	...
	if (!size || (load_flags & FT_LOAD_NO_SCALE) || (load_flags & FT_LOAD_NO_RECURSE)) {
		size = NULL;
	...
	if (size && size->strike_index != 0xFFFFU && sfnt->load_sbits &&
	...
	if (size) {
		...
		loader.exec->pedantic_hinting = FT_BOOL(load_flags & FT_LOAD_PEDANTIC);
