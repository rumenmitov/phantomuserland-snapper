						   ...
						   FT_Long *count)
{
	...
	for (i = 0; i < cnt; ++i) {
		if (FT_READ_LONG(tag_internal) || FT_READ_USHORT(subcnt) || FT_READ_USHORT(rpos))
			return error;
				 ...
				 FT_Error *errors)
{
	...
	for (i = 0; i < FT_RACCESS_N_RULES; i++) {
		...
		errors[i] =
			(funcs[i])(library, stream, base_name, &(new_names[i]), &(offsets[i]));
	}
										   ...
										   FT_Long *result_offset)
{
	...
	*result_file_name = NULL;
	return raccess_guess_apple_generic(
		library, stream, base_file_name, magic, result_offset);
}
												...
												FT_Long *result_offset)
{
	...
	error = raccess_guess_linux_double_from_file_name(library, newpath, result_offset);
	if (!error)
											 ...
											 FT_Long *result_offset)
{
	...
	if (FT_ALLOC(newpath, ft_strlen(base_file_name) + ft_strlen("/rsrc") + 1))
		return error;
											...
											FT_Long *result_offset)
{
	...
	for (i = 0; i < n_of_entries; i++) {
		...
		if (entry_id == resource_fork_entry_id) {
			if (FT_READ_LONG(entry_offset) || FT_READ_LONG(entry_length))
				continue;
														  ...
														  FT_Long *result_offset)
{
	...
	error =
		raccess_guess_apple_double(library, stream2, file_name, &nouse, result_offset);
