...
void ctl_scroll_bar_paint(window_handle_t w, control_t *cc)
{
...

	{
		...
		drv_video_bitmap_t *bg_m = alt_bg ? &scrollbar_bed_middle_bmp
										  : &scrollbar_bed_middle_narrow_bmp;
		...
		if (bg_mid_w < 0)
			bg_mid_w = 0;  // wrong - too small

		w_draw_bitmap(w, cc->r.x, cc->r.y, &scrollbar_bed_left_bmp);
		w_draw_bitmap(
			w, cc->r.x + bg_lside_w + bg_mid_w, cc->r.y, &scrollbar_bed_right_bmp);

		// Easy way - just replicate one column of bg
		// w_replicate_hor( w, cc->r.x + bg_lside_w, cc->r.y, bg_mid_w, bg_m->pixel,
		// bg_m->ysize );
	...
	if ((cc->value >= 0) && (cc->value_width > 0)) {
		...
		drv_video_bitmap_t *fg_l = alt_fg ? &scrollbar_orange_bar_left_bmp
										  : &scrollbar_green_bar_left_bmp;
		drv_video_bitmap_t *fg_r = alt_fg ? &scrollbar_orange_bar_right_bmp
										  : &scrollbar_green_bar_right_bmp;
		drv_video_bitmap_t *fg_m = alt_fg ? &scrollbar_orange_bar_middle_bmp
										  : &scrollbar_green_bar_middle_bmp;

		int fg_width = cc->r.xsize - fg_xgap * 2;
		int fg_lside_w = fg_l->xsize;
		int fg_rside_w = fg_r->xsize;
		int fg_mid_w_max = fg_width - fg_lside_w - fg_rside_w +
						   1;  // dunno why +1, but it fits
		...
		w_draw_bitmap(w,
					  cc->r.x + fg_xshift + fg_lside_w + fg_mid_w_actual + pos_shift,
					  cc->r.y + fg_yshift,
					  fg_r);
		...
		w_replicate_hor(w,
						cc->r.x + fg_xshift + fg_lside_w + pos_shift,
						cc->r.y + fg_yshift,
						fg_mid_w_actual,
						fg_m->pixel,
						fg_m->ysize);
		...
		if ((!alt_fg) && !(cc->flags & (CONTROL_FLAG_READONLY | CONTROL_FLAG_NOFOCUS)))
			w_draw_blend_bitmap(w,
								cc->r.x + fg_xshift + pos_shift + 3,
								cc->r.y + fg_yshift + 13,
								&scroll_bar_green_handle_bmp);
	}
