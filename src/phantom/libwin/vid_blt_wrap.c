...
void vid_bitblt_forw(const struct rgba_t *from,
					 int xpos,
					 int ypos,
					 int xsize,
					 int ysize,
...
void vid_bitblt_rev(const struct rgba_t *from,
					int xpos,
					int ypos,
					int xsize,
					int ysize,
...
void vid_win_winblt(const window_handle_t from, int xpos, int ypos, zbuf_t zpos)
{
	...
	vid_bitblt_worker(
		from->r_pixel, xpos, ypos, from->xsize, from->ysize, 0, zpos, flags);
}
...
void vid_win_winblt_rev(const window_handle_t from, int xpos, int ypos, zbuf_t zpos)
{
	...
	win2blt_flags(&flags, from);
	vid_bitblt_worker(
		from->r_pixel, xpos, ypos, from->xsize, from->ysize, 1, zpos, flags);
}
#endif
