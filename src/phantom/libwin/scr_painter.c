 ...
 * - if window is opaque, just paint its part and subtract painted rectangle from paint
 *list
 * - if window is transparent, add it to paintback window list, add currently painted
 *rectangle to paintback rect list
 *
 **/
...
// 2D accelerator use
//
// Take same mutex as painter - vmware driver crashes if we access it from different
// threads
//
