...
{
	return video_drv->xsize;
}
int scr_get_ysize(void)
{
	return video_drv->ysize;
}
int scr_get_bpp(void)
{
	return video_drv->bpp;
}


// movers. default to 24bpp
// void rgba2rgba_zbmove( struct rgba_t *dest, const struct rgba_t *src, zbuf_t *zb, int
// nelem, zbuf_t zpos )
void (*bit_zbmover_to_screen)(void *dest,
							  const struct rgba_t *src,
							  zbuf_t *zb,
							  int nelem,
							  zbuf_t zpos) = (void *)rgba2rgb_zbmove;
void (*bit_mover_to_screen)(void *dest,
							const struct rgba_t *src,
							int nelem) = (void *)rgba2rgb_move;
void (*bit_mover_to_screen_noalpha)(void *dest,
									const struct rgba_t *src,
									int nelem) = (void *)rgba2rgb_move_noalpha;

void (*bit_mover_from_screen)(struct rgba_t *dest,
							  void *src,
							  int nelem) = (void *)rgb2rgba_move;
