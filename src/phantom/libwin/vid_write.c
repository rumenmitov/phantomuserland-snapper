...
{
	vid_bitblt_part(
		from, xsize, ysize, 0, 0, xpos, ypos, xsize, ysize, reverse, zpos, flags);
}
					   ...
					   u_int32_t flags)
{
	...
	if (reverse) {
		...
		for (; sline < yafter; sline++, wline++) {
			// Screen start pos in line
			char *s_start = DRV_VIDEO_FORWARD_LINESTART(sline) +
							xpos * bit_mover_byte_step;
			// Window start pos in line
			const struct rgba_t *w_start = from + ((wline * xsize) + xshift);
					 ...
					 u_int32_t flags)
{
	...
	if (reverse) {
		...
		for (; sline < dst_yafter; sline++, wline++) {
			...
			zbuf_t *zb =
				zbuf + ((video_drv->xsize * ((video_drv->ysize - 1) - sline)) + dst_xpos);
			if (noalpha)
