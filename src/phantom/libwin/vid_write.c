...
void vid_bitblt_worker(const struct rgba_t *from, int xpos, int ypos, int xsize,
  ...
  vid_bitblt_part(from, xsize, ysize, 0, 0, xpos, ypos, xsize, ysize, reverse,
                  zpos, flags);
}
...
void vid_bitblt_worker(const struct rgba_t *from, int xpos, int ypos, int xsize,
  ...
  if (xafter <= 0 || yafter <= 0 || xpos >= video_drv->xsize ||
      ypos >= video_drv->ysize)
  ...
  // char *lowest_line = ((drv_video_screen.ysize -1) - ypos) +
  // drv_video_screen.screen;
  ...
  if (reverse) {
    ...
    for (; sline < yafter; sline++, wline++) {
      ...
      char *s_start =
          DRV_VIDEO_REVERSE_LINESTART(sline) + xpos * bit_mover_byte_step;
      // Window start pos in line
      const struct rgba_t *w_start = from + ((wline * xsize) + xshift);

      zbuf_t *zb =
          zbuf + ((video_drv->xsize * ((video_drv->ysize - 1) - sline)) + xpos);
      // ZBUF_TOP is a special value for mouse painting. XXX hack!
      if (zpos == ZBUF_TOP)
        bit_mover_to_screen((void *)s_start, w_start, xlen);
      else
        bit_zbmover_to_screen((void *)s_start, w_start, zb, xlen, zpos);
    }
  } else {
    for (; sline < yafter; sline++, wline++) {
      // Screen start pos in line
      char *s_start =
          DRV_VIDEO_FORWARD_LINESTART(sline) + xpos * bit_mover_byte_step;
      // Window start pos in line
      const struct rgba_t *w_start = from + ((wline * xsize) + xshift);
...
void vid_bitblt_part(const rgba_t *from, int src_xsize, int src_ysize,
  ...
  if (reverse) {
    ...
    for (; sline < dst_yafter; sline++, wline++) {
      // Screen start pos in line
      char *s_start = DRV_VIDEO_REVERSE_LINESTART(sline) +
                      (bit_mover_byte_step ? (dst_xpos * bit_mover_byte_step)
                                           : (dst_xpos / 4));
      // Window start pos in line
      ...
      zbuf_t *zb =
          zbuf +
          ((video_drv->xsize * ((video_drv->ysize - 1) - sline)) + dst_xpos);
      if (noalpha)
        bit_mover_to_screen_noalpha((void *)s_start, w_start, xsize);
      else if (nozbuf)
        bit_mover_to_screen((void *)s_start, w_start, xsize);
      else
        bit_zbmover_to_screen((void *)s_start, w_start, zb, xsize, zpos);
    }
  } else {
    for (; sline < dst_yafter; sline++, wline++) {
      // Screen start pos in line
      char *s_start =
          DRV_VIDEO_FORWARD_LINESTART(sline) + dst_xpos * bit_mover_byte_step;
      // Window start pos in line
      const struct rgba_t *w_start = from + ((wline * src_xsize) + src_xpos);
