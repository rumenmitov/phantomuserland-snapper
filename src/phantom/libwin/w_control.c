...
static void paint_or_replicate(window_handle_t win,
							   control_t *cc,
							   drv_video_bitmap_t *img)
{
	if (img->xsize == 1)
...
void ctl_paint_notification(window_handle_t win, control_t *cc)
{
	...
	w_font_draw_string(win,
					   &drv_video_kolibri1_font,
					   counter,
					   black90,
					   COLOR_TRANSPARENT,
					   cc->r.x + x + 10,
...
int ctl_paint_icon(window_handle_t win, control_t *cc)
{
	...
	if (cc->flags & CONTROL_FLAG_TEXT_RIGHT) {
		...
		// add y to x and subtract from y to have equal dist
		w_draw_bitmap(win,
					  cc->r.x + cc->r.xsize - y - cc->icon_image->xsize,
					  cc->r.y + y,
					  cc->icon_image);
...
void ctl_paint_text(window_handle_t win, control_t *cc, int shift)
{
	...
	w_ttfont_draw_string(win,
						 decorations_title_font,
						 cc->text,
						 cc->fg_color,
						 cc->r.x + t_ypos + 2 + shift,
						 cc->r.y + t_ypos);  // +2?
}
...
void w_paint_control(window_handle_t w, control_t *cc)
{
	if (!(cc->flags & (CONTROL_FLAG_DISABLED | CONTROL_FLAG_NOPAINT))) {
		switch (cc->type) {
			case ct_label:
				ctl_label_paint(w, cc);
				break;
			case ct_button:
				ctl_button_paint(w, cc);
				break;
			case ct_menuitem:
				ctl_menu_item_paint(w, cc);
				break;
			case ct_text:
				ctl_text_field_paint(w, cc);
				break;
			case ct_scrollbar:
				ctl_scroll_bar_paint(w, cc);
				break;
...
static errno_t do_paint_changed_control(pool_t *pool,
										void *el,
...
void w_control_action(window_handle_t w, control_t *cc, ui_event_t *ie)
{
	...
	if (cc->c_child)
		w_control_set_visible(cc->w_child, cc->c_child, isPressed);
	else if (cc->w_child) {
		// if(isPressed) w_to_top( cc->w_child ); // recursive mutex lock - TODO need
		// event
...
static errno_t do_event_to_control(pool_t *pool,
								   void *el,
								   pool_handle_t handle,
								   void *arg)
{
	...
	if (cc->flags &
		(CONTROL_FLAG_DISABLED | CONTROL_FLAG_NOFOCUS | CONTROL_FLAG_READONLY))
		return 0;  // Event is not seen by me

	if ((env->e.type == UI_EVENT_TYPE_MOUSE) &&
		point_in_rect(env->e.rel_x, env->e.rel_y, &cc->r)) {
		if (!cc->focused) {
		...
		if ((env->e.m.clicked == UI_MOUSE_BTN_RIGHT)) {
			...
			if (cc->context_menu) {
				...
				ev_q_put_win(cc->r.x + cc->r.xsize,
							 cc->r.y + cc->r.ysize,
							 UI_EVENT_WIN_MOVE,
							 cc->context_menu);
	...
	switch (cc->type) {
		...
		case ct_text:
			return ctl_text_field_events(cc, env);
			break;
		case ct_scrollbar:
			return ctl_scroll_bar_events(cc, env);
			break;
 ...
 *
 **/
void w_reset_controls(
	window_handle_t w)  // focus lost, mouse off window - make sure all buttons are off
{
	if (w->controls == 0)
		return;
...
static errno_t do_pass_focus(pool_t *pool, void *el, pool_handle_t handle, void *arg)
{
	...
	if (cc->flags & CONTROL_FLAG_NOFOCUS)
		return 0;
 ...
 *
 * \return True (non-zero) if event is consumed, and should not be delivered to containing
 *window and other controls.
 *
 **/
int w_event_to_controls(window_handle_t w, ui_event_t *e)
{
	...
	// LOG_FLOW( 1, "control event check @ %d.%d buttons %x", e->rel_x, e->rel_y,
	// e->m.buttons );
	struct foreach_control_param env;
	bzero(&env, sizeof(env));
	env.e = *e;
	...
	if ((e->type == UI_EVENT_TYPE_KEY) && (e->k.vk == KEY_TAB) &&
		UI_MOD_DOWN(e->modifiers)) {
