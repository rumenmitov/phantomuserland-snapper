 ...
 *
 * Z buffer value 0 is lowest (most far) position. It is supposed to be used by
 *background.
 *
 **/
...
void scr_zbuf_reset_win(window_handle_t w)
{
	...
	scr_zbuf_reset_square(
		w->x - bw, w->y - bw, w->xsize + bw * 2, w->ysize + bw * 2 + 31);
#else
...
void scr_zbuf_set_win_z(window_handle_t w)
{
	// TODO XXX HACK alert - hardcoded decorations size
	const int bw = 3;
	scr_zbuf_reset_square_z(
		w->x - bw, w->y - bw, w->xsize + bw * 2, w->ysize + bw * 2 + 21, w->z);
}
...
static void do_scr_zbuf_reset_square_z(
	zbuf_t *target, int x, int y, int xsize, int ysize, zbuf_t zpos)
{
	// LOG_FLOW( 2, "@ %d/%d, sz %d x %d, z %d", x, y, xsize, ysize, zpos );
	// lprintf( "zb reset @ %d/%d, sz %d x %d, z %d\n", x, y, xsize, ysize, zpos );
