			...
			switch (e->extra) {
				...
				case WBUTTON_SYS_ROLLUP:
					if (mainw->state & WSTATE_WIN_ROLLEDUP) {
						...
						if (mainw->w_decor)
							mainw->w_decor->state &= ~WSTATE_WIN_ROLLEDUP;
						goto redecorate;
					} else {
						// ph_printf("roll up\n" );
						mainw->state |= WSTATE_WIN_ROLLEDUP;
...
{
	// ph_printf("titleMouseEventProcessor buttons %x, %d-%d", e->m.buttons, e->abs_x,
	// e->abs_y);
