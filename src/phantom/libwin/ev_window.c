...
void w_event_deliver_thread(void)
{
	...
	while (1) {
		// if(phantom_virtual_machine_snap_request) // Check if we need release access to
		// persistent memory
		//     phantom_thread_wait_4_snap();

		hal_sem_acquire(
			&we_sem);  // TODO need some 'acquire_all' method to eat all releases
...
static void select_event_target(struct ui_event *e)
{
	...
	// If focused window is being dragged - don't try to change focus - TODO global
	// push_focus_lock/push_focus_lock?
...
ret:
	...
	if (later_lost)
		ev_q_put_win(later_x, later_y, UI_EVENT_WIN_LOST_FOCUS, later_lost);
	if (later_gain)
		ev_q_put_win(later_x, later_y, UI_EVENT_WIN_GOT_FOCUS, later_gain);
