...
// be called out of spinlock, trapno 0: Divide error, error 00000000 EIP 1f5ae0:
// _gray_render_line
...
// XXX : __inline__ causes undefined reference when using -O0 option
// __inline__
int w_font_draw_char(window_handle_t win,
					 ...
					 int y)
{
	...
	int bpcx =
		1 +
		((font->xsize - 1) /
		 8);  // Bytes per Character for X coord (for example, 2 bytes for 16 pixels wide)

	if (xafter <= 0 || yafter <= 0 || x >= win->xsize || y >= win->ysize)
		return -1;  // Totally clipped off

	if (x < 0 || y < 0 || xafter >= win->xsize || yafter >= win->ysize)
	...
	for (; yc < yafter; yc++, fyc++) {
		...
		for (; bc >= 0; bc--) {
			...
			int xe =
				(bc == 0)
					? (font->xsize % 8)
					: 8;  // bc==1 because we decrement it later, and we need last pass
...
static void set_ansi_color(
	int v, int *bright, rgba_t *fg, rgba_t *bg, rgba_t def_fg, rgba_t def_bg)
{
	// Special cases
	switch (v) {
		case 0:  // defaults
...
void w_font_tty_string(window_handle_t win,
					   const drv_video_font_t *font,
					   const char *s,
					   const rgba_t _color,
					   const rgba_t _back,
