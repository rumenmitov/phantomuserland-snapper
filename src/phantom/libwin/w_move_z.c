...
void w_to_top(drv_video_window_t *w)
{
	...
	if ((w->flags & WFLAG_WIN_ONTOP) || (queue_empty(&allwindows))) {
		// Trivial case - our window has 'ontop' flag too and can be topmost, or queue is
		// empty
		if (w->w_decor)
			queue_enter(&allwindows, w->w_decor, drv_video_window_t *, chain);
		if (w->w_title)
			queue_enter(&allwindows, w->w_title, drv_video_window_t *, chain);
		queue_enter(&allwindows, w, drv_video_window_t *, chain);
	} else {
		// Our window has no 'ontop' flag and must go under topmost ones and queue is not
		// empty
		...
		queue_iterate_back(&allwindows, iw, drv_video_window_t *, chain)
		{
			if (!(iw->flags & WFLAG_WIN_ONTOP)) {
				...
				if (w->w_decor)
					queue_enter_after(
						&allwindows, iw, w->w_decor, drv_video_window_t *, chain);
				if (w->w_title)
					queue_enter_after(
						&allwindows, iw, w->w_title, drv_video_window_t *, chain);
				goto inserted;
			}
		}
