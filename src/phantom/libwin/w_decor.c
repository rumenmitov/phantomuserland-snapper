...
#define border_0_color                          \
	{                                           \
		.r = 210, .g = 203, .b = 188, .a = 0xFF \
	}  // bgr!
#define border_1_color                          \
	{                                           \
		.r = 183, .g = 171, .b = 146, .a = 0xFF \
	}
#define border_2_color                          \
	{                                           \
		.r = 210, .g = 203, .b = 188, .a = 0xFF \
	}
...
#endif
rgba_t title_back_color_focus = {
	.r = 213, .g = 172, .b = 101, .a = 0xFF};  // { 122, 230, 251, 0xFF };
rgba_t title_back_color_nofocus = {
	214, 227, 231, 0xFF};  // border_1_color;  // { 122, 251, 195, 0xFF };
...
void window_basic_border(window_handle_t dest,
						 const rgba_t *src,
						 int srcSize,
						 int isTitle)
{
	...
	w_replicate_ver(
		dest, dest->xsize - srcSize, stepOff, dest->ysize - (2 * stepOff), src, srcSize);

	w_replicate_hor(dest, stepOff, 0, dest->xsize - (2 * stepOff), src, srcSize);
	w_replicate_hor(
		dest, stepOff, dest->ysize - srcSize, dest->xsize - (2 * stepOff), src, srcSize);
	...
	if (isTitle)
		w_draw_bitmap(dest, r.x, r.y, &win_decor_ljoin_bmp);
	else
		w_draw_bitmap(dest, r.x, r.y, &win_decor_dl_bmp);
	...
	// w_fill_rect( dest, brdr[1], r );
	if (isTitle)
		w_draw_bitmap(dest, r.x, r.y, &win_decor_rjoin_bmp);
	else
		w_draw_bitmap(dest, r.x, r.y, &win_decor_dr_bmp);
...
void win_make_decorations(drv_video_window_t *w)
{
	...
	if (0 == w->w_decor) {
...
#endif
		drv_video_window_t *wdecor =
			private_drv_video_window_create(w->xsize + bordr_size * 2, dysize);

		// wdecor->flags |= WFLAG_WIN_NOTINALL; // On destroy don't try to remove from
		// allwindows
		wdecor->w_owner = w;
		w->w_decor = wdecor;
		...
		w_add_button(
			w->w_decor, WBUTTON_SYS_CLOSE, 5, bmp_y, &close_bmp, &close_pressed_bmp, 0);
#endif
	...
	if (w->w_title == 0) {
		drv_video_window_t *wtitle = private_drv_video_window_create(
			w->xsize + bordr_size * 2, title_size + bordr_size * 2);

		// wtitle->flags |= WFLAG_WIN_NOTINALL; // On destroy don't try to remove from
		// allwindows
		...
		// close button
		w_add_button(w->w_title,
					 WBUTTON_SYS_CLOSE,
					 bxp,
					 3,
					 &vanilla_cream_close_bmp,
					 &vanilla_cream_close_pressed_bmp,
					 0);
		bxp -= bwidth + 0;
		// roll up button
		w_add_button(w->w_title,
					 ...
					 bxp,
					 3,
					 &vanilla_cream_rollup_bmp,
					 &vanilla_cream_rollup_pressed_bmp,
					 0);
	}
	...
	if (w->w_title->state & WSTATE_WIN_FOCUSED)
		focused = 1;  // w_title can't be null here
...
#if CONF_TRUETYPE
	w_ttfont_draw_string(w->w_title,
						 decorations_title_font,
						 w->title,
						 COLOR_BLACK,  // COLOR_TRANSPARENT,
						 bordr_size + 7,
						 bordr_size + 7);
#else
	w_font_draw_string(w->w_title,
					   &drv_video_8x16cou_font,
					   w->title,
					   COLOR_BLACK,
					   COLOR_TRANSPARENT,
					   bordr_size + 3,
...
/*
	...
	w_draw_bitmap( w->w_decor, w->w_decor->xsize - close_bmp.xsize - 5, bmp_y+1,
   &close_bmp ); w_draw_bitmap( w->w_decor, w->w_decor->xsize - pin_bmp.xsize - 2 -
   close_bmp.xsize - 5, bmp_y+1, &pin_bmp );
