...
control_handle_t w_add_radio_button(window_handle_t w, int id, int group_id, int x, int y)
{
	...
	// ctl_img_copy_and_blend( &cb.pas_bg_image, &cb.pas_bg_alloc,
	// &checkbox_round_off_a_x30_bmp, w, &cb ); ctl_img_copy_and_blend( &cb.act_bg_image,
	// &cb.act_bg_alloc, &checkbox_round_on_a_x30_bmp, w, &cb );

	control_handle_t bh = w_add_control(w, &cb);

	w_control_set_background(
		w, bh, &checkbox_round_off_a_x30_bmp, &checkbox_round_on_a_x30_bmp, 0);
...
int ctl_button_events(control_t *cc, struct foreach_control_param *env)
{
	...
	if ((e.type == UI_EVENT_TYPE_MOUSE) && point_in_rect(e.rel_x, e.rel_y, &cc->r)) {
		// LOG_FLOW( 1, "button @ %d.%d in range id %x", env->e.rel_x, env->e.rel_y,
		// cc->id ); cc->mouse_in_bits |= 1;

		if (env->e.m.clicked & 0x1)  // First button click only
		{
			/*
						if( cc->flags & CONTROL_FLAG_TOGGLE)
							cc->state = (cc->state == cs_pressed) ? cs_released :
			   cs_pressed; // toggle else cc->state = cs_pressed;
		...
		return 0;  // Do not eat events on just hover - prevents others from loosing hover
				   // status
	}
