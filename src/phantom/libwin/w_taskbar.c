...
void init_task_bar(void)
{
	...
	task_bar_window = drv_video_window_create(scr_get_xsize(),
											  TB_HEIGHT,
											  0,
											  0,
											  tb_bg,
											  "Task Bar",
											  WFLAG_WIN_ONTOP | WFLAG_WIN_NOKEYFOCUS);

	w_fill(task_bar_window,
		   tb_bg);  // Crashes with SEGV inusermode on mem fill (rep stos)

	int lb_x = scr_get_xsize();
	lb_x -= slide_switch_alpha_v31_off_bmp.xsize + 5;

	control_handle_t shutdown = w_add_button(task_bar_window,
											 -1,
											 lb_x,
											 8,
	...
	w_control_set_background(task_bar_window,
							 shutdown,
							 &slide_switch_alpha_v31_off_bmp,
...
static void task_bar_callback(window_handle_t w, struct control *cc)
{
	...
	if (!iw_is_top(e->w)) {
		...
		if (cc->state != cs_pressed)  // TODO quite a hack? call method? Need internal
									  // ictl_set_state( cc, state )?
		{
			cc->state = cs_pressed;
...
taskbar_handle_t w_add_to_task_bar_ext(window_handle_t w,
									   drv_video_bitmap_t *icon,
									   ...
									   drv_video_bitmap_t *h_bmp)
{
	...
	bh = w_add_button(task_bar_window,
					  0,
					  task_bar_next_x,
					  task_bar_next_y,
					  n_bmp,
...
{
	return w_add_to_task_bar_ext(w,
								 icon,
...
static void create_start_menu(void)
{
	...
	start_menu_window = drv_video_window_create(200,
												186,
												9,
												TB_HEIGHT,
												COLOR_WHITE,
	...
	w_control_set_background(
		lmw, bh, &slide_switch_alpha_v31_off_bmp, &slide_switch_alpha_v31_on_bmp, 0);
	...
	bh = w_add_button(task_bar_window,
					  ...
					  5,
					  5,
					  &start_button_normal_bmp,
					  &start_button_selected_bmp,
					  CONTROL_FLAG_NOBORDER | CONTROL_FLAG_TOGGLE);
