...
{
	// ph_printf("defaultMouseEventProcessor buttons %x, %d-%d\r", e->m.buttons, e->abs_x,
	// e->abs_y);
...
static int defaultWinEventProcessor(drv_video_window_t *w, struct ui_event *e)
{
	...
	switch (e->w.info) {
		...
		case UI_EVENT_WIN_TO_TOP:
			w_to_top(w);
			break;
		case UI_EVENT_WIN_TO_BOTTOM:
			w_to_bottom(w);
			break;

		case UI_EVENT_WIN_MOVE:
			w_move(w, e->abs_x, e->abs_y);
			break;
...
int defaultWindowEventProcessor(drv_video_window_t *w, struct ui_event *e)
{
	...
	switch (e->type) {
		case UI_EVENT_TYPE_MOUSE:
			return defaultMouseEventProcessor(w, e);
		case UI_EVENT_TYPE_KEY:
			return defaultKeyEventProcessor(w, e);
		case UI_EVENT_TYPE_WIN:
			return defaultWinEventProcessor(w, e);
	}
