...
void w_move(drv_video_window_t *w, int x, int y)
{
	...
	if (((w->state & WSTATE_WIN_ROLLEDUP) && !(w->w_title)) ||
		(!(w->state & WSTATE_WIN_VISIBLE)))  // nothing to do
	{
		w->x = x;
		w->y = y;
	...
	if (have_hw_copy) {
		...
		video_drv->copy(src_r.x,
						src_r.y,
						dst_r.x,
						dst_r.y,
						imin(src_r.xsize, dst_r.xsize),
						imin(src_r.ysize, dst_r.ysize) - 1);  // why - 1?
...
void drv_video_window_move(drv_video_window_t *w, int x, int y)
{
	...
	if (w->w_decor) {
		...
		scr_zbuf_reset_square(
			w->w_decor->x, w->w_decor->y, w->w_decor->xsize, w->w_decor->ysize);
	} else
...
void w_move(drv_video_window_t *w, int x, int y)
{
	...
	if (w->w_title)
		scr_zbuf_reset_square(
			w->w_title->x, w->w_title->y, w->w_title->xsize, w->w_title->ysize);
#endif
	if (!(w->state & WSTATE_WIN_ROLLEDUP)) {
		scr_zbuf_reset_square(w->x, w->y, w->xsize, w->ysize);
