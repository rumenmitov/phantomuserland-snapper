 ...
 *
 * Default (software) mouse pointer drawing code. Used by drivers which have no mouse
 *acceleration.
 *
 **/
...
static void get_mouse()
{
	...
	interlock++;
	scr_bitblt_ms(
		screencopy->pixel, last_mouse_x, last_mouse_y, mpointer->xsize, mpointer->ysize);
...
// nipicker component.
__attribute__((deprecated)) static void put_mouse()
{
	if (mouse_on <= 0)
		return;
	if (mouse_is_on_screen)
