...
font_handle_t w_get_tt_font_mem(void *mem_font,
								size_t mem_font_size,
								const char *diag_font_name,
								int font_size);
...
static void w_tt_paint_char(window_handle_t win,
							const struct ttf_symbol *symb,
							int win_x,
							int win_y,
							int top,
							const rgba_t color)
...
// UTF-8 char!
void w_ttfont_draw_char(window_handle_t win,
						font_handle_t font,
						const char *str,
						const rgba_t color,
						int win_x,
...
void w_ttfont_draw_string(window_handle_t win,
						  font_handle_t font,
						  const char *str,
						  const rgba_t color,
						  int win_x,
						  int win_y)
{
	...
	w_ttfont_draw_string_ext(win, font, str, strLen, color, win_x, win_y, 0, 0);
}

/**
 *
 * \param[out] find_x Return x coordinate for the start (left margin) of given character
 *in a string we print \param[in] find_for_char Position in str of character we look x
 *coordinate for
 *
 * Coordinate returned in find_x is window relative, it includes win_x parameter value.
 *
 **/
void w_ttfont_draw_string_ext(window_handle_t win,
							  font_handle_t font,
							  const char *str,
							  size_t strLen,
							  const rgba_t color,
							  ...
							  int find_for_char)
{
	...
	// After lash char
	if (fx == -1)
		fx = symbols[numSymbols - 1].posX + symbols[numSymbols - 1].width + win_x + 2;
 ...
 *
 **/
void w_ttfont_string_size(font_handle_t font, const char *str, size_t strLen, rect_t *r)
{
	int i, rc;
...
)
{
	...
	LOG_FLOW(
		1, "w_ttfont_draw_string f '%s' sz %d\n", s->pe->font_name, s->pe->font_size);
 ...
 *
 **/
void w_ttfont_string_size_w(struct ttf_paint_state *s,  //< Workplace struct
							...
							rect_t *r)
{
	...
	LOG_INFO_(
		10, "left %d top %d bottom %d imageW %d", s->left, s->top, s->bottom, s->right);
	LOG_INFO_(2, "x %d y %d xsize %d ysize %d", r->x, r->y, r->xsize, r->ysize);
}
...
errno_t utf8to32(
	wchar_t *dest, const char *src, size_t destSize, size_t srcSize, size_t *count)
{
	int skip_total = 0;
	int numSymbols = 0;
	while ((skip_total < srcSize) && (numSymbols < destSize - 1)) {
		// const uint32_t charcode = str[i];
		int32_t charcode;
		ssize_t skip = utf8proc_iterate((const uint8_t *)(skip_total + src),
										srcSize - skip_total,
										&charcode);  // TODO check err code!!
		if (skip < 0)
			return EINVAL;
...
errno_t utf32to8(
	char *dest, const wchar_t *src, size_t destSize, size_t srcSize, size_t *count)
{
	int skip_total = 0;
	int numSymbols = 0;
...
static errno_t tt_lookup(pool_t *pool, void *el, font_handle_t handle, void *arg)
{
	...
	if ((req->font_type == cur->font_type) && (req->font_size == cur->font_size) &&
		(0 == ph_strcmp(req->font_name, cur->font_name))) {
		req->face = cur->face;
		req->fhandle = handle;
		return 1;  // found
	}
...
static int w_load_tt_from_file(FT_Face *ftFace, const char *file_name)
{
	...
	ph_snprintf(buf,
				sizeof(buf) - 1,
				"P:/phantomuserland/plib/resources/ttfonts/opensans/%s",
				file_name);
...
static int w_load_tt_from_mem(FT_Face *ftFace,
							  void *mem_font,
							  size_t mem_font_size,
							  const char *diag_font_name)
{
...
{
	OpenSans_Regular_ttf_font_handle = w_get_tt_font_mem( OpenSans_Regular_ttf_font,
OpenSans_Regular_ttf_font_size, "OpenSans Regular", 14 );
}
*/

font_handle_t w_get_system_font_ext(int font_size)
{
	return w_get_tt_font_mem(OpenSans_Regular_ttf_font,
							 OpenSans_Regular_ttf_font_size,
							 "OpenSans Regular",
							 font_size);
	// return w_get_tt_font_mem( OpenSans_SemiBold_ttf_font,
	// OpenSans_SemiBold_ttf_font_size, "OpenSans Semibold", font_size );
}
...
{
	return w_get_tt_font_mem(RobotoMono_Regular_ttf_font,
							 RobotoMono_Regular_ttf_font_size,
							 "RobotoMono Regular",
							 font_size);
}
...
static void dump_face( FT_Face ftFace )
{
	...
	ph_printf( "\tFaces %ld curr %ld glyphs %ld\n", fr->num_faces, fr->face_index,
fr->num_glyphs ); ph_printf( "\tFamily '%s' style '%s' face flags %lx style flags %lx\n",
