...
void phantom_dump_windows_buf(char *bp, int len)
{
...

	{
		...
		rc = ph_snprintf(bp,
						 len,
						 "\x1b[35m Size     Position  Zpoz Name (flags)\x1b[37m\n");
		bp += rc;
		len -= rc;
	}
	...
	{
		int pn = ph_snprintf(bp,
							 len,
							 "%s%4dx%-4d @%4dx%-4d z %2d %-20.20s\x1b[37m\n",
...
{
	// json_foreach( jo, "windows", phantom_kernel_threads, sizeof(phantom_thread_t *),
	// MAX_THREADS, json_encode_thread );
