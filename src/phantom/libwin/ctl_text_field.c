...
{
	LOG_FLOW(1,
			 "Cursor pos = %d, str_len = %d, vis_shift  = %d, vis_len = %d, buf='%s'",
			 cc->cursor_pos,
			 cc->str_len,
			 cc->vis_shift,
			 cc->vis_len,
...
static void find_visible_text_len(control_t *cc)
{
	...
	while (1) {
		...
		w_ttfont_string_size(
			decorations_title_font, cc->buffer + cc->vis_shift, cc->vis_len, &r);

		int vis_width_pixels = r.xsize +
							   5;  // TODO hack - +5 is to move away the right decoration
		LOG_FLOW(
			4, " vis_width_pixels %d, cc->r.xsize %d ", vis_width_pixels, cc->r.xsize);
	...
	if (cc->text_height <= 0) {
		w_ttfont_string_size(decorations_title_font, cc->buffer, cc->str_len, &r);
		cc->text_height = r.ysize;
	}
}
...
void ctl_text_field_paint(window_handle_t win, control_t *cc)
{
	...
	w_ttfont_draw_string_ext(win,
							 decorations_title_font,
							 cc->buffer + cc->vis_shift,
							 cc->vis_len,
							 cc->fg_color,
							 cc->r.x + t_ypos + 2,
							 cc->r.y + t_ypos,
							 &cursor_x_pos,
							 cc->cursor_pos - cc->vis_shift);  // +2?
		...
		switch (e.k.ch) {
			...
			case KEY_DELETE:
					...
					break;
				{
					...
					cc->buffer[cc->str_len] = 0;
					LOG_FLOW(
						2, "Delete ch done @%d buf='%s'", cc->cursor_pos, cc->buffer);
				}
#endif
				break;
			...
			default:
				...
				if (!KEY_IS_FUNC(e.k.ch)) {
					...
					LOG_FLOW(
						2, "Insert ch '%c' (%d) buf = '%s'", e.k.ch, e.k.ch, cc->buffer);
					LOG_FLOW(
						1,
						"Cursor pos = %d, str_len = %d, vis_shift  = %d, vis_len = %d",
						cc->cursor_pos,
						cc->str_len,
						cc->vis_shift,
						cc->vis_len);
					...
					// if(insert_mode)
					if ((cc->str_len > 0) &&
						(cc->cursor_pos < cc->str_len))  // Can't extend empty string
					{
						assert(len > 0);
