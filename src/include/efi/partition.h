...
#define LEN_EFI_PAD                                                         \
	(EFI_LABEL_SIZE - ((5 * sizeof(diskaddr_t)) + (7 * sizeof(u_int32_t)) + \
					   (8 * sizeof(char)) + (1 * (sizeof(struct uuid)))))
...
{
	u_int32_t PartitionAttrs : 16, Reserved2 : 16;
	u_int32_t Reserved1 : 31, RequiredPartition : 1;
} efi_gpe_Attrs_t;
