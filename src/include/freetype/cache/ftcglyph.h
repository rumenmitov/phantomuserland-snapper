...
FT_EXPORT(FT_Error)
FTC_GCache_Lookup(
	FTC_GCache cache, FT_UInt32 hash, FT_UInt gindex, FTC_GQuery query, FTC_Node *anode);
...
#define FTC_GCACHE_LOOKUP_CMP(cache, famcmp, nodecmp, hash, gindex, query, node, error) \
	...
	FTC_MRULIST_LOOKUP_CMP(                                                             \
		&_gcache->families, _gquery, _fcompare, _gquery->family, error);                \
	if (!error) {                                                                       \
...
#define FTC_GCACHE_LOOKUP_CMP(cache, famcmp, nodecmp, hash, gindex, query, node, error) \
	FT_BEGIN_STMNT                                                                      \
	error = FTC_GCache_Lookup(                                                          \
		FTC_GCACHE(cache), hash, gindex, FTC_GQUERY(query), (FTC_Node *)&(node));       \
	FT_END_STMNT
