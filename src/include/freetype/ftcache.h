...
#define FTC_FONT_COMPARE(f1, f2)                                             \
	((f1)->face_id == (f2)->face_id && (f1)->pix_width == (f2)->pix_width && \
	 (f1)->pix_height == (f2)->pix_height)

#define FT_POINTER_TO_ULONG(p) ((FT_ULong)(FT_Pointer)(p))

#define FTC_FACE_ID_HASH(i) \
	((FT_UInt32)((FT_POINTER_TO_ULONG(i) >> 3) ^ (FT_POINTER_TO_ULONG(i) << 7)))

#define FTC_FONT_HASH(f)                                                 \
	(FT_UInt32)(FTC_FACE_ID_HASH((f)->face_id) ^ ((f)->pix_width << 8) ^ \
				((f)->pix_height))
...
#define FTC_IMAGE_TYPE_COMPARE(d1, d2) \
	(FTC_FONT_COMPARE(&(d1)->font, &(d2)->font) && (d1)->flags == (d2)->flags)

#define FTC_IMAGE_TYPE_HASH(d) (FT_UFast)(FTC_FONT_HASH(&(d)->font) ^ ((d)->flags << 4))
