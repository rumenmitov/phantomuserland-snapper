...
#define STAT_CNT_PERSISTENT_DA_SIZE \
	(MAX_STAT_COUNTERS * sizeof(struct persistent_kernel_stats))
...
#define STAT_INC_CNT(___nCounter)                \
	do {                                         \
...
#define STAT_INC_CNT_N(___nCounter, ___val)       \
	do {                                          \
		assert(___nCounter < MAX_STAT_COUNTERS);  \
