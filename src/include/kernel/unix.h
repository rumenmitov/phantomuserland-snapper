...
int usys_stat(
	int *err, uuprocess_t *u, const char *path, struct stat *data, int statlink);
...
errno_t usys_setproperty(
	int *err, uuprocess_t *u, int fd, const char *pName, const char *pValue);
errno_t usys_getproperty(
	int *err, uuprocess_t *u, int fd, const char *pName, char *pValue, size_t vlen);
errno_t usys_listproperties(
	int *err, uuprocess_t *u, int fd, int nProperty, char *buf, int buflen);
...
int usys_getsockname(
	int *err, uuprocess_t *u, int fd, struct sockaddr *name, socklen_t *namelen);
int usys_getpeername(
	int *err, uuprocess_t *u, int fd, struct sockaddr *name, socklen_t *namelen);


int usys_getsockopt(int *err,
					uuprocess_t *u,
					int fd,
					int level,
					int optname,
...
int usys_bind(
	int *err, uuprocess_t *u, int fd, const struct sockaddr *my_addr, socklen_t addrlen);
int usys_listen(int *err, uuprocess_t *u, int fd, int backlog);
int usys_accept(
	int *err, uuprocess_t *u, int fd, struct sockaddr *acc_addr, socklen_t *addrlen);
int usys_connect(
	int *err, uuprocess_t *u, int fd, const struct sockaddr *ia, socklen_t addrlen);

int usys_socketpair(
	int *err, uuprocess_t *u, int domain, int type, int protocol, int socket_vector[2]);


ssize_t usys_recv(int *err, uuprocess_t *u, int fd, void *buf, size_t len, int flags);
ssize_t usys_recvfrom(int *err,
					  uuprocess_t *u,
					  int fd,
					  void *buf,
					  size_t len,
...
ssize_t usys_send(
	int *err, uuprocess_t *u, int fd, const void *buf, size_t len, int flags);
ssize_t usys_sendto(int *err,
					uuprocess_t *u,
					int fd,
					const void *buf,
					size_t len,
					...
					socklen_t tolen);
ssize_t usys_sendmsg(
	int *err, uuprocess_t *u, int fd, const struct msghdr *msg, int flags);
