...
#define PTCHSToLBA(c, h, s, scnt, hcnt) \
	((s) & 0x3f) + (scnt) * ((h) + (hcnt) * ((c) | (((s) & 0xc0) << 2)))
...
typedef enum PartitionTypes
{
	...
	PTPhantom = PHANTOM_PARTITION_TYPE_ID
} partition_type_t;

#define PartitionIsExtended(P) ((P)->PartitionType == PTDosExtended)
#define PartitionIsPhantom(P)  ((P)->PartitionType == PTPhantom)
