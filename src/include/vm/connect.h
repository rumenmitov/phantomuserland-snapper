...
void vm_lock_persistent_memory(
	void);  // request access to persistent memory address space, prevent snapshots
void vm_unlock_persistent_memory(
	void);  // release access to persistent memory address space, enable snapshots
...
#define CON_F_PROTOS(__short)                                             \
	errno_t cn_##__short##_do_operation(int op_no,                        \
										struct data_area_4_connection *c, \
										struct data_area_4_thread *tc,    \
										pvm_object_t o);                  \
	errno_t cn_##__short##_init(struct data_area_4_connection *c,         \
								struct data_area_4_thread *tc,            \
								const char *suffix);                      \
