...
struct internal_class
{
	...
	gc_iterator_func_t
		iter;  // Call func for all the object pointer contained in this internal object
	gc_finalizer_func_t finalizer;
	o_restart_func_t
		restart;  // Called on OS restart if object put itself to restart list, see root.c
	int da_size;
	int flags;
...
// #define DEF_SIZE(cn)    const int n_syscall_table_4_##cn = (sizeof
// syscall_table_4_##cn) / sizeof(syscall_func_t);
#define DECLARE_SIZE(__cn)       \
	int n_syscall_table_4_##__cn \
		__attribute__((used)) = (sizeof syscall_table_4_##__cn) / sizeof(syscall_func_t)
