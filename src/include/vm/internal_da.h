...
#define pvm_object_da(o, type)                                             \
	({                                                                     \
		if (!pvm_object_class_exactly_is(o, pvm_get_##type##_class()))     \
			pvm_exec_panic0("type err for " pvm_object_da_tostring(type)); \
		((struct data_area_4_##type *)&(o->da));                           \
	})
/** Extract (typed) object data area pointer from object pointer. */
...
// #  define VM_SPIN_LOCK(___var) ({ hal_disable_preemption(); atomic_add(&(___var),1);
// assert( (___var) == 1 ); }) #  define VM_SPIN_UNLOCK(___var) ({
// atomic_add(&(___var),-1); hal_enable_preemption(); assert( (___var) == 0 ); })
...
struct data_area_4_call_frame
{
	...
	// pvm_object_t                      cs;     // code segment, ref just for gc - OK
	// without
...
struct data_area_4_class
{
	...
	unsigned int
		object_data_area_size;  // object of this class will have data area of this size
	pvm_object_t object_default_interface;  // default one

	unsigned int
		sys_table_id;  // See above - index into the kernel's syscall tables table
...
struct data_area_4_tty
{
	...
	int font_height;  // Font is hardcoded yet - BUG - but we cant have ptr to kernel from
					  // object
	int font_width;
...
#if WEAKREF_SPIN
	pvm_spinlock_t
		lock;  // interlocks access tp object from GC finalizer and from getting ref
#else
	hal_mutex_t mutex;
#endif
};
...
errno_t hdir_find(hashdir_t *dir,
				  const char *ikey,
				  size_t i_key_len,
				  pvm_object_t *out,
				  int delete_found);
...
struct data_area_4_thread
{
	...
	VM_SPIN_TYPE
		*spin_to_unlock;      // This spin will be unlocked after putting thread asleep
	pvm_object_t cond_mutex;  // If we wait for cond, this mutex was unlocked by cond and
							  // has to be relocked
#endif
	// timedcall_t                         timer;          // Who will wake us
	// net_timer_event                     timer;          // Who will wake us
	// pvm_object_t                        sleep_chain;    // More threads sleeping on the
	// same event, meaningless for running thread
	...
	struct data_area_4_exception_stack
		*_estack;  // Loaded by load_fast_acc from call_frame


	// misc data
	int stack_depth;  // number of frames
	// long                              memory_size;    // memory allocated - deallocated
	// by this thread
};
...
struct pvm_connection_ops
{
	...
	errno_t (*check_operation)(int op_no,
							   struct data_area_4_connection *c,
							   struct data_area_4_thread *tc);

	// request to wake me up when ready
	errno_t (*req_wake)(int op_no,
						struct data_area_4_connection *c,
						struct data_area_4_thread *tc);
#endif

	// Actually do op
	errno_t (*do_operation)(int op_no,
							struct data_area_4_connection *c,
							struct data_area_4_thread *tc,
							pvm_object_t o);

	// Init connection
	errno_t (*init)(struct data_area_4_connection *c,
					struct data_area_4_thread *tc,
					const char *suffix);

	// Finish connection
	// errno_t     (*disconnect)( struct data_area_4_connection *c, struct
	// data_area_4_thread *tc );
...
errno_t phantom_connect_object(struct data_area_4_connection *da,
							   struct data_area_4_thread *tc);
// errno_t phantom_disconnect_object( struct data_area_4_connection *da, struct
// data_area_4_thread *tc);
...
// int pvm_connect_object_internal(struct data_area_4_connection *da, int connect_type,
// pvm_object_t host_object, void *arg);
errno_t phantom_connect_object_internal(struct data_area_4_connection *da,
										int connect_type,
										pvm_object_t host_object,
										void *arg);
