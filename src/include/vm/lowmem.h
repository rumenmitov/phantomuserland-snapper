...
struct vm_lowmem
{
	...
	u_int64_t
		first_unused_ptr;  // pointer to first unused byte (place to alloc next arena)
};

#define VM_ARENA_FLAG_STATIC \
	(1 << 0)  // Objects supposed to be not deleted at all (not enforced, though)
...
#define VM_ARENA_FLAG_LARGE_SINGLE \
	(1 << 20)  // Allocation of very large object - one per arena
...
errno_t vm_allocate_arena(vm_arena_t **a,
						  int flags,
						  native_t size);  // size -1 for automatic
