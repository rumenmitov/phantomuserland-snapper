...
struct object_PVM_ALLOC_Header
{
	...
	volatile int32_t refCount;  //< for fast dealloc of locally-owned objects. If grows to
								//INT_MAX it will be fixed at that value and ignored
								//further. Such objects will be GC'ed in usual way
	unsigned char alloc_flags;
	unsigned char gc_flags;
	size_t exact_size;  //< full object size including this header
};
...
struct pvm_object_storage
{
	...
	struct pvm_object_storage
		*_satellites;  // Points to chain of objects related to this one
	u_int32_t _flags;
...
// #define pvm_class_check(__o,__c) if(!pvm_object_class_is( __o, __c )) pvm_panic("Wrong
// class");
