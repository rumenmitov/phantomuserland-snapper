...
// #define SYSCALL_THROW(obj) do { pvm_ostack_push( tc->_ostack, obj ); return 0; }
// while(0)
#define SYSCALL_THROW(obj) \
	({                     \
		*ret = (obj);      \
		return 0;          \
	})
...
// #define DECLARE_SIZE(class) int n_syscall_table_4_##class =	(sizeof
// syscall_table_4_##class) / sizeof(syscall_func_t) #define DECLARE_SIZE(class)
...
#define ASSERT_INT(obj)                                          \
	do {                                                         \
		if (!IS_PHANTOM_INT(obj))                                \
			SYSCALL_THROW_STRING("not an integer arg: " __FILE__ \
								 ":" __XSTRING(__LINE__));       \
	} while (0)
...
#define SYSCALL_PUT_THIS_THREAD_ASLEEP(__unl_spin) \
	phantom_thread_put_asleep(tc, (__unl_spin))
#define SYSCALL_WAKE_THIS_THREAD_UP() phantom_thread_wake_up(tc)

#define SYSCALL_PUT_THREAD_ASLEEP(__thread, __unl_spin) \
	phantom_thread_put_asleep((__thread), (__unl_spin))
...
typedef int (*syscall_func_t)(pvm_object_t o,
							  pvm_object_t *ret,
...
#define DECLARE_SYSTABLE(type,tab_id) \
	...
	void pvm_syscall_init_##tab_id() { pvm_exec_systables[tab_id] =
(syscall_table_4_##type); }
*/
...
int si_void_1_destruct(pvm_object_t o,
					   ...
					   int n_args,
					   pvm_object_t *args);
int si_void_2_class(pvm_object_t o,
					pvm_object_t *ret,
					struct data_area_4_thread *tc,
					int n_args,
					pvm_object_t *args);
int si_void_3_clone(pvm_object_t o,
					...
					int n_args,
					pvm_object_t *args);
int si_void_4_equals(pvm_object_t o,
					 pvm_object_t *ret,
					 struct data_area_4_thread *tc,
					 int n_args,
					 pvm_object_t *args);
...
int si_void_7_fromXML(pvm_object_t o,
					  ...
					  int n_args,
					  pvm_object_t *args);
int si_void_8_def_op_1(pvm_object_t o,
					   pvm_object_t *ret,
					   struct data_area_4_thread *tc,
					   int n_args,
					   pvm_object_t *args);
int si_void_9_def_op_2(pvm_object_t o,
					   pvm_object_t *ret,
					   struct data_area_4_thread *tc,
					   int n_args,
					   pvm_object_t *args);
int si_void_15_hashcode(pvm_object_t o,
						pvm_object_t *ret,
						struct data_area_4_thread *tc,
						int n_args,
						pvm_object_t *args);
