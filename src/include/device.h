...
struct phantom_dev_ops
{
	...
	int (*sendw)(struct phantom_device *dev,
				 pvm_object_t data,
				 struct data_area_4_thread *tc);
	int (*recvw)(struct phantom_device *dev,
				 pvm_object_t data,
				 struct data_area_4_thread *tc);
	...
	// rich man's ioctl
	errno_t (*getproperty)(struct phantom_device *dev,
						   const char *pName,
						   char *pValue,
						   int vlen);
	errno_t (*setproperty)(struct phantom_device *dev,
						   const char *pName,
						   const char *pValue);
	errno_t (*listproperties)(struct phantom_device *dev,
							  int nProperty,
							  char *pValue,
							  int vlen);
};
...
#define phantom_bus_add_dev(__bus, __dev) \
	do {                                  \
		(__dev)->next = (__bus)->devices; \
		(__bus)->devices = (__dev);       \
		(__dev)->bus = (__bus);           \
		devfs_register_dev(__dev);        \
	} while (0)
#define phantom_bus_add_bus(bus, child_bus) \
	do {                                    \
		(child_bus)->next = (bus)->buses;   \
		(bus)->buses = (child_bus);         \
	} while (0)
