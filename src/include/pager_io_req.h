...
typedef struct pager_io_request
{
	...
	disk_page_no_t
		disk_page;  // disk address in pages - as pager requested (ignored by io code)
	...
	u_int64_t blockNo;  // disk sector (usually 512-byte) no - this is what real io code
						// looks at
	...
	//    unsigned char       flag_ioerror; 		// BUG - not used yet - TODO replace
	//    by rc below
	...
	int unit;  // Used on devices that have one q but multiple units that can't work at
			   // once
...
static __inline__ void pager_io_request_init(pager_io_request *me)
{
