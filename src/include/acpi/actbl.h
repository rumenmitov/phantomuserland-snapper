...
typedef struct acpi_table_facs
{
	...
	UINT32 Flags;
	UINT64 XFirmwareWakingVector; /* 64-bit version of the Firmware Waking Vector
									 (ACPI 2.0+) */
...
#define ACPI_FADT_FIXED_RTC \
	(1 << 6) /* 06: [V1] RTC wakeup status not in fixed register space */
...
#define ACPI_FADT_SLEEP_TYPE                                                      \
	(1 << 13) /* 13: [V3] Must execute native instruction after writing  SLP_TYPx \
				 register */
