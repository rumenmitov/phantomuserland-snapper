...
void t_set_paged_mem(
	bool enable);  //< Enable or disable access to paged memory - calls arch pagemap func.
#endif

void vm_release_snap_lock(
	int count);  // update vm_{un}lock_persistent_memory counter on thread death.
...
#define THREAD_FLAG_VM \
	0x0002  // runs virtual machine thread, owner points to phantom thread object?? GC?!
...
#define THREAD_FLAG_UNDEAD \
	0x0200  // This thread can't be killed for some reason. Usually it's some special one
			// like CPU idle thread.
#define THREAD_FLAG_NOSCHEDULE \
	0x0400  // Must not be selected by scheduler in usual way - per CPU 'parking' (idlest)
			// thread

#define THREAD_FLAG_SNAPPER 0x1000  // I am is a snapper thread
// #define THREAD_FLAG_SNAP_OK     0x2000 // I am paused so snapper can set pages
// writeonly

#define THREAD_FLAG_HAS_PORT \
	0x10000  // This thread (possibly) owns port (see newos/ports)

// Can be passed to thread creation function
#define CREATION_POSSIBLE_FLAGS                                                 \
	(THREAD_FLAG_USER | THREAD_FLAG_VM | THREAD_FLAG_JIT | THREAD_FLAG_NATIVE | \
	 THREAD_FLAG_KERNEL)
...
tid_t hal_start_kernel_thread_arg_dh(void (*thread)(void *),
									 void *arg,
...
//! Returns 'owner' field, which is, for VM threads, is used to keep pointer to VM thread
//! object
