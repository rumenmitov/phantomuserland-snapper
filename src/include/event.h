...
#define UI_MOD_DOWN(__mod)             \
	({                                 \
		...
		if (m & UI_MODIFIER_SHIFT)     \
			yn = 0;                    \
		if (m & UI_MODIFIER_ALT)       \
			yn = 0;                    \
		if (m & UI_MODIFIER_CTRL)      \
			yn = 0;                    \
		yn;                            \
	})
...
#define UI_EVENT_GLOBAL_REPAINT_RECT \
	(0xFF00 | 1)  //! Repaint all windows that intersect w. rect
#define UI_EVENT_GLOBAL_COPY_RECT \
	(0xFF00 | 2)  //! TODO Copy part of screen to other place, rect is scr, rect2.x/y is
				  //! dest pos - FOR 2D ACCELERATOR
#define UI_EVENT_GLOBAL_FILL_RECT \
	(0xFF00 | 3)  //! TODO Fill part of screen with color, rect is scr pos, color is color
				  //! - FOR 2D ACCELERATOR
