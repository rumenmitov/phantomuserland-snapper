...
#define E2K_LARGE_PAGE_SHIFT \
	((machine.iset_ver <= E2K_ISET_V2) ? (E2K_4M_PAGE_SHIFT) : (E2K_2M_PAGE_SHIFT))

#define BOOT_E2K_LARGE_PAGE_SHIFT \
	((boot_machine.iset_ver <= E2K_ISET_V2) ? (E2K_4M_PAGE_SHIFT) : (E2K_2M_PAGE_SHIFT))
 ...
 */
#define _PAGE_ALIGN_UP(addr, page_size) ((e2k_addr_t)(addr) & ~(page_size - 1))
...
#define E2K_SMALL_PAGE_ALIGN_DOWN(addr) _PAGE_ALIGN_DOWN(addr, E2K_SMALL_PAGE_SIZE)
...
#define E2K_LARGE_PAGE_ALIGN_DOWN(addr) _PAGE_ALIGN_DOWN(addr, E2K_LARGE_PAGE_SIZE)
#define E2K_LARGE_PAGE_ALIGN_UP(addr)   _PAGE_ALIGN_UP(addr, E2K_LARGE_PAGE_SIZE)
 ...
 */
#define clear_memory_8(addr, size)                               \
	recovery_memset_8(addr,                                      \
					  0,                                         \
					  ETAGEWD,                                   \
					  size,                                      \
...
#define clear_user_highpage(page, vaddr)   clear_page(page_address(page))
...
#else
#define boot_pa(x) \
	((READ_OSCUD_LO_REG().OSCUD_lo_base >= PAGE_OFFSET) ? __pa(x) : (void *)(x))
#define boot_va(x) \
	((READ_OSCUD_LO_REG().OSCUD_lo_base >= PAGE_OFFSET) ? __va(x) : (void *)(x))
...
#define boot_nodes_phys_mem ((node_phys_mem_t *)boot_vp_to_pp(nodes_phys_mem))
...
extern void __meminit memmap_init_node(unsigned long size,
									   int nid,
									   node_phys_mem_t *node_mem,
									   unsigned long zone,
									   unsigned long start_pfn);
#define __HAVE_ARCH_MEMMAP_INIT
extern void __meminit memmap_init(unsigned long size,
								  int nid,
								  unsigned long zone,
								  unsigned long start_pfn);
	...
	({                                                                       \
		...
		if (current->ptrace & PT_PTRACED) {                                  \
			...
			if (current->thread.flags & E2K_FLAG_32BIT)                      \
				__stack_def_flags |= USER_CODES_32_INDEX << VM_CUI_SHIFT;    \
			else                                                             \
				__stack_def_flags |= USER_CODES_START_INDEX << VM_CUI_SHIFT; \
		}                                                                    \
		__stack_def_flags;                                                   \
	})
...
#define i386_PAGE_TABLE                                                       \
	(i386_PAGE_PRESENT | i386_PAGE_RW | i386_PAGE_USER | i386_PAGE_ACCESSED | \
	 i386_PAGE_DIRTY)
#define i386_KERNPG_TABLE \
	(i386_PAGE_PRESENT | i386_PAGE_RW | i386_PAGE_ACCESSED | i386_PAGE_DIRTY)
...
#define i386_pte_index(a) (((a) >> i386_PAGE_SHIFT) & (i386_PTRS_PER_PTE - 1))

#define i386_pgd_index(a) (((a) >> i386_PGDIR_SHIFT) & (i386_PTRS_PER_PGD - 1))
#define i386_pte_offset_kernel(dir, a) \
	((i386_pte_t *)i386_pmd_page_kernel(*(dir)) + i386_pte_index(a))

#define i386_pmd_page_kernel(dir) (__va(i386_pgd_val(dir) & i386_PAGE_MASK))
...
extern e2k_addr_t user_address_to_phys(struct task_struct *tsk, e2k_addr_t address);
