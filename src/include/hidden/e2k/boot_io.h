...
extern void boot_conf_outb(int domain,
						   unsigned char bus,
						   unsigned char byte,
						   unsigned long port);
extern void boot_ioh_e3s_outb(int domain,
							  unsigned char bus,
							  unsigned char byte,
							  unsigned long port);
extern void boot_conf_outw(int domain,
						   unsigned char bus,
						   u16 halfword,
						   unsigned long port);
extern void boot_ioh_e3s_outw(int domain,
							  unsigned char bus,
							  u16 halfword,
							  unsigned long port);
extern u16 boot_conf_inw(int domain, unsigned char bus, unsigned long port);
extern void boot_conf_outl(int domain, unsigned char bus, u32 word, unsigned long port);
extern u32 boot_conf_inl(int domain, unsigned char bus, unsigned long port);
extern void boot_ioh_e3s_outl(int domain,
							  unsigned char bus,
							  u32 word,
							  unsigned long port);
extern unsigned char boot_conf_inb(int domain, unsigned char bus, unsigned long port);
