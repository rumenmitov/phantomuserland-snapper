...
struct uufileops
{
	...
	errno_t (*getproperty)(struct uufile *f,
						   const char *pName,
						   char *pValue,
						   size_t vlen);
...
#define CHECK_FD_RANGE(_fd)               \
	do {                                  \
		if (_fd < 0 || _fd > MAX_UU_FD) { \
			*err = EBADF;                 \
			return -1;                    \
		}                                 \
	} while (0)
#define GETF(_fd) (u->fd[_fd])
#define CHECK_FD_OPEN(__fd)    \
	do {                       \
		if (GETF(__fd) == 0) { \
			*err = EBADF;      \
			return -1;         \
...
errno_t auto_mount(const char *name,
				   uufs_t *fs,
				   char *out_mount_point,
				   size_t out_mount_point_buf_size,
				   int flags);
