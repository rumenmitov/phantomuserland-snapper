...
struct phantom_thread
{
	...
	int sub_from_pers_mem_lock_count;  // Times we incremented
...
// extern volatile int             all_threads_snap_lock; // nonzero = can't begin a
// snapshot


#define THREAD_SLEEP_USER \
	0x0001  // just paused by user's call - must be unpaused manually
 ...
 * clear unlock.
 */
void phantom_switch_context(phantom_thread_t *from,
							phantom_thread_t *to,
...
#define SET_CURRENT_THREAD(t)                      \
	({                                             \
		percpu_current_thread[GET_CPU_ID()] = (t); \
		(void)0;                                   \
	})
...
void phantom_scheduler_schedule_soft_irq(
	void);  // do not enforce, just make it come in some near future


// TODO in fact, possibly, it is ok just to call phantom_scheduler_soft_interrupt() from
// it?
