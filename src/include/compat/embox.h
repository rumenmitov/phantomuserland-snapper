...
// alloc and not map void        				hal_free_phys_pages(physaddr_t  page, int
// npages); // alloc and not map - WILL PANIC if page is mapped!


#define phymem_alloc(___np)                                \
	({                                                     \
		physaddr_t result = 0;                             \
		errno_t rc = hal_alloc_phys_pages(&result, ___np); \
		if (rc)                                            \
			result = 0;                                    \
		result;                                            \
	})
