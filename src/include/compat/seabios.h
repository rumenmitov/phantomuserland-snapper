...
#define GET_FARVAR(seg, var) (*((typeof(&(var)))MAKE_FLATPTR((seg), &(var))))
#define SET_FARVAR(seg, var, val)         \
	do {                                  \
		GET_FARVAR((seg), (var)) = (val); \
	} while (0)
#define GET_VAR(seg, var) (var)
#define SET_VAR(seg, var, val) \
	do {                       \
		(var) = (val);         \
	} while (0)
...
#define memcpy_far(__toseg, __tooff, __fromseg, __fromoff, __len) \
	memcpy(__tooff, __fromoff, __len)
...
#define foreachpci(BDF, MAX) \
	for (MAX = 0x0100, BDF = pci_next(0, &MAX); BDF >= 0; BDF = pci_next(BDF + 1, &MAX))
...
static inline void nullTrailingSpace(char *buf)
{
	int len = strlen(buf);
	char *end = &buf[len - 1];
	while (end >= buf && *end <= ' ')
...
static inline char *strtcpy(char *dest, const char *src, size_t len)
{
	char *d = dest;
	while (--len && *src != '\0')
		*d++ = *src++;
...
#define CLEARBITS_BDA(var, val) \
	do {                        \
		var &= ~val;            \
	} while (0)
#define SETBITS_BDA(var, val) \
	do {                      \
		var |= val;           \
	} while (0)
