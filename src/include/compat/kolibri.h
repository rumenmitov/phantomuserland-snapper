...
struct kolibri_thread_info
{
	...
	u_int16_t
		ecx_win_slot;  // GOD HELP US... read http://wiki.kolibrios.org/wiki/SysFn09/ru
	u_int16_t reserved1;
	...
	u_int16_t state;  // 0 - run, 1 - blocked, 2 - blokc + wait 4 event??, 3 - zombie, 4 -
					  // exception zombie, 5 - wait 4 event, 9 - slot is empty
	u_int16_t reserved3;
