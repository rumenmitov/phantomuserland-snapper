...
Walkqid *devwalk(Chan *c,
				 Chan *nc,
				 char **name,
				 int nname,
				 Dirtab *dir,
				 int ndir,
				 int (*gen)(Chan *c, char *, Dirtab *tab, int ntab, int i, Dir *dp));
int devstat(Chan *c,
			uchar *dp,
			int n,
			Dirtab *dir,
...
void devdir(
	Chan *c, Qid *qid, char *path, int _x1, it _whatTheFuckIs_eve, int perm, Dir *dp);
...
void intrenable(
	int irq_no, void (*intr)(Ureg *, void *), int _x, int _bus, const char *name);
// errno_t
