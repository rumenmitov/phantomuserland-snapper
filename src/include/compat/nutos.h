...
/*
static inline int NutUdpReceiveFrom( void *prot_data, uint32_t *raddr, uint16_t *rport,
void *buf, size_t bufsz, int flags )
{
	i4sockaddr saddr;

	//ssize_t rc = udp_recvfrom( prot_data, void *buf, ssize_t len, i4sockaddr *saddr, int
flags, bigtime_t timeout); ssize_t rc = udp_recvfrom( prot_data, buf, bufsz, &saddr, 0,
0);
...
#define NutThreadCreate(__name, __f, __a, _stk) hal_start_kernel_thread_arg((__f), (__a))
