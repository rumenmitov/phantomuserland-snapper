...
errno_t dpart_write_block(partition_handle_t h,
						  const void *from,
						  long blockNo,
						  int nBlocks);
...
errno_t dpart_write_sector(partition_handle_t h,
						   const void *from,
						   long sectorNo,
						   int nSectors);
...
struct phantom_disk_partition
{
	...
	time_t last_snap;  // 0 or time of last snapshot to this partition (for autoselecting
					   // swap part)
	...
	//! Sync - return after all prev reqs are done - PHYSICALLY! Ie not only reported as
	//! such, but really written!
...
phantom_disk_partition_t *phantom_create_partition_struct(
	phantom_disk_partition_t *base,
	long shift,
	long size);  // __attribute__((deprecated));

errno_t phantom_register_disk_drive(phantom_disk_partition_t *p);


// These work with 4096 byte blocks
errno_t phantom_sync_read_block(phantom_disk_partition_t *,
								void *to,
								long blockNo,
								int nBlocks);
errno_t phantom_sync_write_block(phantom_disk_partition_t *,
								 const void *from,
								 long blockNo,
								 int nBlocks);

// These work with native sized (512byte) sectors
errno_t phantom_sync_read_sector(phantom_disk_partition_t *,
								 void *to,
								 long sectorNo,
								 int nSectors);
errno_t phantom_sync_write_sector(phantom_disk_partition_t *,
								  const void *from,
								  long sectorNo,
								  int nSectors);
...
extern errno_t partAsyncIo(struct phantom_disk_partition *p,
						   pager_io_request *rq);  // disk_pool.c
extern errno_t partTrim(struct phantom_disk_partition *p,
						pager_io_request *rq);  // disk_pool.c
