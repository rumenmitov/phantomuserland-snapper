...
/**
 *
 * This header defines (very unclear and chaotic at this moment) hardware abstraction
 *layer. It is supposed (but not really achieved) that everything 'above' this layer is
 *hardware independent.
 *
 * TODO: Bring here all the code that is different between kernel and Windows-based
...
typedef enum page_mapped_t
{
	page_unmap = 0,
	page_map = 1,
	page_map_io = 2
} page_mapped_t;
typedef enum page_access_t
{
	page_noaccess = 0,
	page_readonly = 1,
	page_readwrite = 2,
...
// paging support
static __inline__ unsigned int hal_min_pagesize(void)
{
	return __MEM_PAGE;
}
static __inline__ unsigned int hal_mem_pagesize(void)
{
	return __MEM_PAGE;
}
void *hal_paged_space(void);  // where paged memory starts
void hal_grow_paged_space(unsigned add_bytes);
static __inline__ int hal_address_is_aligned(void *addr)
{
	return (((addr_t)addr) % hal_min_pagesize()) == 0;
}


long hal_phys_mem_4_paging(void);  // how much of phys mem is available for paging at all
long hal_free_phys_mem_4_paging(
	void);  // how much of phys mem is available for paging is left unmapped now

void hal_page_control(physaddr_t p,
					  void *page_start_addr,
					  page_mapped_t mapped,
					  page_access_t access);
void hal_pages_control(physaddr_t p,
					   void *page_start_addr,
					   int n_pages,
					   page_mapped_t mapped,
					   page_access_t access);
...
int32_t arch_switch_pdir(
	bool paged_mem_enable);  //< NB! DO NOT CALL, used in t_set_paged_mem() ONLY
int32_t arch_get_pdir(bool paged_mem_enable);  //< Get pdir (cr3 value) for given mode
int arch_is_object_land_access_enabled(
	void);  //< check if current thread attempts to access object space having access
			//disabled
#endif
...
errno_t hal_alloc_phys_page(
	physaddr_t *result);  // alloc and not map -- returns 1 on success
void hal_free_phys_page(
	physaddr_t page);  // alloc and not map - WILL PANIC if page is mapped!

errno_t hal_alloc_phys_pages(physaddr_t *result, int npages);  // alloc and not map
void hal_free_phys_pages(
	physaddr_t page, int npages);  // alloc and not map - WILL PANIC if page is mapped!

errno_t hal_alloc_vaddress(void **result,
						   int n_pages);  // alloc address of a page, but not memory
...
void hal_register_page_fault_handler(void (*page_fault_handler)(void *address,
																int write));
 ...
 **/
// int                                   hal_irq_alloc_soft( int irq, int (*func)(void
// *arg), void *arg, int (*soft_func)(void *soft_arg), void *soft_arg, int is_shareable );
...
u_int64_t hal_get_interrupt_profiling_time();  // returns time in CPU/arch dependent
											   // ticks, on ia32 it is RDTSC instruction
