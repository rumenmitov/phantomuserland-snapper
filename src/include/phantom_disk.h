...
#define DISK_STRUCT_VERSION \
	(DISK_STRUCT_VERSION_MINOR | (DISK_STRUCT_VERSION_MAJOR << 16))
...
#define DISK_STRUCT_SB_OFFSET_LIST              \
	{                                           \
		0x10, 0x100, 0x220, 0x333, 0x500, 0x666 \
	}
	...
	typedef struct phantom_disk_superblock
	{
		...
		u_int32_t checksum;  // Simple sum of all the superblock bytes with this field is
							 // set to zero
		...
		disk_page_no_t
			free_start;  // number of the first block that was never allocated. This and
						 // following blocks are not in any list and are free.
		...
		disk_page_no_t
			boot_progress_page;  // Boot process is updating this page to track boot
								 // progress and recover from hangs - NOT IMPL
		disk_page_no_t boot_module[DISK_STRUCT_N_MODULES];  // List of blocks for boot
															// time modules, up to 32.
		...
		char sys_name[DISK_STRUCT_SB_SYSNAME_SIZE];  // 0-terminated description for
													 // bootloader menu
	...
	void phantom_disk_format(struct phantom_disk_superblock *sb,
							 unsigned int n_pages,
							 const char *sysname);
	int phantom_calc_sb_checksum(struct phantom_disk_superblock *sb);
	int superblocks_are_equal(const struct phantom_disk_superblock *a,
							  const struct phantom_disk_superblock *b);
...
#define N_REF_PER_BLOCK                          \
	((DISK_STRUCT_BS / sizeof(disk_page_no_t)) - \
	 sizeof(struct phantom_disk_blocklist_head))
