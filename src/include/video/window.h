...
static __inline__ int drv_video_window_bytes(int xsize, int ysize)
{
	return (sizeof(rgba_t) * xsize * ysize * 2) + sizeof(drv_video_window_t);
}


// dynamic allocation
drv_video_window_t *drv_video_window_create(
	int xsize, int ysize, int x, int y, rgba_t bg, const char *title, int flags);
// free dynamically allocated window
void drv_video_window_free(drv_video_window_t *w);

// init for statically allocated ones
void drv_video_window_init(drv_video_window_t *w,
						   void *pixels,
						   int xsize,
						   int ysize,
						   int x,
...
void w_draw_blend_bitmap(drv_video_window_t *w,
						 int x,
						 int y,
						 drv_video_bitmap_t *bmp);  //< Draw with alpha blending
...
void w_replicate_ver(window_handle_t dest,
					 int destX,
					 int destY,
					 int nSteps,
					 const rgba_t *src,
...
void w_replicate_hor(window_handle_t dest,
					 int destX,
					 int destY,
					 int nSteps,
					 const rgba_t *src,
