...
void rgba2rgba_24_move(struct rgba_t *dest,
					   const struct rgba_t *src,
					   int nelem);  //! copies 24 bits, sets alpha byte to FF
...
void rgba2rgb_zbmove(
	struct rgb_t *dest, const struct rgba_t *src, zbuf_t *zb, int nelem, zbuf_t zpos);
void rgb2rgba_zbmove(
	struct rgba_t *dest, const struct rgb_t *src, zbuf_t *zb, int nelem, zbuf_t zpos);
void rgba2rgba_zbmove(
	struct rgba_t *dest, const struct rgba_t *src, zbuf_t *zb, int nelem, zbuf_t zpos);
void rgba2rgba_zbreplicate(
	struct rgba_t *dest, const struct rgba_t *src, zbuf_t *zb, int nelem, zbuf_t zpos);
void int565_to_rgba_zbmove(
	struct rgba_t *dest, const short int *src, zbuf_t *zb, int nelem, zbuf_t zpos);


void bitmap2bitmap(rgba_t *dest,
				   int destWidth,
				   int destHeight,
				   int destX,
				   int destY,
...
// + flip vert
void bitmap2bitmap_yflip(struct rgba_t *dest,
						 int destWidth,
						 int destHeight,
						 int destX,
						 int destY,
...
#ifdef ARCH_ia32
// void sse_rgba2rgba_zbmove(struct rgba_t *dest, const struct rgba_t *src, zbuf_t *zb,
// int nelem, zbuf_t zpos);
void auto_rgba2rgba_zbmove(
	struct rgba_t *dest, const struct rgba_t *src, zbuf_t *zb, int nelem, zbuf_t zpos);
#endif  // ARCH_ia32
...
typedef void (*rgba_move_func_t)(struct rgba_t *dest,
								 const struct rgba_t *src,
								 int nelem);

// Using mover
void bitmap2bitmap_generic(struct rgba_t *dest,
						   int destWidth,
						   int destHeight,
						   int destX,
						   int destY,
...
void bitmap2bitmap_blend(struct rgba_t *dest,
						 int destWidth,
						 int destHeight,
						 int destX,
						 int destY,
...
void bitmap2bitmap_blend_destalpha(struct rgba_t *dest,
								   int destWidth,
								   int destHeight,
								   int destX,
								   int destY,
 ...
 *
 * \param[inout] dst Image of control (button, text field, etc) background with aplha
 *channel \param[in] src Image of window background \param[in] x Shift of dst in src
 * \param[in] y Shift of dst in src
 *
 **/
void w_blend_bitmap_destalpha_shift(drv_video_bitmap_t *dst,
