...
struct drv_video_screen_t
{
	...
	errno_t (*accel)(void);  // Start driver in accelerating mode - video mode is already
							 // set by VESA, just add some acceleration to existing drv
...
#if VIDEO_DRV_WINBLT
#if NEW_WINDOWS
	void (*winblt)(window_handle_t from,
				   rect_t src,
				   int src_stride,
				   int dest_xpos,
				   int dest_ypos,
	...
	void (*bitblt_part)(const rgba_t *from,
						int src_xsize,
						int src_ysize,
						int src_xpos,
						int src_ypos,
...
extern void vid_bitblt_forw(const rgba_t *from,
							int xpos,
							int ypos,
							int xsize,
							int ysize,
							zbuf_t zpos,
							u_int32_t flags);
extern void vid_bitblt_rev(const rgba_t *from,
						   int xpos,
						   int ypos,
						   int xsize,
						   int ysize,
...
void vid_bitblt_part_forw(const rgba_t *from,
						  int src_xsize,
						  int src_ysize,
						  int src_xpos,
						  int src_ypos,
						  ...
						  zbuf_t zpos,
						  u_int32_t flags);
void vid_bitblt_part_rev(const rgba_t *from,
						 int src_xsize,
						 int src_ysize,
						 int src_xpos,
						 int src_ypos,
...
extern void vid_win_winblt_rev(const window_handle_t from,
							   int xpos,
							   int ypos,
							   zbuf_t zpos);
#endif
...
void vid_bitblt_worker(const struct rgba_t *from,
					   int xpos,
					   int ypos,
					   int xsize,
					   int ysize,
...
// zbuf_t zpos);
void vid_bitblt_part(const rgba_t *from,
					 int src_xsize,
					 int src_ysize,
					 int src_xpos,
					 int src_ypos,
...
void scr_mouse_disable_p(
	struct drv_video_screen_t *video_drv, int xpos, int ypos, int xsize, int ysize);
void scr_mouse_enable_p(
	struct drv_video_screen_t *video_drv, int xpos, int ypos, int xsize, int ysize);
