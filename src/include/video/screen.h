...
struct drv_video_screen_t {
  ...
  errno_t (*accel)(
      void); // Start driver in accelerating mode - video mode is already set by
             // VESA, just add some acceleration to existing drv
  int (*stop)(void); // Stop driver, switch to text mode. Can be called in
...
#if VIDEO_DRV_WINBLT
#if NEW_WINDOWS
  void (*winblt)(window_handle_t from, rect_t src, int src_stride,
                 int dest_xpos, int dest_ypos, zbuf_t zpos);
#else
  ...
  void (*copy)(int src_xpos, int src_ypos, int dst_xpos, int dst_ypos,
               int xsize, int ysize);

  // Screen rect clear
  void (*clear)(int xpos, int ypos, int xsize, int ysize);

  void (*bitblt_part)(const rgba_t *from, int src_xsize, int src_ysize,
                      int src_xpos, int src_ypos, int dst_xpos, int dst_ypos,
                      int xsize, int ysize, zbuf_t zpos, u_int32_t flags);
};
...
extern void vid_bitblt_forw(const rgba_t *from, int xpos, int ypos, int xsize,
                            int ysize, zbuf_t zpos, u_int32_t flags);
extern void vid_bitblt_rev(const rgba_t *from, int xpos, int ypos, int xsize,
                           int ysize, zbuf_t zpos, u_int32_t flags);

void vid_bitblt_part_forw(const rgba_t *from, int src_xsize, int src_ysize,
                          int src_xpos, int src_ypos, int dst_xpos,
                          int dst_ypos, int xsize, int ysize, zbuf_t zpos,
                          u_int32_t flags);
void vid_bitblt_part_rev(const rgba_t *from, int src_xsize, int src_ysize,
                         int src_xpos, int src_ypos, int dst_xpos, int dst_ypos,
                         int xsize, int ysize, zbuf_t zpos, u_int32_t flags);

#if !NEW_WINDOWS
extern void vid_win_winblt(const window_handle_t from, int xpos, int ypos,
                           zbuf_t zpos);
extern void vid_win_winblt_rev(const window_handle_t from, int xpos, int ypos,
                               zbuf_t zpos);
#endif
...
void vid_bitblt_reader(rgba_t *to, int xpos, int ypos, int xsize, int ysize,
                       int reverse);
void vid_bitblt_worker(const struct rgba_t *from, int xpos, int ypos, int xsize,
                       int ysize, int reverse, zbuf_t zpos, u_int32_t flags);

// void vid_bitblt_part(const rgba_t *from, int src_xsize, int src_ysize, int
// src_xpos, int src_ypos, int dst_xpos, int dst_ypos, int xsize, int ysize,
// zbuf_t zpos);
void vid_bitblt_part(const rgba_t *from, int src_xsize, int src_ysize,
                     int src_xpos, int src_ypos, int dst_xpos, int dst_ypos,
                     int xsize, int ysize, int reverse, zbuf_t zpos,
                     u_int32_t flags);

void scr_mouse_disable_p(struct drv_video_screen_t *video_drv, int xpos,
                         int ypos, int xsize, int ysize);
void scr_mouse_enable_p(struct drv_video_screen_t *video_drv, int xpos,
                        int ypos, int xsize, int ysize);
...
//! Switch video bitblt functions, used to read/write videomem, to 32 bit mode.
//! Default is 24 bit mode.
...
#define scr_readblt(from, xpos, ypos, xsize, ysize)                            \
  (video_drv->mouse_disable(),                                                 \
   video_drv->readblt(from, xpos, ypos, xsize, ysize),                         \
...
// These are special for mouse pointer code - they're not trying to disable
// mouse
#define scr_readblt_ms(from, xpos, ypos, xsize, ysize)                         \
  video_drv->readblt(from, xpos, ypos, xsize, ysize)
#define scr_bitblt_ms(from, xpos, ypos, xsize, ysize)                          \
  video_drv->bitblt(from, xpos, ypos, xsize, ysize, ZBUF_TOP, 0)
