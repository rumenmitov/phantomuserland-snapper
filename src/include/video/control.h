...
#define W_CONTROL_BS \
	64  // Size of text buffer iside a control struct - for save in persisten mem
...
#define CONTROL_FLAG_NOBACKGROUND \
	(1 << 9)  //< Do not paint control background - TODO check me
#define CONTROL_FLAG_READONLY \
	(1 << 10)  //< Control is just for display, don't accept user input - TODO implement
#define CONTROL_FLAG_ALT_FG \
	(1 << 11)  //< Control foreground (active part) is paint in a different way - TODO
#define CONTROL_FLAG_ALT_BG \
	(1 << 12)  //< Control background (passive part) is paint in a different way - TODO
...
typedef enum
{
	...
	// ct_group       = 0xFF+0,            //< To store it in an .internal.ui.control we
	// put group in a control union too.
} control_type_t;
...
typedef struct control
{
	...
	u_int32_t group_id;   //< If not zero, all controls with same group_id will be brought
						  //to same group
	...
	union
	{
		...
		struct
		{
			uint32_t maxlen;  //< Max __BYTES__ of text. Note UTF8 takes more. If o - use
							  //W_CONTROL_BS
	...
	uint32_t notify;  //< Notifications count - round bullet with number in the top right
					  //corner

	// control_group_t *   group;          //< Group we belong, if any
	// struct control *    next_in_group;  //< linked list of controls in group - radio or
	// menu
...
control_handle_t w_get_control_by_id(
	window_handle_t w, uint32_t id);  // To be able to access mass added controls

/// Same as w_add_control(), but actually USES given control_t structure, which supposed
/// to live in persistent storage.
pool_handle_t w_add_control_persistent(window_handle_t w, control_t *c);

/// Same as w_add_control_persistent(), but assumes we are restarting and tries to use
/// internal state too.
...
void w_control_set_callback(window_handle_t w,
							...
							void *callback_arg);
/// If just window is given, switch its visibility. If window and control - switch control
/// visibility.
void w_control_set_children(window_handle_t w,
							control_handle_t c,
							window_handle_t w_child,
							control_handle_t c_child);
void w_control_set_visible(window_handle_t w,
						   control_handle_t ch,
						   int visible);  // unimpl yet
...
void w_control_set_notify(window_handle_t w,
						  control_handle_t ch,
						  int count);  //< show bullet with number in top right corner
...
void w_control_set_state(window_handle_t w,
						 control_handle_t ch,
						 int pressed);  //< Is checkbox checked or switch turned on?
 ...
 * \param[in] width Width of scroll bar - how big part of possible values it takes. If ==
 *maxval - minval - takes 100% of scrollbar size
 *
...
control_handle_t w_add_button(window_handle_t w,
							  int id,
							  int x,
							  int y,
...
control_handle_t w_add_radio_button(
	window_handle_t w, int id, int group_id, int x, int y);
...
void w_reset_controls(window_handle_t w);  // focus lost, mouse off window - make sure all
										   // buttons are off -- TODO used?
int w_event_to_controls(
	window_handle_t w,
	ui_event_t *e);  // Deliver events to controls - return nonzero if event consumed
